                    ...
                    while self.running: #loop while race is running
                        try:
                            ...
                            for participant in participants:
                                ...
                                #print(f"Should I store Lap, current lap: {participant.get('mCurrentLap', 0)} Last lap: {last_lap_counts.get(participant['mName'], 1)}")
                                if participant.get('mCurrentLap', 0) > last_lap_counts.get(participant['mName'], 1):
                                    ...
                                    logging.info(f"Updated lap_times for {participant['mName']} {self.lap_times_dict[participant['mName']]}   at lap {participant.get('mCurrentLap', 0)}: {self.lap_times_dict[participant['mName']]}")

                                    if participant.get('mCurrentLap', 0) == 2 and participant.get('mLastLapTimes') == -123:
                                    ...
                                    if participant.get('mLastLapTimes') != -123: 
                                        ...
                                        self.db_queue.put(('insert_lap_data', (self.current_race_id, participant['mName'], participant.get('mCurrentLap', 0) - 1,participant.get('mLastLapTimes')))) #insert lap into Table
                                        self.lap_times_dict[participant['mName']].append(participant.get('mLastLapTimes', None))
                                        logging.info(f"Storing lap time for {participant['mName']}: Lap {participant.get('mCurrentLap', 0)}, Time {participant.get('mLastLapTimes', 0)},Last Lap:{participant.get('mLastLapTimes', None)} Driver Total Time: {driver_total_times[participant['mName']]} ")
                                        print(f"Storing lap time for {participant['mName']}: Lap {participant.get('mCurrentLap', 0)}, Time {participant.get('mLastLapTimes', 0)},Last Lap:{participant.get('mLastLapTimes', None)} Driver Total Time: {driver_total_times[participant['mName']]} ")
...
class RaceMonitorApp(QMainWindow):
    ...
    def delete_db(self):
        logging.info("Delete DB def entered")
    ...
    def update_live_view(self, data):
        ...
        if self.live_first_time_run: 
            ...
            logging.info(f"Number of participants: {len(sorted_participants)}")
    ...
    def calculate_score(self, races, participants, laps):
        ...
        for race in races:
            ...
            for driver, time in ranked_drivers:
                ...
                logging.info(f"Ranking: {ranked_drivers} Driver: {driver}, Flag: {driver_flags.get(driver)}, Time: {driver_total_times}, ")
    ...
    def load_selected_race(self): # Function to be called when the data is loaded
        ...
        if selected_index >= 0:
            ...
            #print(f"Signal Sent")
            logging.info(f"Load selected race Signal Sent")
    ...
    def set_delete_db_mode(self, is_active):
        logging.info("Def set_delete_db_mode entered")
        if is_active:
