    ...
    def update_config_file(self, gui_window):
        ...
        required_fields = {
            'config': {
                ...
                'monitor': '1'  # Default monitor to 1, can be changed in the GUI
            },
            'simsettings': {
                'sim-1': '127.0.0.1',
            },
...
class MonitorThread(QThread):
    ...
    async def connection_monitor(self):
        last_scan_time = time.monotonic() - 3600  # Initialize last scan time to 60 minutes ago
        ...
        while self.running:
            try:
                ...
                if time.monotonic() - last_scan_time >= 3600:
...
class RaceMonitorApp(QMainWindow):
    ...
    def base64_to_font_old(self, base64_str, filename):
        """Convert a base64 string to a QFont object."""
        os.makedirs(self.temp_dir, exist_ok=True)  # Ensure tmp/ exists
    ...
    '''
    def unused_setup_api_response_chart(self):         
        '''
    ...
    def update_driver_view(self, data):
        ...
        if self.local_record_updated:
            ...
            if self.deltalocalrecord < 0: 
                print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, deltalocalrecord: {self.deltalocalrecord}, worldrecorddb: {self.worldrecorddb}, trackworldrecord: {self.trackworldrecord}")
        ...
        if  not self.waiting_for_highscores:
            ...
            updates = [
                ...
                (self.car_txt.setText, (carname,)),
                (self.collison_txt.setText, (str(crashname),)),
                (self.speed_display.setText, (str(speed),)),
                (self.gear_display.setText, ('N' if gear == 0 else str(gear),)),
    ...
    def on_first_live_view_run(self, data):
        ...
        f"Session ID: {self.session_id} - World record: {(trackworldrecord if trackworldrecord != 0 else 'Mod Car/Track')}"
        )
                    ...
                    if self.classrace:
                        ...
                        if not self.waiting_for_highscores:
                            if carname in self.drivertrackrecorddb:
                                print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , WorldTrack Record: {self.trackworldrecord}, Local Record: {self.trackrecorddb}")
                        ...
                        if carname in self.drivertrackrecord:
                            print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record: {self.drivertrackrecord}")
    ...
    def identical_cars_old(self, data):
        if not data or 'participants' not in data or 'mParticipantInfo' not in data['participants']:
            return False
        participants = data['participants']['mParticipantInfo']
        if not participants:
    ...
    def update_heading(self, new_car, worldrecord, localrecord):
        print(f"RMA Line {inspect.currentframe().f_lineno} update_heading called with new_car: {new_car}, worldrecord: {worldrecord}, localrecord: {localrecord}, Current Text: {self.live_heading.text()}")
        ...
        if "Local Record: " in current_text:
            ...
            current_text = self.live_heading.text()
        if new_car != "skip":
            if "Car: " in current_text and not self.classrace:
