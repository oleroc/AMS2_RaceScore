from timeit import Timer; import atexit, sys, re, inspect, requests, sqlite3, queue, json, time, traceback, logging, os, asyncio, aiohttp, math, base64, configparser, tempfile
...
class ConfigManager:
    ...
    def update_config_file(self, gui_window):
        ...
        required_fields = {
            'config': {
                ...
                'testmode': 'False', # Enable test mode with simulated API data
        ...
        try:
            if not os.path.exists(self.config_file):
                ...
                if existing_version != self.__version__ and existing_version != None: # Check if the existing version matches the current version
                    # Check if this is a major/minor version change that needs handling
                    app_version_parts = self.__version__.split(".")
                    existing_version_parts = existing_version.split(".")
                    
                    if app_version_parts[1] != existing_version_parts[1] or app_version_parts[0] != existing_version_parts[0]:
                        # All version upgrades use automatic migration with data preservation
                        print(f"CM Line {inspect.currentframe().f_lineno} Version upgrade from {existing_version} to {self.__version__} - automatic database migration will handle compatibility.")
        ...
        with open(self.config_file, 'r+') as configfile:
            ...
            for line in lines: 
                ...
                if line.strip().startswith('ip_address'):
                    ...
                    configfile.write("; Minimum height is 800\n")
                ...
                elif line.strip().startswith('testmode'):
                    configfile.write("; Enable test mode to use simulated API data instead of real AMS2 (True/False)\n")
    ...
    def read_testmode_status(self):
        #config = configparser.ConfigParser()
        try:
            self.config.read(self.config_file) #Read the existing config file
            return self.config.get('config', 'testmode', fallback='False').strip().lower() == 'true' # Return the data from the config and convert to boolean
        except (configparser.Error, IOError) as e:
            print(f"CM Line {inspect.currentframe().f_lineno} An error occurred while accessing the config file: {e}")
            logging.info(f"CM Line {inspect.currentframe().f_lineno} An error occurred while accessing the config file: {e}")
            return False  # Default to False if error reading config   
...
class DatabaseThread(QThread):
    ...
    write_race_signal = pyqtSignal(object)  # Missing signal for race writing callbacks

    def __init__(self, task_queue, config_manager, mt_instance=None):
        super().__init__()
        self.task_queue = task_queue
        self.running = True
        ...
        self.mt_instance = mt_instance  # Reference to MultiThreadMonitor for active race checking
    ...
    def process_queue(self, operation, *args):
        try:
            ...
            elif operation == 'get_active_sessions': self.get_active_sessions(*args)
            elif operation == 'get_session_participants': self.get_session_participants(*args)
            elif operation == 'check_session_has_active_race': self.check_session_has_active_race(*args)
            elif operation == 'update_ip_name_flag': self.update_ip_name_flag(*args)
            elif operation == 'mark_session_complete': self.mark_session_complete(*args)
            elif operation == 'cleanup_old_sessions': self.cleanup_old_sessions(*args)
            else: 
    ...
    def create_database(self):
        # Check current database version and ensure it's v1.7.0
        migration_ok = self.check_and_migrate_database()
        if not migration_ok:
            # Migration utility is required - application will exit
            return
        
        # Create main tables with enhanced schema for v1.7.0
        self.cursor.execute('''
    ...
    def check_and_migrate_database(self):
        ...
        try:
            # Check if DatabaseMetadata table exists
            self.cursor.execute('''
            ...
            if not metadata_exists:
                # This is likely a pre-1.7.0 database
                current_version = self.detect_legacy_database_version()
                if current_version != '1.7.0':
                    self.show_migration_required_message(current_version)
                    return False
            else:
                ...
                current_version = result[0] if result else '1.0.0'
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Current database version: {current_version}")
            
            return True
                
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Database version check failed: {e}")
            print(f"DB Line {inspect.currentframe().f_lineno} Database version check failed: {e}")
            return True  # Continue anyway

    def detect_legacy_database_version(self):
        """Detect the version of a legacy database based on its structure"""
        try:
    ...
    def show_migration_required_message(self, current_version):
        """Show migration required message and exit"""
        try:
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Database Migration Required")
            msg_box.setIcon(QMessageBox.Warning)
            
            msg_box.setText(f"Database Upgrade Required")
            
            detailed_text = (
                f"Your database (v{current_version}) needs to be upgraded to v1.7.0.\n\n"
                f"Please run the migration utility first:\n\n"
                f"1. Close this application\n"
                f"2. Run: python migrate_to_v1_7_0.py\n"
                f"3. Restart the application\n\n"
                f"The migration utility will:\n"
                f"• Backup your current database\n"
                f"• Upgrade to v1.7.0 multi-race monitoring\n"
                f"• Preserve all existing data\n\n"
                f"Migration utility: migrate_to_v1_7_0.py"
            )
            ...
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to show migration message: {e}")
            print(f"❌ Database migration required!")
            print(f"   Current version: v{current_version}")
            print(f"   Required version: v1.7.0")
            print(f"   Please run: python migrate_to_v1_7_0.py")
            import sys
            sys.exit(1)
    ...
    def create_database_indexes(self):
        ...
        try:
            indexes = [
                ...
                "CREATE INDEX IF NOT EXISTS idx_sessions_completed ON Sessions(IsCompleted)",
                "CREATE INDEX IF NOT EXISTS idx_sessions_created ON Sessions(CreatedAt)",
                "CREATE INDEX IF NOT EXISTS idx_session_participants ON SessionParticipants(SessionID)",
                "CREATE INDEX IF NOT EXISTS idx_participants_race ON Participants(RaceID)",
                "CREATE INDEX IF NOT EXISTS idx_laps_race ON Laps(RaceID)",
                "CREATE INDEX IF NOT EXISTS idx_laps_name ON Laps(mName)"
            ]
            
            for index_sql in indexes:
                self.cursor.execute(index_sql)
                
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Created performance indexes")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to create indexes: {e}")
    ...
    def cleanup_old_sessions(self, days_to_keep=7):
        """Daily cleanup of old completed sessions"""
        try:
            ...
            if deleted_count > 0:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Cleaned up {deleted_count} old sessions older than {days_to_keep} days")
                print(f"DB Line {inspect.currentframe().f_lineno} Cleaned up {deleted_count} old sessions")
                
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to cleanup old sessions: {e}")
    ...
    def mark_session_complete(self, session_id):
        ...
        try:
            ...
            self.cursor.execute('''
                UPDATE Races 
                SET IsCompleted = 1, CompletedAt = CURRENT_TIMESTAMP 
                WHERE SessionID = ? AND IsCompleted = 0
            ''', (session_id,))
            
            self.conn.commit()
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Session {session_id} marked as completed in database")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to mark session {session_id} as completed: {e}")
    ...
    def write_race(self, data, session_id, ip_list=None):
        try:
            ...
            logging.info(f"DB Line {inspect.currentframe().f_lineno} track_info to write:{track_info}, IPs: {ip_list}")
            print(f"DB Line {inspect.currentframe().f_lineno} track_info to write:{track_info}, IPs: {ip_list}")
            self.cursor.execute('''
            ...
            # Emit callback for race ID update
            if ip_list:
                # Multi-race mode: create callback with IP context
                callback = lambda race_id: self.monitor_thread.set_race_id_with_ips(race_id, ip_list)
                self.write_race_signal.emit(callback)
            else:
                # Legacy single-race mode
                self.write_race_signal.emit(self.monitor_thread.set_race_id)
    ...
    def get_active_sessions(self, callback=None):
        """Get all sessions that exist in the database"""
        try:
            conn = sqlite3.connect('RaceDB.db')
            cursor = conn.cursor()
            
            cursor.execute('SELECT DISTINCT SessionID FROM Races ORDER BY SessionID')
            sessions = [row[0] for row in cursor.fetchall()]
            conn.close()
            
            print(f"DB Line {inspect.currentframe().f_lineno} Found {len(sessions)} active sessions: {sessions}")
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Found {len(sessions)} active sessions: {sessions}")
            ...
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Error getting active sessions: {e}")
            print(f"DB Line {inspect.currentframe().f_lineno} Error getting active sessions: {e}")
            if callback:
                callback([])
            return []

    def get_session_participants(self, session_id, callback=None):
        """Get all unique participants from a session"""
        try:
            conn = sqlite3.connect('RaceDB.db')
            cursor = conn.cursor()
            
            # Get all distinct driver names from all races in this session
            cursor.execute('''
                SELECT DISTINCT DriverName 
                FROM LapData 
                INNER JOIN Races ON LapData.Race_ID = Races.Race_ID 
                WHERE Races.SessionID = ? AND DriverName IS NOT NULL AND DriverName != ""
                ORDER BY DriverName
            ''', (session_id,))
            ...
            print(f"DB Line {inspect.currentframe().f_lineno} Session {session_id} has {len(participants)} participants: {participants}")
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Session {session_id} participants: {participants}")
            ...
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Error getting session participants: {e}")
            print(f"DB Line {inspect.currentframe().f_lineno} Error getting session participants: {e}")
            if callback:
                callback([])
            return []

    def check_session_has_active_race(self, session_id, callback=None):
        """Check if a session has races currently in progress (based on active_races)"""
        try:
            # Check if any active race belongs to this session
            has_active_race = False
            active_sessions = []
            
            # Check self.active_races from MultiThreadMonitor if available
            if hasattr(self, 'mt_instance') and self.mt_instance and hasattr(self.mt_instance, 'active_races'):
                for ip, race_info in self.mt_instance.active_races.items():
                    race_session_id = race_info.get('session_id', 0)
                    if race_session_id == session_id:
                        has_active_race = True
                    if race_session_id not in active_sessions:
                        active_sessions.append(race_session_id)
            
            print(f"DB Line {inspect.currentframe().f_lineno} Session {session_id} has active race: {has_active_race}, Active sessions: {active_sessions}")
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Session {session_id} has active race: {has_active_race}, Active sessions: {active_sessions}")
            
            if callback:
                callback(has_active_race)
            return has_active_race
                
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Error checking session active race: {e}")
            print(f"DB Line {inspect.currentframe().f_lineno} Error checking session active race: {e}")
            if callback:
                callback(False)
            return False
    ...
    def update_ip_name_flag(self, ip_name_flag):
        ...
        human_drivers = []
        for ip, ip_data in ip_name_flag.items():
            # Handle both old format [name, in_race] and new format [name, in_race, race_group_letter]
            if len(ip_data) >= 2:
                name, in_race = ip_data[:2]
                if in_race == 1:
                    human_drivers.append(name)
...
class MonitorThread(QThread):
    ...
    # Multi-race monitoring signals
    active_races_updated = pyqtSignal(dict)  # Signal to update GUI with active races {ip: {'session_id': int, 'track': str, 'drivers': list}}
    primary_race_changed = pyqtSignal(str)  # Signal when primary race IP changes
    ...
    def __init__(self, gui_main_app, tab_widget, task_queue, config_manager, *args, **kwargs):
        ...
        # Initialize available IPs from config for radio button functionality
        self.available_ips = [ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '').strip() for i in range(1, 25) if ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '')]

        # Multi-race monitoring state
        self.active_races = {}  # Format: {ip: {'session_id': int, 'race_id': int, 'data': dict, 'lap_times': dict, 'driver_totals': dict, 'race_in_session': bool}}
        self.primary_race_ip = None  # IP of the race currently displayed in GUI
        self.race_ip_mapping = {}  # Track which IPs belong to which race_id: {race_id: [ip1, ip2, ...]}
        self.ip_name_flag = {}  # Store ip_name_flag for human driver detection
        
        # Participant update tracking
        self.participants_need_update = False
    ...
    async def run_main_loop(self):
        ...
        asyncio.create_task(self.periodic_fetch_enriched_data())  # Start multi-IP monitoring
        ...
        while self.running:
            #try:
                ...
                await self.detect_race_data_all_ips()    
                # Wait for multi-race data to be available
                await asyncio.sleep(5)
                 # Detect active races from multi-IP data
                # active_races = self.detect_active_races(self.multi_data)
                if not self.running: break
                
                if index % 50 == 0:
                    print(f"MT Line {inspect.currentframe().f_lineno} Multi-race monitoring... Loop {index}, Active races: {len(self.active_races)}")
                    if self.active_races:
                        print(f"MT Line {inspect.currentframe().f_lineno} Active race IPs: {list(self.active_races.keys())}, Primary: {self.primary_race_ip}")
                        
                index += 1
                
                # Check if we have valid multi-race data
                if not hasattr(self, 'multi_data') or not self.multi_data:
                    # In testmode, fetch multi-race data from API files
                    testmode_enabled = self.config_manager.read_testmode_status()
                    if testmode_enabled:
                        await self._fetch_testmode_multi_data()
                        # Wait for signal to be processed
                        await asyncio.sleep(0.5)
                        # Check again after fetch
                        if hasattr(self, 'multi_data') and self.multi_data:
                            print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Multi-data loaded successfully")
                        else:
                            print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Multi-data not yet available")
                ...
                if not active_races and self.first_time_run and not self.race_may_not_be_finished:
                    self.first_time_run = False
                    race_not_finished_message_shown = False
                    logging.info(f"MT Line {inspect.currentframe().f_lineno} No active races detected, waiting for race start.")
                    print(f"MT Line {inspect.currentframe().f_lineno} Waiting for races to start.....")
                    index = 0
                    continue
                else:
                    if self.race_may_not_be_finished and active_races:
                        if not race_not_finished_message_shown:
                            print(f"MT Line {inspect.currentframe().f_lineno} Races still running....")
                            race_not_finished_message_shown = True
                        continue
                    if self.race_may_not_be_finished:
                        print(f"MT Line {inspect.currentframe().f_lineno} All races finished!")
                        self.race_may_not_be_finished = False
                        continue
                ...
                if active_races:
                    ...
                    # Manage concurrent races (start new, continue existing, end finished)
                    self.manage_concurrent_races(active_races)
                    
                    # Update participants IPs only when needed (new races or participants changed)
                    if getattr(self, 'participants_need_update', False):
                        print(f"MT Line {inspect.currentframe().f_lineno} Participants changed - updating IP name flag")
                        participants = getattr(self, 'pending_participants', [])
                        if participants:
                        ...
                        if hasattr(self, 'pending_participants'):
                            delattr(self, 'pending_participants')
                    
                    # Handle primary race specific logic if primary race exists
                    if self.primary_race_ip and self.primary_race_ip in self.active_races:
                        primary_data = self.active_races[self.primary_race_ip]['data']
                        
                        # Handle session types for primary race (only emit if changed)
                        session_state = primary_data['gameStates']['mSessionState']
                        current_session_type = None
                        
                        if session_state == 3:
                            current_session_type = "Qualifying"
                        elif session_state == 1:
                            current_session_type = "Practice"
                        else:
                            # Determine if time trial
                            if primary_data['eventInformation']['mLapsInEvent'] == 0:
                                current_session_type = "TimeTrail"
                            else:
                                current_session_type = "Race"
                        
                        # Only emit if session type changed
                        if not hasattr(self, 'last_session_type') or self.last_session_type != current_session_type:
                            print(f"MT Line {inspect.currentframe().f_lineno} Primary race {current_session_type}")
                            self.session_type_updated.emit(current_session_type)
    ...
    async def get_participants_ips(self, participants):
        """
        Queries all configured sim IPs and builds a mapping of IP -> [name, in_race, race_group_letter].
        In testmode, creates simulated mappings based on active multi-race data.
        Uses mViewedParticipantIndex to determine the driver for each IP, mapping directly to button labels.
        """
        ip_name_flag = {}  # Format: {"192.168.3.201": ["Fred", 1, 'A'], "192.168.3.202": ["S-2", 0, 'A'], ...}
        
        # Check if we're in testmode
        testmode_enabled = self.config_manager.read_testmode_status()
        participant_names = {p['mName'] for p in participants if p.get('mName')}
        
        if testmode_enabled:
            print(f"MT Line {inspect.currentframe().f_lineno} Testmode: Creating simulated radio button mappings")
            
            # Get race group information
            race_groups = {}
            if hasattr(self, 'active_races'):
                for ip, race_info in self.active_races.items():
                    race_group_letter = race_info.get('race_group_letter', 'A')
                    race_groups[ip] = race_group_letter
                    print(f"MT Line {inspect.currentframe().f_lineno} Testmode: Race group mapping - {ip} -> Group {race_group_letter}")
            
            # Get all active race IPs
            active_race_ips = []
            if hasattr(self, 'multi_data'):
                active_race_ips = list(self.multi_data.keys())
                print(f"MT Line {inspect.currentframe().f_lineno} Testmode: Active race IPs found: {active_race_ips}")
            
            if not active_race_ips:
                active_race_ips = ['192.168.3.201', '192.168.3.202', '192.168.3.203', '192.168.3.204']
                print(f"MT Line {inspect.currentframe().f_lineno} Testmode: Using default test IPs: {active_race_ips}")
            
            # Create mappings for available IPs
            for index, ip in enumerate(self.available_ips):
                sim_number = index + 1
                driver_name = f"S-{sim_number}"
                in_race = 0
                race_group_letter = 'A'
                
                if ip in active_race_ips and hasattr(self, 'multi_data') and ip in self.multi_data:
                    data = self.multi_data[ip]
                    participants_data = data.get('participants', {})
                    viewed_index = participants_data.get('mViewedParticipantIndex', -1)
                    infos = participants_data.get('mParticipantInfo', [])
                    
                    game_state = data.get('gameStates', {}).get('mGameState', 0)
                    race_state = data.get('gameStates', {}).get('mRaceState', 0)
                    
                    if game_state == 2 and race_state == 2:
                        in_race = 1
                        
                        # Use mViewedParticipantIndex to get the driver name
                        if 0 <= viewed_index < len(infos):
                            fetched_name = infos[viewed_index].get('mName')
                            if fetched_name and not fetched_name.endswith('(AI)'):
                                driver_name = fetched_name
                                print(f"MT Line {inspect.currentframe().f_lineno} Testmode: {ip} -> {driver_name} (viewed driver at index {viewed_index})")
                        else:
                            print(f"MT Line {inspect.currentframe().f_lineno} Testmode: {ip} -> Invalid viewed index {viewed_index}, using fallback {driver_name}")
                    
                    race_group_letter = race_groups.get(ip, 'A')
                else:
                    race_group_letter = race_groups.get(ip, chr(65 + (index // 4)))
                
                # Format caption
                if in_race == 1:
                    formatted_name = f"{driver_name}({sim_number})[{race_group_letter}]"
                else:
                    formatted_name = f"{driver_name}({sim_number})"
                
                ip_name_flag[ip] = [formatted_name, in_race, race_group_letter]
                
        else:
            # Unified live mode logic: build race group mapping from active_races
            ...
            # Build IP to race group letter mapping
            race_groups = {}
            if hasattr(self, 'active_races'):
                for ip, race_info in self.active_races.items():
                    race_group_letter = race_info.get('race_group_letter', 'A')
            ...
            # Build set of all active race IPs
            active_race_ips = set(self.active_races.keys()) if hasattr(self, 'active_races') else set()
            ...
            for sim_index, (ip, data) in enumerate(results):
                ...
                if data is not None:
                    ...
                    if 0 <= viewed_index < len(infos):
                        ...
                        if fetched_name:
                            driver_name = fetched_name
                            # Mark as in_race if IP is in any active race group
                            if ip in active_race_ips:
                                in_race = 1
                                print(f"MT Line {inspect.currentframe().f_lineno} Live mode: {ip} -> {driver_name} (in race, group {race_group_letter})")
                            else:
                                print(f"MT Line {inspect.currentframe().f_lineno} Live mode: {ip} -> {driver_name} (not in race)")
                    else:
                        print(f"MT Line {inspect.currentframe().f_lineno} Live mode: {ip} -> {driver_name} (no viewed participant)")
                else:
                    print(f"MT Line {inspect.currentframe().f_lineno} Live mode: {ip} -> {driver_name} (no connection)")
        ...
        print(f"MT Line {inspect.currentframe().f_lineno} Signal sent to Radio Buttons: {len(ip_name_flag)} IPs")
    ...
    async def detect_race_data_all_ips(self):
        if hasattr(self, "ip_detection_cycle_reset"):
            self.ip_detection_cycle_reset += 1
        else:
            self.ip_detection_cycle_reset = 1
        ...
        After 3 failed attempts, sets in_race = 0 for that IP.
        """
        self.failed_attempts = getattr(self, "_fetch_failures", {})
        if self.ip_detection_cycle_reset > 10:
            self.failed_attempts = {}
            self.ip_detection_cycle_reset = 0
        async def fetch(ip):
            if ip not in self.failed_attempts or self.failed_attempts.get(ip, 0) < 3:
                url = f"http://{ip}:8180/crest2/v1/api"
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url, timeout=0.5) as response:
                            #print(f"MT Line {inspect.currentframe().f_lineno} response data {response}")
                            if response.status == 200:
                                return ip, await response.json()
                ...
                if self.failed_attempts[ip] >= 3:
                    print(f"MT Line {inspect.currentframe().f_lineno} Marking {ip} as offline after 3 failed attempts")
                return ip, None
            return ip, None

        # Use all configured IPs from config.ini
        sim_ips = [ConfigManager.get_config_value(f"sim-{i}", "simsettings", "").strip()
            for i in range(1, 25)
            if ConfigManager.get_config_value(f"sim-{i}", "simsettings", "").strip()]
        tasks = [fetch(ip) for ip in sim_ips]
        results = await asyncio.gather(*tasks)
        ...
        print(f"MT Line {inspect.currentframe().f_lineno} Fetched data from {len(dataset)} IPs")
        #self.multi_data_signal.emit(dataset)
        self.multi_data = dataset  # Store for later use
    ...
    def set_race_id(self, latest_race_id):
        ...
        if latest_race_id is None:
            ...
            # Update race_id for primary race
            if self.primary_race_ip and self.primary_race_ip in self.active_races:
                self.active_races[self.primary_race_ip]['race_id'] = latest_race_id
    ...
    def set_race_id_with_ips(self, latest_race_id, ip_list):
        """
        Set race ID and establish IP mapping for multi-race mode.
        latest_race_id: The race ID from database
        ip_list: List of IPs participating in this race
        """
        logging.info(f"MT Line {inspect.currentframe().f_lineno} Def set_race_id_with_ips entered - Race ID: {latest_race_id}, IPs: {ip_list}")
        
        if latest_race_id is None:
            print(f"MT Line {inspect.currentframe().f_lineno} Race ID is None for IPs {ip_list}!")
            logging.error(f"MT Line {inspect.currentframe().f_lineno} Race ID is None for IPs {ip_list}")
            return
            
        # Establish race_id to IPs mapping
        self.add_ip_to_race(latest_race_id, ip_list)
        
        # Update race_id in active_races for all IPs in this race
        for ip in ip_list:
            if ip in self.active_races:
                self.active_races[ip]['race_id'] = latest_race_id
                print(f"MT Line {inspect.currentframe().f_lineno} Race ID {latest_race_id} assigned to IP {ip}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} Race ID {latest_race_id} assigned to IP {ip}")
        
        # If primary race is in this race, update main race_id
        if self.primary_race_ip and self.primary_race_ip in ip_list:
            self.race_id = latest_race_id
            print(f"MT Line {inspect.currentframe().f_lineno} Primary race updated to Race ID: {latest_race_id}")
            logging.info(f"MT Line {inspect.currentframe().f_lineno} Primary race updated to Race ID: {latest_race_id}")
    ...
    def get_race_id_for_ip(self, ip):
        """
        Find which race_id an IP belongs to.
        Returns race_id if found, None if IP not in any race.
        """
        for race_id, ip_list in self.race_ip_mapping.items():
            if ip in ip_list:
                return race_id
        return None

    def add_ip_to_race(self, race_id, ip_list):
        """
        Add IPs to a race_id mapping.
        race_id: The race ID from database
        ip_list: List of IPs participating in this race
        """
        self.race_ip_mapping[race_id] = ip_list.copy()
        print(f"MT Line {inspect.currentframe().f_lineno} Race {race_id} mapped to IPs: {ip_list}")
        logging.info(f"MT Line {inspect.currentframe().f_lineno} Race {race_id} mapped to IPs: {ip_list}")

    def remove_race_mapping(self, race_id):
        """
        Remove race mapping when race is finished.
        """
        if race_id in self.race_ip_mapping:
            removed_ips = self.race_ip_mapping.pop(race_id)
            print(f"MT Line {inspect.currentframe().f_lineno} Removed mapping for race {race_id}, IPs: {removed_ips}")
            logging.info(f"MT Line {inspect.currentframe().f_lineno} Removed race mapping: {race_id}")
    ...
    def calculate_session_id_for_participants(self, current_participants, ip_name_flag, previous_participants=None, session_has_races=False):
        """
        Enhanced session assignment logic based on active sessions and race-in-progress checking.
        
        Logic from user specifications:
        2.1 Get all active sessions from database
        2.2 If one active session, use this
        2.3 Two active sessions found, check race-in-progress:
            - If session 1 has active race, use session 2
            - If session 2 has active race, use session 1  
            - If both have active race, use highest session + 1
            - If neither have active race, compare participants >50% overlap
        2.4 Three active sessions found, check race-in-progress:
            - If session 1&2 have active race, use session 3
            - If session 1&3 have active race, use session 2
            - If session 2&3 have active race, use session 1
            - If all have active race, use highest session + 1
            - If none have active race, compare participants >50% overlap
        2.5 Four active sessions found, find session without active race or create new one
        """
        try:
            ...
            print(f"MT Line {inspect.currentframe().f_lineno} === Enhanced Session Assignment Logic ===")
            print(f"MT Line {inspect.currentframe().f_lineno} Human drivers detected: {human_drivers} (count: {len(human_drivers)})")
            logging.info(f"MT Line {inspect.currentframe().f_lineno} Starting enhanced session assignment for {len(human_drivers)} human drivers: {human_drivers}")
            
            # Fallback if no human drivers detected from ip_name_flag
            if not human_drivers:
                print(f"MT Line {inspect.currentframe().f_lineno} No human drivers detected from ip_name_flag, trying participant list")
                fallback_names = [p.get('mName', '') for p in current_participants if p.get('mName') and not p.get('mName', '').endswith('(AI)')]
                human_drivers = fallback_names
                print(f"MT Line {inspect.currentframe().f_lineno} Fallback detected {len(human_drivers)} human drivers: {human_drivers}")
            
            # Final fallback if still no human drivers
            if not human_drivers:
            ...
            # Storage for async database operations
            active_sessions = []
            sessions_with_races = {}
            sessions_with_active_races = {}
            sessions_participants = {}
            
            # Async callback handlers
            def store_active_sessions(sessions):
                nonlocal active_sessions
                active_sessions = sessions
                print(f"MT Line {inspect.currentframe().f_lineno} Found {len(sessions)} active sessions: {sessions}")
                
            def store_session_has_races(session_id, has_races):
                nonlocal sessions_with_races
                sessions_with_races[session_id] = has_races
                print(f"MT Line {inspect.currentframe().f_lineno} Session {session_id} has races: {has_races}")
                
            def store_session_has_active_race(session_id, has_active_race):
                nonlocal sessions_with_active_races
                sessions_with_active_races[session_id] = has_active_race
                print(f"MT Line {inspect.currentframe().f_lineno} Session {session_id} has active race: {has_active_race}")
                
            def store_session_participants(session_id, participants_list):
                nonlocal sessions_participants
                sessions_participants[session_id] = participants_list
                print(f"MT Line {inspect.currentframe().f_lineno} Session {session_id} participants ({len(participants_list)}): {participants_list}")
            
            # Step 1: Get all active sessions
            self.task_queue.put(('get_active_sessions', (store_active_sessions,)))
            
            # Wait for active sessions to be loaded
            wait_count = 0
            while not active_sessions and wait_count < 50:  # 5 second timeout
                time.sleep(0.1)
                wait_count += 1
                
            if not active_sessions:
                # No sessions exist yet - create session 1 for single driver, session 2 for multi-driver
                calculated_session_id = 2 if len(human_drivers) > 1 else 1
                print(f"MT Line {inspect.currentframe().f_lineno} No active sessions found - creating new session {calculated_session_id}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} No active sessions found - new session {calculated_session_id} for {len(human_drivers)} drivers")
                self.auto_session_assigned_signal.emit(calculated_session_id)
                return calculated_session_id
            
            # Step 2: Check each session for races and active races
            for session_id in active_sessions:
                self.task_queue.put(('check_session_has_races', (session_id, lambda has_races, sid=session_id: store_session_has_races(sid, has_races))))
                self.task_queue.put(('check_session_has_active_race', (session_id, lambda has_active, sid=session_id: store_session_has_active_race(sid, has_active))))
                self.task_queue.put(('get_session_participants', (session_id, lambda parts, sid=session_id: store_session_participants(sid, parts))))
            
            # Wait for all session data to be loaded
            wait_count = 0
            while (len(sessions_with_races) < len(active_sessions) or 
                   len(sessions_with_active_races) < len(active_sessions) or 
                   len(sessions_participants) < len(active_sessions)) and wait_count < 100:  # 10 second timeout
                time.sleep(0.1)
                wait_count += 1
            
            print(f"MT Line {inspect.currentframe().f_lineno} Session analysis complete:")
            print(f"MT Line {inspect.currentframe().f_lineno} - Sessions with races: {sessions_with_races}")
            print(f"MT Line {inspect.currentframe().f_lineno} - Sessions with active races: {sessions_with_active_races}")
            print(f"MT Line {inspect.currentframe().f_lineno} - Sessions participants: {sessions_participants}")
            
            # Step 3: Apply enhanced session assignment logic
            num_sessions = len(active_sessions)
            
            if num_sessions == 1:
                # 2.2 If one active session, use this
                calculated_session_id = active_sessions[0]
                print(f"MT Line {inspect.currentframe().f_lineno} One active session found - using session {calculated_session_id}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} One active session {calculated_session_id} - direct assignment")
                
            elif num_sessions == 2:
                # 2.3 Two active sessions found
                session1, session2 = sorted(active_sessions)
                has_active_1 = sessions_with_active_races.get(session1, False)
                has_active_2 = sessions_with_active_races.get(session2, False)
                
                print(f"MT Line {inspect.currentframe().f_lineno} Two sessions: {session1} (active: {has_active_1}), {session2} (active: {has_active_2})")
                
                if has_active_1 and not has_active_2:
                    calculated_session_id = session2
                    print(f"MT Line {inspect.currentframe().f_lineno} Session {session1} has active race - using session {session2}")
                elif has_active_2 and not has_active_1:
                    calculated_session_id = session1
                    print(f"MT Line {inspect.currentframe().f_lineno} Session {session2} has active race - using session {session1}")
                elif has_active_1 and has_active_2:
                    calculated_session_id = max(session1, session2) + 1
                    print(f"MT Line {inspect.currentframe().f_lineno} Both sessions have active races - creating new session {calculated_session_id}")
                else:
                    # Neither has active race - compare participants
                    calculated_session_id = self.find_session_by_participant_overlap(human_drivers, sessions_participants, active_sessions)
                    print(f"MT Line {inspect.currentframe().f_lineno} Neither session has active race - participant matching assigned session {calculated_session_id}")
            
            elif num_sessions == 3:
                # 2.4 Three active sessions found
                session1, session2, session3 = sorted(active_sessions)
                has_active_1 = sessions_with_active_races.get(session1, False)
                has_active_2 = sessions_with_active_races.get(session2, False)
                has_active_3 = sessions_with_active_races.get(session3, False)
                
                print(f"MT Line {inspect.currentframe().f_lineno} Three sessions: {session1} (active: {has_active_1}), {session2} (active: {has_active_2}), {session3} (active: {has_active_3})")
                
                if has_active_1 and has_active_2 and not has_active_3:
                    calculated_session_id = session3
                    print(f"MT Line {inspect.currentframe().f_lineno} Sessions {session1}&{session2} have active races - using session {session3}")
                elif has_active_1 and has_active_3 and not has_active_2:
                    calculated_session_id = session2
                    print(f"MT Line {inspect.currentframe().f_lineno} Sessions {session1}&{session3} have active races - using session {session2}")
                elif has_active_2 and has_active_3 and not has_active_1:
                    calculated_session_id = session1
                    print(f"MT Line {inspect.currentframe().f_lineno} Sessions {session2}&{session3} have active races - using session {session1}")
                elif has_active_1 and has_active_2 and has_active_3:
                    calculated_session_id = max(session1, session2, session3) + 1
                    print(f"MT Line {inspect.currentframe().f_lineno} All three sessions have active races - creating new session {calculated_session_id}")
                else:
                    # Find best match by participant overlap
                    calculated_session_id = self.find_session_by_participant_overlap(human_drivers, sessions_participants, active_sessions)
                    print(f"MT Line {inspect.currentframe().f_lineno} Mixed or no active races - participant matching assigned session {calculated_session_id}")
            
            elif num_sessions >= 4:
                # 2.5 Four or more active sessions found
                sessions_without_active = [s for s in active_sessions if not sessions_with_active_races.get(s, False)]
                
                if sessions_without_active:
                    # Use participant matching among sessions without active races
                    available_sessions = {s: sessions_participants.get(s, []) for s in sessions_without_active}
                    calculated_session_id = self.find_session_by_participant_overlap(human_drivers, available_sessions, sessions_without_active)
                    print(f"MT Line {inspect.currentframe().f_lineno} {len(sessions_without_active)} sessions without active races - assigned session {calculated_session_id}")
                else:
                    # All sessions have active races - create new one
                    calculated_session_id = max(active_sessions) + 1
                    print(f"MT Line {inspect.currentframe().f_lineno} All {num_sessions} sessions have active races - creating new session {calculated_session_id}")
            
            # Ensure minimum session ID for multi-driver races
            if len(human_drivers) > 1 and calculated_session_id < 2:
                calculated_session_id = 2
                print(f"MT Line {inspect.currentframe().f_lineno} Multi-driver race - adjusting to minimum session {calculated_session_id}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} Multi-driver race adjusted to minimum session ID = 2")
            
            print(f"MT Line {inspect.currentframe().f_lineno} === Session Assignment Complete: {calculated_session_id} ===")
            logging.info(f"MT Line {inspect.currentframe().f_lineno} Enhanced session assignment complete - assigned session {calculated_session_id}")
            ...
            logging.error(f"MT Line {inspect.currentframe().f_lineno} Error in enhanced session calculation: {e}")
            print(f"MT Line {inspect.currentframe().f_lineno} Error in enhanced session calculation: {e}")
            # Fallback to current MT session or new session based on driver count
            fallback_session = 2 if len(human_drivers) > 1 else 1
    ...
    def find_session_by_participant_overlap(self, current_drivers, sessions_participants, available_sessions):
        """
        Find the best session match based on >50% participant overlap.
        If no match found, return the lowest available session or create new one.
        """
        try:
            best_session = None
            best_overlap_percentage = 0
            
            print(f"MT Line {inspect.currentframe().f_lineno} Comparing current drivers {current_drivers} with session participants")
            
            for session_id in available_sessions:
                session_participants = sessions_participants.get(session_id, [])
                ...
                for name in current_drivers:
                    ...
                    if clean_name:
                        current_normalized.add(clean_name)
                
                session_normalized = set()
                for name in session_participants:
                    clean_name = name.split(' (')[0].strip().lower() if name else ""
                    if clean_name:
                        session_normalized.add(clean_name)
                
                # Calculate overlap percentage
                if current_normalized and session_normalized:
                    intersection = current_normalized.intersection(session_normalized)
                    overlap_percentage = len(intersection) / len(current_normalized) * 100
                    
                    print(f"MT Line {inspect.currentframe().f_lineno} Session {session_id}: {overlap_percentage:.1f}% overlap ({len(intersection)}/{len(current_normalized)})")
                    
                    if overlap_percentage > 50 and overlap_percentage > best_overlap_percentage:
                        best_session = session_id
                        best_overlap_percentage = overlap_percentage
            
            if best_session:
                print(f"MT Line {inspect.currentframe().f_lineno} Best match: Session {best_session} with {best_overlap_percentage:.1f}% overlap")
                return best_session
            else:
                # No good match - use lowest available session or create new one
                if available_sessions:
                    lowest_session = min(available_sessions)
                    print(f"MT Line {inspect.currentframe().f_lineno} No >50% overlap found - using lowest available session {lowest_session}")
                    return lowest_session
                else:
                    # This shouldn't happen, but create new session as fallback
                    new_session = max(sessions_participants.keys()) + 1 if sessions_participants else 1
                    print(f"MT Line {inspect.currentframe().f_lineno} No available sessions - creating new session {new_session}")
                    return new_session
                    
        except Exception as e:
            logging.error(f"MT Line {inspect.currentframe().f_lineno} Error finding session by participant overlap: {e}")
            print(f"MT Line {inspect.currentframe().f_lineno} Error finding session by participant overlap: {e}")
            return available_sessions[0] if available_sessions else 1
    ...
    def detect_active_races(self, multi_data):
        """
        Analyze multi_data to detect which IPs have active races and group IPs that share participants.
        Returns dict of grouped races with their participants and IPs.
        
        Logic:
        1. Find all IPs with active races
        2. For each race, create participant signature (name + index pairs)
        3. Group IPs with matching participant signatures
        4. Return grouped races with all IPs that belong to each race
        """
        print(f"MT Line {inspect.currentframe().f_lineno} Detecting active races from multi_data with {len(multi_data)} IPs")
        active_ips = {}
        grouped_races = {}
        
        # Step 1: Find all IPs with active races
        for ip, data in multi_data.items():
            if not data:
                continue
                
            # Check if race is active on this IP
            game_state = data.get('gameStates', {}).get('mGameState', 0)
            race_state = data.get('gameStates', {}).get('mRaceState', 0)
            
            # Game state 2 = In Race, Race state 2 = Racing
            if game_state == 2 and race_state == 2:
                participants = data.get('participants', {}).get('mParticipantInfo', [])
                human_participants = [p for p in participants if p.get('mName') and not p.get('mName', '').endswith('(AI)')]
                
                if human_participants:  # Only track races with human drivers
                    # Create participant signature: list of (name, race_position) pairs
                    participant_signature = []
                    human_participants = [p for p in participants if p.get('mName') and not p.get('mName', '').endswith('(AI)')]
                    sorted_names = sorted(p['mName'] for p in human_participants)
                    for i, name in enumerate(sorted_names, 1):
                        participant_signature.append((name, i))
                    participant_signature_tuple = tuple(participant_signature)
                    
                    active_ips[ip] = {
                        'data': data,
                        'participants': participants,
                        'human_participants': human_participants,
                        'participant_signature': participant_signature_tuple,
                        'track_location': data['eventInformation']['mTranslatedTrackVariation'] + ' - ' + data['eventInformation']['mTranslatedTrackLocation']
                    }
        
        # Step 2: Group IPs by matching participant signatures
        signature_to_ips = {}
        for ip, race_info in active_ips.items():
            signature = race_info['participant_signature']
            if signature not in signature_to_ips:
                signature_to_ips[signature] = []
            signature_to_ips[signature].append(ip)
        
        # Step 3: Create separate race groups for different track/participant combinations
        race_group_id = 0
        for signature, ip_list in signature_to_ips.items():
            # Use first IP as representative for race data
            first_ip = ip_list[0]
            race_data = active_ips[first_ip]
            
            # If we have a large group (>4 IPs), split into smaller race groups of 4 IPs each
            if len(ip_list) > 4:
                for i in range(0, len(ip_list), 4):
                    sub_ip_list = ip_list[i:i+4]
                    grouped_races[f"race_group_{race_group_id}"] = {
                        'ip_list': sub_ip_list,
                        'participants': race_data['participants'],
                        'human_participants': race_data['human_participants'],
                        'data': race_data['data'],
                        'track_location': race_data['track_location'],
                        'participant_signature': signature,
                        'race_group_letter': chr(65 + race_group_id)  # A, B, C, D...
                    }
                    
                    print(f"MT Line {inspect.currentframe().f_lineno} Detected race group {chr(65 + race_group_id)} with IPs: {sub_ip_list}")
                    print(f"MT Line {inspect.currentframe().f_lineno} Track: {race_data['track_location']}, Participants: {[name for name, pos in signature]}")
                    logging.info(f"MT Line {inspect.currentframe().f_lineno} Race group {chr(65 + race_group_id)} - IPs: {sub_ip_list}, Track: {race_data['track_location']}, Participants: {len(signature)}")
                    
                    race_group_id += 1
            else:
                # Create single race group entry
                grouped_races[f"race_group_{race_group_id}"] = {
                    'ip_list': ip_list,
                    'participants': race_data['participants'],
                    'human_participants': race_data['human_participants'],
                ...
                print(f"MT Line {inspect.currentframe().f_lineno} Detected race group {chr(65 + race_group_id)} with IPs: {ip_list}")
                print(f"MT Line {inspect.currentframe().f_lineno} Track: {race_data['track_location']}, Participants: {[name for name, pos in signature]}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} Race group {chr(65 + race_group_id)} - IPs: {ip_list}, Track: {race_data['track_location']}, Participants: {len(signature)}")
                
                race_group_id += 1
        #print (f"MT Line {inspect.currentframe().f_lineno} Detected {grouped_races} race groups")
        return grouped_races

    def manage_concurrent_races(self, grouped_races):
        """
        Manage concurrent races: start new race groups, continue existing ones, end finished ones.
        Updates self.active_races with current state.
        
        Args:
            grouped_races: Dict of race groups from detect_active_races()
                          Format: {'race_group_0': {'ip_list': [ip1, ip2], 'participants': [...], ...}}
        """
        # Get all currently active IPs from grouped races
        current_ips = set()
        for race_group_id, race_info in grouped_races.items():
            current_ips.update(race_info['ip_list'])
        
        # Get currently tracked IPs
        tracked_ips = set(self.active_races.keys())
        
        # Start new race groups
        new_race_ips = current_ips - tracked_ips
        participants_changed = False
        
        if new_race_ips:
            # Find which race groups contain new IPs and start them
            for race_group_id, race_info in grouped_races.items():
                race_ip_list = race_info['ip_list']
                if any(ip in new_race_ips for ip in race_ip_list):
                    # This race group contains new IPs, start the entire race group
                    self.start_race_group(race_ip_list, race_info)
                    participants_changed = True
        
        # Continue existing races
        continuing_ips = current_ips & tracked_ips
        for race_group_id, race_info in grouped_races.items():
            race_ip_list = race_info['ip_list']
            for ip in race_ip_list:
                if ip in continuing_ips:
                    self.continue_race_for_ip(ip, race_info)
        
        # End finished races
        finished_ips = tracked_ips - current_ips
        for ip in finished_ips:
            self.end_race_for_ip(ip)
            if finished_ips:
                participants_changed = True
        
        print(f"MT Line {inspect.currentframe().f_lineno} Active races: {len(self.active_races)} - IPs: {list(self.active_races.keys())}")
        
        # Set flag for main loop to update participants if needed
        self.participants_need_update = participants_changed and bool(grouped_races)
        if self.participants_need_update:
            # Store participants for main loop to process
            first_race_group = next(iter(grouped_races.values()))
            self.pending_participants = first_race_group['participants']
        
        # Emit updated active races to GUI
        self.emit_active_races_update()

    def start_race_group(self, ip_list, race_info):
        """
        Start a new race group with multiple IPs participating in the same race.
        
        Args:
            ip_list: List of IPs in this race
            race_info: Race information from detect_active_races
        """
        participants = race_info['participants']
        data = race_info['data']
        
        print(f"MT Line {inspect.currentframe().f_lineno} Starting new race group with IPs: {ip_list}")
        logging.info(f"MT Line {inspect.currentframe().f_lineno} New race group started - IPs: {ip_list}, Track: {race_info['track_location']}")
        
        # Determine session ID for this race (use first IP for human driver detection)
        first_ip = ip_list[0]
        human_drivers = [name for ip_addr, (name, in_race) in self.ip_name_flag.items() 
                        if ip_addr == first_ip and in_race == 1]
        
        if not human_drivers:
            # Fallback to participant data
            human_drivers = [p.get('mName', '') for p in race_info['human_participants']]
            
        session_id = self.calculate_session_id_for_participants(
            participants, 
            {first_ip: (human_drivers[0] if human_drivers else 'Unknown', 1)},
            None, 
            False
        )
        
        # Initialize race state for all IPs in this race
        is_time_trial = data['eventInformation']['mLapsInEvent'] == 0
        race_group_letter = race_info.get('race_group_letter', 'A')
        
        for ip in ip_list:
            self.active_races[ip] = {
                'session_id': session_id,
                'race_id': 0,  # Will be set after database write
                'data': data,
                'lap_times': {},
                'driver_totals': {},
                'race_in_session': True,
                'track_location': race_info['track_location'],
                'time_trial': is_time_trial,
                'race_group_ips': ip_list.copy(),  # Store which IPs belong to this race
                'race_group_letter': race_group_letter  # Store race group identifier (A, B, C, D)
            }
        
        # Set as primary race if none exists (use first IP)
        if self.primary_race_ip is None:
            self.primary_race_ip = first_ip
            self.session_id = session_id
            self.race_id = 0  # Will be updated after DB write
            
        # Write race to database if not time trial (only write once per race group)
        if not is_time_trial:
            self.task_queue.put(('write_race', (data, session_id, ip_list)))
            print(f"MT Line {inspect.currentframe().f_lineno} Race written to DB for IPs: {ip_list}, session: {session_id}")
            logging.info(f"MT Line {inspect.currentframe().f_lineno} Race written to DB - IPs: {ip_list}, Session: {session_id}")

    def emit_active_races_update(self):
        """
        Emit signal with current active races information for GUI display.
        """
        active_races_info = {}
        for ip, race_data in self.active_races.items():
            # Get driver list from current data
            participants = race_data.get('data', {}).get('participants', {}).get('mParticipantInfo', [])
            human_drivers = [p.get('mName', '') for p in participants if p.get('mName') and not p.get('mName', '').endswith('(AI)')]
            
            # Get race_id from mapping
            race_id = self.get_race_id_for_ip(ip)
            
            active_races_info[ip] = {
                'session_id': race_data.get('session_id', 0),
                'race_id': race_id if race_id else 0,
                'track': race_data.get('track_location', 'Unknown'),
                'drivers': human_drivers,
                'is_primary': ip == self.primary_race_ip,
                'time_trial': race_data.get('time_trial', False)
            }
            
        self.active_races_updated.emit(active_races_info)

    def continue_race_for_ip(self, ip, race_data):
        """
        Continue processing an existing race for the given IP.
        """
        if ip not in self.active_races:
            return
            
        # Update race data
        self.active_races[ip]['data'] = race_data['data']
        participants = race_data['participants']
        
        # Process lap times and driver progress (similar to current race loop)
        lap_times_dict = self.active_races[ip]['lap_times']
        driver_total_times = self.active_races[ip]['driver_totals']
        
        # Get last lap counts (we'll need to track this per race)
        last_lap_counts = getattr(self, f'last_lap_counts_{ip}', {})
        
        for participant in participants:
            driver_name = participant['mName']
            current_lap = participant.get('mCurrentLap', 0)
            last_lap_time = participant.get('mLastLapTimes')
            
            if driver_name not in lap_times_dict:
                lap_times_dict[driver_name] = []
            if driver_name not in driver_total_times:
                driver_total_times[driver_name] = 0
                
            # Check for new lap completion
            if (current_lap > last_lap_counts.get(driver_name, 1)) and (last_lap_time != -123) and (last_lap_time is not None):
                driver_total_times[driver_name] += last_lap_time
                
                # Get race_id for this IP using the mapping
                race_id = self.get_race_id_for_ip(ip)
                if not self.active_races[ip]['time_trial'] and race_id and race_id > 0:
                    self.task_queue.put(('insert_lap_data', (
                        race_id, 
                        driver_name, 
                        current_lap - 1, 
                        last_lap_time, 
                        self.active_races[ip]['track_location'], 
                        participant['mCarNames']
                    )))
                    lap_times_dict[driver_name].append(last_lap_time)
                    
                last_lap_counts[driver_name] = current_lap
                
        # Update last lap counts for this IP
        setattr(self, f'last_lap_counts_{ip}', last_lap_counts)
        
        # If this is the primary race, emit data to GUI
        if ip == self.primary_race_ip:
            self.data_updated.emit(race_data['data'])
            
    def end_race_for_ip(self, ip):
        """
        Clean up a finished race for the given IP.
        """
        if ip not in self.active_races:
            return
            
        print(f"MT Line {inspect.currentframe().f_lineno} Race finished for IP {ip}")
        logging.info(f"MT Line {inspect.currentframe().f_lineno} Race ended - IP: {ip}, Session: {self.active_races[ip].get('session_id', 'Unknown')}")
        
        # Get race_id for cleanup before removing from active_races
        race_id = self.get_race_id_for_ip(ip)
        race_group_ips = self.active_races[ip].get('race_group_ips', [ip])
        
        # Clean up lap tracking
        if hasattr(self, f'last_lap_counts_{ip}'):
            delattr(self, f'last_lap_counts_{ip}')
            
        # Remove from active races
        del self.active_races[ip]
        
        # Check if all IPs in this race group have finished
        if race_id:
            remaining_ips_in_race = [ip for ip in race_group_ips if ip in self.active_races]
            if not remaining_ips_in_race:
                # All IPs in this race have finished, remove the race mapping
                self.remove_race_mapping(race_id)
                print(f"MT Line {inspect.currentframe().f_lineno} All IPs finished for race {race_id}, removed race mapping")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} Race {race_id} completely finished, mapping removed")
        
        # If this was the primary race, switch to another active race or reset
        if ip == self.primary_race_ip:
            if self.active_races:
                # Switch to another active race
                new_primary_ip = next(iter(self.active_races.keys()))
                self.switch_primary_race_to(new_primary_ip)
            else:
                # No more active races
                self.primary_race_ip = None
                self.race_id = 0
                self.session_id = 1

    def switch_primary_race_to(self, ip):
        """
        Switch the GUI view to display a different race.
        """
        if ip not in self.active_races:
            print(f"MT Line {inspect.currentframe().f_lineno} Cannot switch to IP {ip} - race not active")
            return
            
        self.primary_race_ip = ip
        race_state = self.active_races[ip]
        self.session_id = race_state['session_id']
        
        # Get race_id from mapping
        race_id = self.get_race_id_for_ip(ip)
        self.race_id = race_id if race_id else 0
        
        # Trigger GUI refresh by emitting current data
        self.data_updated.emit(race_state['data'])
        self.session_id_updated.emit(self.session_id)
        self.primary_race_changed.emit(ip)
        
        print(f"MT Line {inspect.currentframe().f_lineno} Switched primary race to IP {ip}, session {self.session_id}, race_id {self.race_id}")
        logging.info(f"MT Line {inspect.currentframe().f_lineno} Primary race switched to IP: {ip}, Session: {self.session_id}, Race: {self.race_id}")
        
        # Update active races display
        self.emit_active_races_update()

    def request_switch_to_race(self, ip):
        """
        Public method to switch primary race to a specific IP.
        Can be called from GUI.
        """
        if ip in self.active_races:
            self.switch_primary_race_to(ip)
            return True
        else:
            print(f"MT Line {inspect.currentframe().f_lineno} Cannot switch to IP {ip} - race not active")
            return False
    ...
    async def get_api_data(self):
        ...
        # Check testmode configuration
        testmode_enabled = self.config_manager.read_testmode_status()
        if testmode_enabled:
            return await self._get_simulated_api_data()

        # Original AMS2 API connection logic
    ...
    async def _get_simulated_api_data(self):
        """Get simulated API data from JSON files for testing"""
        try:
            import os
            import json
            
            # Handle API connection simulation (always show as connected in testmode)
            if not self.connecton_restored_message_shown:
                logging.info(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Simulated Connection")
                print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Using Simulated API Data")
                self.api_signal_status_updated.emit(False, "Testmode - Simulated Data")
                self.connecton_restored_message_shown = True
            
            self.no_server_connection = False
            
            # Look for API file matching current IP address
            api_dir = "api_files"
            filename = f"{self.ip_address}.json"
            ...
            if os.path.exists(filepath):
                ...
                # Update state tracking like real API data
                for k in ["Game", "Race", "Session"]:
                    current_state = data["gameStates"][f"m{k}State"]
                    attr = f"previous_{k.lower()}_state"
                    prev = getattr(self, attr)
                    if current_state != prev:
                        print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - {k} state changed to {current_state}")
                        logging.info(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - {k} state changed to {current_state}")
                        setattr(self, attr, current_state)
                
                return data if self.running else None
            else:
                # No simulation file found for this IP - return empty race data
                logging.warning(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - No simulation file found for {self.ip_address}")
                return await self._get_empty_race_data()
                
        except Exception as e:
            logging.error(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode error: {e}")
            print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode error: {e}")
            return await self._get_empty_race_data()
    
    async def _get_empty_race_data(self):
        """Return empty race data structure for testmode when no simulation file exists"""
        return {
            "buildinfo": {"mVersion": 14, "mBuildVersionNumber": 2913},
            "gameStates": {"mGameState": 1, "mSessionState": 0, "mSessionIsPrivate": 0, "mRaceState": 0},
            "participants": {"mViewedParticipantIndex": -1, "mNumParticipants": -1, "mParticipantInfo": []},
            "unfilteredInput": {"mUnfilteredThrottle": 0, "mUnfilteredBrake": 0, "mUnfilteredSteering": 0, "mUnfilteredClutch": 0, "mJoyPad0": 0, "mDPad": 0},
            "vehicleInformation": {"mCarName": "", "mCarClassName": ""},
            "eventInformation": {"mLapsInEvent": 0, "mSessionDuration": 0, "mSessionAdditionalLaps": 0, "mTrackLocation": "", "mTrackVariation": "", "mTrackLength": 0, "mTranslatedTrackLocation": "", "mTranslatedTrackVariation": ""},
            "timings": {"mLapInvalidated": False, "mBestLapTime": -1, "mLastLapTime": -1, "mCurrentTime": 0, "mSplitTimeAhead": 0, "mSplitTimeBehind": -1, "mSplitTime": 0, "mEventTimeRemaining": -1, "mPersonalFastestLapTime": -1, "mWorldFastestLapTime": -1, "mCurrentSector1Time": 0, "mCurrentSector2Time": 0, "mCurrentSector3Time": -1, "mFastestSector1Time": -1, "mFastestSector2Time": -1, "mFastestSector3Time": -1, "mPersonalFastestSector1Time": -1, "mPersonalFastestSector2Time": -1, "mPersonalFastestSector3Time": -1, "mWorldFastestSector1Time": -1, "mWorldFastestSector2Time": -1, "mWorldFastestSector3Time": -1},
            ...
            "carState": {"mCarFlags": 2, "mOilTempCelsius": 20, "mWaterTempCelsius": 20, "mWaterPressureKPa": 0, "mFuelPressureKPa": 0, "mFuelLevel": 0, "mFuelCapacity": 77, "mSpeed": 0, "mRpm": 0, "mMaxRPM": 8500, "mBrake": 0, "mThrottle": 0, "mClutch": 1, "mSteering": 0, "mGear": 0, "mNumGears": 6, "mOdometerKM": 0},
            "motionAndDeviceRelated": {"mOrientation": [0, 0, 0], "mLocalVelocity": [0, 0, 0], "mWorldVelocity": [0, 0, 0], "mAngularVelocity": [0, 0, 0], "mLocalAcceleration": [0, 0, 0], "mWorldAcceleration": [0, 0, 0], "mExtentsCentre": [0, 1.20686, 0.119275]},
            "wheelsAndTyres": {"mTyreFlags": [0, 0, 0, 0], "mTerrain": [0, 0, 0, 0], "mTyreY": [0, 0, 0, 0], "mTyreRPS": [0, 0, 0, 0], "mTyreSlipSpeed": [0, 0, 0, 0], "mTyreTemp": [20, 20, 20, 20], "mTyreGrip": [0, 0, 0, 0], "mTyreCompound": ["", "", "", ""]},
            "carDamage": {"mCrashState": 0, "mAeroDamage": 0, "mEngineDamage": 0},
            "weather": {"mAmbientTemperature": 20, "mTrackTemperature": 20, "mRainDensity": 0, "mWindSpeed": 0, "mWindDirectionX": 0, "mWindDirectionY": 0, "mCloudBrightness": 2, "mSnowDensity": 0},
            "timestamp": int(time.time() * 1000)
        }

    async def _fetch_testmode_multi_data(self):
        """Fetch multi-race data from API files in testmode"""
        try:
            test_ips = [
                "192.168.3.201", "192.168.3.202", "192.168.3.203", "192.168.3.204",
                "192.168.3.205", "192.168.3.206", "192.168.3.207", "192.168.3.208",
                "192.168.3.209", "192.168.3.210", "192.168.3.211", "192.168.3.212",
                "192.168.3.213", "192.168.3.214", "192.168.3.215", "192.168.3.216",
                "192.168.3.217", "192.168.3.218", "192.168.3.219", "192.168.3.220"
            ]
            ...
            for ip in test_ips:
                ...
                if os.path.exists(api_file):
                    try:
                        ...
                        logging.error(f"MT Line {inspect.currentframe().f_lineno}MT: Error reading testmode file {api_file}: {e}")
            
            if multi_data:
                print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Found {len(multi_data)} active races")
                logging.info(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - Multi-race data loaded for IPs: {list(multi_data.keys())}")
                
                # Set multi_data directly on the monitoring thread
                self.multi_data = multi_data
                
                # Also emit signal for RaceApp
                self.multi_data_signal.emit(multi_data)
            else:
                print(f"MT Line {inspect.currentframe().f_lineno}MT: Testmode - No active races found in API files")
                
        except Exception as e:
            logging.error(f"MT Line {inspect.currentframe().f_lineno}MT: Error fetching testmode multi-data: {e}")
            print(f"MT Line {inspect.currentframe().f_lineno}MT: Error fetching testmode multi-data: {e}")
    ...
    async def race_running(self, data):
        ...
        self.gui_main_app.session_id_updated.emit(self.session_id)
        ...
        while self.race_in_session:
            ...
            try:
                ...
                for participant in participants:
                    ...
                    if (participant.get('mCurrentLap', 0) > last_lap_counts.get(participant['mName'], 1)) and (participant.get('mLastLapTimes') != -123) and (participant.get('mLastLapTimes') is not None):
                        if participant['mName'] not in lap_times_dict: lap_times_dict[participant['mName']] = [] # Initialize the key with an empty list
                        if participant['mName'] not in driver_total_times: driver_total_times[participant['mName']] = 0  # Initialize the key with a value of 0                                    
...
class GuiMainApp(QMainWindow):
    ...
    live_view_reset = pyqtSignal()  # Signal to reset live view when race group changes
    ...
    def init_threads_and_managers(self):
        ...
        self.db_thread = DatabaseThread(self.task_queue, self.config_manager, self.monitor_thread)
    ...
    def connect_signals(self):
        ...
        self.live_view_reset.connect(self.race_app.reset_live_view)  # Connect live view reset signal to RaceApp
    ...
    def init_state_variables(self):
        ...
        # Initialize race group tracking for radio button switching
        self.current_race_group = None
        self.last_radio_data = None
    ...
    def handle_radio_button_update(self, data):
        print(f"GA Line {inspect.currentframe().f_lineno} Handling radio button update with data: {data}")
        """
        Handle radio button updates based on either:
        - List of active IPs → legacy behavior (only enable/disable buttons).
        - Dict of {ip: [name, in_race, race_group_letter]} → set label + enable/disable + color coding.
        """
        if data is None:
            ...
            # Check for race group changes before updating data
            if isinstance(data, dict) and hasattr(self, 'last_radio_data') and isinstance(self.last_radio_data, dict):
                # Find currently selected radio button's IP
                selected_button = self.sim_radio_group.checkedButton()
                if selected_button:
                    selected_index = self.sim_radio_group.id(selected_button)
                    if 0 <= selected_index < len(self.available_ips):
                        selected_ip = self.available_ips[selected_index]
                        
                        # Get race group from old data
                        old_button_data = self.last_radio_data.get(selected_ip, [])
                        old_race_group = old_button_data[2] if len(old_button_data) >= 3 else 'A'
                        
                        # Get race group from new data
                        new_button_data = data.get(selected_ip, [])
                        new_race_group = new_button_data[2] if len(new_button_data) >= 3 else 'A'
                        
                        # Check if race group changed
                        if old_race_group != new_race_group:
                            print(f"GA Line {inspect.currentframe().f_lineno} Race group changed from {old_race_group} to {new_race_group} for IP {selected_ip}")
                            self.live_view_reset.emit()  # Signal RaceApp to reset live view
                            print(f"GA Line {inspect.currentframe().f_lineno} Emitted live_view_reset signal due to race group change")
            
            self.active_computers = data
            # Store radio button data for race group detection
            self.last_radio_data = data

        # Define colors for different race groups
        race_group_colors = {
            'A': '#FF6B6B',  # Red
            'B': '#4ECDC4',  # Teal  
            'C': '#45B7D1',  # Blue
        ...
        for i, ip in enumerate(self.available_ips):
            ...
            if isinstance(data, dict):  # New style: {ip: [name, in_race, race_group_letter]}
                button_data = data.get(ip, [f"S-{i+1}", 0, 'A'])
                
                if len(button_data) >= 3:
                    name, in_race, race_group_letter = button_data[:3]
                else:
                    name, in_race = button_data[:2] if len(button_data) >= 2 else [f"S-{i+1}", 0]
                    race_group_letter = 'A'
                
                # Set button text with race group indicator
                button.setText(f"{race_group_letter}-{name}")
                button.setEnabled(bool(in_race))
                
                # Set color based on race group, with special handling for checked state
                group_color = race_group_colors.get(race_group_letter, '#FF6B6B')
                checked_color = '#00FF00'  # Green for checked button
                ...
                print(f"GA Line {inspect.currentframe().f_lineno} Button {i+1}: {race_group_letter}-{name}, {'ENABLED' if in_race else 'DISABLED'} (Group: {race_group_letter})")
                
            elif isinstance(data, list):  # Old style: list of IPs
                button.setEnabled(ip in data)
                # Reset to default styling for old format
                button.setStyleSheet(f"""
                    QRadioButton::indicator {{
                        width: 0px;
                        height: 0px;
    ...
    def show_version_update_message(self, existing_version, new_version):
        ...
        try:
            ...
            detailed_text = (
                f"WARNING: This version change requires database reset!\n\n"
                f"Your current version: {existing_version}\n"
                f"New version: {new_version}\n\n"
                f"This legacy version change requires deleting existing race data\n"
                f"because automatic migration is not available.\n\n"
                f"Note: v1.7.0+ supports automatic data migration without deletion.\n\n"
                f"Do you want to proceed and reset the database?"
            )
            msg_box.setInformativeText(detailed_text)
            ...
            logging.error(f"GA Line {inspect.currentframe().f_lineno} Failed to show version update dialog: {e}")
            # Fallback - assume user doesn't want to delete data
            return False 
    ...
    def set_monitored_ip(self, ip_index):
        if 0 <= ip_index < len(self.available_ips):
            new_ip = self.available_ips[ip_index]
            
            # Get race group letter for the new IP
            new_race_group = self.get_race_group_for_ip(new_ip)
            
            # Check if we're switching between different race groups
            if hasattr(self, 'current_race_group') and hasattr(self, 'current_ip'):
                current_race_group = self.get_race_group_for_ip(self.current_ip)
                if new_race_group != current_race_group:
                    print(f"GA Line {inspect.currentframe().f_lineno} Race group changed from {current_race_group} to {new_race_group}, resetting live view")
                    self.live_view_reset.emit()  # Signal RaceApp to reset live view
            
            # Update current IP and race group
            self.current_ip = new_ip
            self.current_race_group = new_race_group
            
            print(f"GA Line {inspect.currentframe().f_lineno} set_monitored_ip: {self.current_ip} (Group: {new_race_group})")
    ...
    def get_race_group_for_ip(self, ip):
        """Get the race group letter for a given IP address"""
        # Check if we have radio button data with race group information
        if hasattr(self, 'last_radio_data') and isinstance(self.last_radio_data, dict):
            ip_data = self.last_radio_data.get(ip, [])
            if len(ip_data) >= 3:
                return ip_data[2]  # race_group_letter is at index 2
        
        # Fallback: determine group based on IP index (A, B, C, D for every 4 IPs)
        try:
            ip_index = self.available_ips.index(ip)
            return chr(65 + (ip_index // 4))  # A=65, B=66, C=67, D=68
...
class RaceApp(QThread):
    ...
    def reset_live_view(self):
        """Reset live view when race group changes are detected"""
        logging.info(f"RA Line {inspect.currentframe().f_lineno} Def reset_live_view entered - resetting live view flags")
        print(f"RA Line {inspect.currentframe().f_lineno} Def reset_live_view entered - resetting live view flags")
        self.live_first_time_run = True
        self.live_first_time_run_done = False
        self.on_first_live_run_started = False
    ...
    def update_driver_names(self, ip_name_flag):
        ...
        Format: {'192.168.3.201': ['S-1', 1, 'A'], '192.168.3.202': ['S-2', 0, 'B']}
        """
        driver_names_dict = {}
        for sim_index, (ip, ip_data) in enumerate(ip_name_flag.items(), start=1):
            # Handle both old format [name, in_race] and new format [name, in_race, race_group_letter]
            if len(ip_data) >= 2:
                name, in_race = ip_data[:2]
                if in_race == 1:
    ...
    def on_first_live_view_run(self, data):
        ...
        # Initialize driver_sims immediately to prevent AttributeError during execution
        self.driver_sims = {}
        ...
        self.heading_session = (f"Session Type: {getattr(self, 'session_type', 'Unknown')} - ")
        self.heading_track = (
    ...
    def update_live_view(self, data):
        ...
        if not getattr(self, 'multi_data', None):
            print(f"RA Line {inspect.currentframe().f_lineno} No multi_data available, skipping live view update.")
            return
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            carname = participant.get('mCarNames', 'Unknown Car')
            self.current_lap[name] = participant['mCurrentLap']
            if participant.get('mLastLapTimes', -123) != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    self.lap_time[name] = participant.get('mLastLapTimes', 0)
        ...
        if self.best_lap_time < self.previous_best_lap_time or self.live_first_time_run:
            for j in range(min(24, len(sorted_participants))):
                p = sorted_participants[j]
                bg_color = (self.top_background_color
                            if p.get('mFastestLapTimes', float('inf')) <= self.best_lap_time and p['mCurrentLap'] > 1
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            last_lap_str   = self.format_lap_time(participant.get('mLastLapTimes', 0))
    ...
    def get_full_api_data_by_driver_name(self, name):
        ...
        if not hasattr(self, "multi_data") or not hasattr(self, "ip_name_flag"):
            return None
        for ip, ip_data in self.ip_name_flag.items():
            # Handle both old format [name, in_race] and new format [name, in_race, race_group_letter]
            if len(ip_data) >= 2:
                driver_name, in_race = ip_data[:2]
                if driver_name == name:
    ...
    def update_driver_view(self, data):
        ...
        carnode = data.get('carState', {})
        timingsnode = data.get('timings', {})
        ...
        inputnode = data.get('unfilteredInput', {})
        speed = int(carnode.get('mSpeed', 0) * 3.6)
        crashindex = carnode.get('mLastOpponentCollisionIndex', -1)
        crashname = participants['mParticipantInfo'][crashindex]['mName'] if crashindex != -1 else "None"
        rpm = carnode.get('mRpm', 0)
        #mMaxRPM = carnode.get('mMaxRPM', 0)
        bestlap = timingsnode.get('mBestLapTime', -1)
        ...
        self.previous_carname = carname
        laptime = self.format_lap_time(timingsnode.get('mCurrentTime', 0) if timingsnode.get('mCurrentTime', -1) != -1 else 0)
        remaining = timingsnode.get('mEventTimeRemaining', -1)
        gear = carnode.get('mGear', 0)
        ...
        self.gui_realtime_update.emit({
            "steering": data.get('unfilteredInput', {}).get('mUnfilteredSteering', 0),
            "speed": speed,
            "rpm": rpm,
            "throttle": throttle,
            "brake": brake,
    ...
    def identical_cars(self, data):
        ...
        # Filter out SafetyCars and PaceCars with safe key access
        car_names = []
        for p in participants:
            if 'mCarNames' in p:
                car_name = p['mCarNames']
                if 'Safety' not in car_name and 'PaceCar' not in car_name:
    ...
    def create_participant_text(self, participant, flag, last_lap_str, lap_times_str, sessionstate,
        ...
        participant_speed = participant.get('mSpeeds', 0)
        if self.top_speed.get(name, -1000) < math.floor(participant_speed):
            self.top_speed[name] = math.floor(participant_speed)
        ...
        fastest_lap_str = self.format_lap_time(participant.get('mFastestLapTimes', 0))
        ...
        pit_status = f"<span style='color:#00FF00;font-weight:bold'>(PIT)</span>" if participant.get('mPitModes', 0) != 0 else ""
        ...
        output += (
            f"<span style='color:#00fff0;'>Speed: <span style='color:#FFD700;'>{participant_speed * 3.6:.0f} Km/t</span> "
    ...
    def update_best_lap_and_speed(self, participant, sessionstate):
        ...
        participant_speed = participant.get('mSpeeds', 0)
        if math.floor(participant_speed) > self.best_top_speed:
            self.best_top_speed = math.floor(participant_speed)
            self.top_speed_driver = name
            updated = True

        if (
            participant.get('mFastestLapTimes', float('inf')) < self.best_lap_time
            and participant['mCurrentLap'] > 1
            and participant.get('mFastestLapTimes', -123) != -123
        ):
            self.best_lap_time = participant.get('mFastestLapTimes', 0)
    ...
    def handle_pit_stop(self, participant):
        ...
        if participant.get('mPitModes', 0) == 2 and participant['mCurrentLap'] not in self.pit_stop[name]:
