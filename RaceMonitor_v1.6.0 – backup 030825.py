...
class RaceApp(QThread):
    ...
    def on_first_live_view_run(self, data):
        ...
        self.driverlocalrecorddb = {}  # Reset track record tracking for each update
        ...
        self.heading_wrecord = (
            f"World record: {(trackworldrecord if trackworldrecord != 0 else 'MOD Car/Track')} - ")
                ...
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if self.classrace:
                        ...
                        if not self.waiting_for_highscores:
                            if carname in self.driverlocalrecorddb:
                                if self.lap_time[name] < self.driverlocalrecorddb[carname]['worldrecord']:
    ...
    def update_driver_view(self, data):
        ...
        if self.driver_view_first_update: # Or if IP changes
            ...
            if not self.waiting_for_highscores:
                ...
                self.driverlocalrecorddb[self.carname] = { 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
        ...
        if self.classrace:
            if self.carname != self.previous_carname:  # Check if the car has changed
                ...
                if self.carname not in self.driverlocalrecorddb and not self.waiting_for_highscores:
                    self.task_queue.put(('load_highscores', (self.carname, self.selected_track)))
                    self.waiting_for_highscores = True
                    self.local_record_updated = False
                else: 
                    self.worldrecorddb = self.driverlocalrecorddb[self.carname]['worldrecord']
                    self.recorddriver = self.driverlocalrecorddb[self.carname]['recorddriver']
            ...
            if not self.local_record_updated and not self.waiting_for_highscores:
                    ...
                    self.driverlocalrecorddb[self.carname] = {'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
                    self.update_heading(self.carname, self.trackworldrecord, self.worldrecorddb) 
        ...
        if self.local_record_updated:
            ...
            if self.deltalocalrecord < 0 and not self.deltarecordupdated: 
                ...
                self.driverlocalrecorddb[self.carname] = {'worldrecord': bestlap, 'recorddriver': drivername}
    ...
    def identical_cars(self, data):
        ...
        # Filter out SafetyCars
        car_names = [p['mCarNames'] for p in participants if p['mCarNames'] != 'SafetyCar']

        # If there are no non-SafetyCar entries, we can't compare — return False
        for carname in car_names:
