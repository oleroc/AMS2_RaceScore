    ...
    def load_highscores(self, car, track):
        ...
        try:
            ...
            if high_scores:
                ...
                self.highscore_data_loaded_signal.emit(high_scores, car)
            else:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} No High Scores found, emitting None")
                self.highscore_data_loaded_signal.emit(None, car)
        except Exception as e:
...
class RaceMonitorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.calendar_widget.setStyleSheet("""
            ...
            padding: 2px 5px;  /* Optional: padding inside the dropdown */
        """)
        self.calendar_widget.setGeometry(700, 1075, 300, 150)  # Set the geometry of the calendar widget
    ...
    def on_highscore_loaded(self, high_scores, car):
        ...
        if self.tab_widget.currentWidget() != self.highscore_view_widget:
            if high_scores != None:
                ...
                self.driverlocalrecorddb[car] = { 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
    ...
    def on_first_live_view_run(self, data):
        ...
        self.classrace = self.identical_cars(data)  # Check if all cars are identical
        ...
        f"Session ID: {self.session_id} - World record: {('Mod Car/Track' if self.trackworldrecord == -1 else self.format_lap_time(self.trackworldrecord))}"
        )
            ...
            if participant['mLastLapTimes'] != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if carname in self.driverlocalrecorddb:
                        print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record: {self.driverlocalrecorddb}")
                        print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , WorldTrack Record: {self.trackworldrecord}, Local Record: {self.driverlocalrecorddb[carname]['worldrecord']}")
                        if self.lap_time[name] < self.driverlocalrecorddb[carname]['worldrecord']:
                            ...
                            self.driverlocalrecorddb[carname] = {'worldrecord': self.lap_time[name], 'recorddriver': name}
                        ...
                        self.world_record_label.setText(f"Brand New Local Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                        self.driverlocalrecorddb[carname] = {'worldrecord': self.lap_time[name], 'recorddriver': name}
                        self.update_heading(carname, self.trackworldrecord, self.lap_time[name])
                        self.flash_timer.start(500)
                    if carname in self.driverworldrecord:
                        ...
                        if self.lap_time[name] < self.driverworldrecord[carname]['worldrecord']:
                            self.world_record_label.setText(f"Laptime: {self.lap_time[name]} New World Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                            ...
                            self.driverworldrecord[carname] = {'worldrecord': self.lap_time[name], 'recorddriver': name}
    ...
    def update_driver_view(self, data):
        if (self.tab_widget.currentWidget() != self.driver_info_tab and self.driver_view_first_update == False) or not data or 'participants' not in data or 'mParticipantInfo' not in data['participants']:
            return
        ...
        if self.driver_view_first_update:
            ...
            if not self.waiting_for_highscores:
                print(f"RMA Line {inspect.currentframe().f_lineno} Driver view first update, track record db: {self.trackrecorddb}, track: {self.selected_track}, car: {carname}, driver: {drivername} worldrecord: {self.trackworldrecord},drivertrackrecorddb {self.driverlocalrecorddb}")                
                if carname not in self.driverlocalrecorddb:
                    self.driverlocalrecorddb[carname] = { 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
                    print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname} not found, creating new entry.")
                self.localrecord = self.driverlocalrecorddb[carname]['worldrecord']
                self.recorddriver = self.driverlocalrecorddb[carname]['recorddriver']
        ...
        if carname != self.previous_carname:  # Must be a classrace.
            ...
            if carname not in self.driverlocalrecorddb:
                self.driverlocalrecorddb[carname] = {'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
                print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname} not found, creating new entry.")
            self.localrecord = self.driverlocalrecorddb[carname]['worldrecord']
        ...
        # Batch UI updates
        if  not self.waiting_for_highscores:
            #print (f"RMA Line {inspect.currentframe().f_lineno} self.local_record_updated: {self.local_record_updated}, self.waiting_for_highscoresr: {self.waiting_for_highscores}, worldrecord: {self.worldrecord}, worldrecorddb: {self.localrecord}, deltaworldrecord: {deltaworldrecord}, deltalocalrecord: {deltalocalrecord}")
            updates = [
                ...
                (self.localrecord_txt.setText, (f"{self.format_lap_time(self.localrecord)} - {self.recorddriver}" if self.localrecord != 0 else "No Record",)),
    ...
    def identical_cars(self, data):
        ...
        for carname in car_names:
            if carname != current_car:
               self.task_queue.put(('load_highscores', (carname, self.selected_track)))
            else:
                print(f"RMA Line {inspect.currentframe().f_lineno} Car {carname} already loaded, skipping load_highscores task.")
                continue
            current_car = carname        
    ...
    def update_heading(self, new_car, worldrecord, localrecord):
        ...
        if "Local Record: " in current_text:
            ...
            self.live_heading.setText(current_text[:local_record_start] + localrecord + (f" for {new_car}" if self.classrace else ""))
            current_text = self.live_heading.text()
        else:
            self.live_heading.setText(f"{current_text} - Local Record: {self.format_lap_time(localrecord) if localrecord != 0 else ' First Race'} {f' for {new_car}' if self.classrace else ''}")
            current_text = self.live_heading.text()
        if new_car != "skip":
