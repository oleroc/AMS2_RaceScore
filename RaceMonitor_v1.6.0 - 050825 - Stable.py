...
class GuiMainApp(QMainWindow):
    ...
    def set_session_id(self, new_session_id, dato):
        ...
        if new_session_id is None:
            print(f"GA Line {inspect.currentframe().f_lineno} Session ID not found in the database, setting to 1")
            logging.info(f"GA Line {inspect.currentframe().f_lineno} Session ID not found in the database, setting to 1")
            self.session_id = 1
            ...
            if dato != datetime.now().date():
                ...
                self.session_id = new_session_id
                print(f"GA Line {inspect.currentframe().f_lineno} Session ID is for today, setting to {self.session_id}.")
            self.session_id_updated.emit(self.session_id)
            self.latest_session_id = new_session_id
        print(f"GA Line {inspect.currentframe().f_lineno} set_session:id: Latest DB Session ID set to new_session_id or 0 if empty DB: {self.latest_session_id}")
    ...
    def display_final_results(self, race_valid, session_id): # Display the score data in the final view
        ...
        if race_valid:
            if session_id > self.latest_session_id:
                print(f"GA Line {inspect.currentframe().f_lineno} Display Final Result. Self.latest_session_id: {self.latest_session_id} wil be updated to: {session_id}")
    ...
    def calculate_score(self, races, participants, laps):
        ...
        if races: # Iterate over each race
            ...
            if self.latest_session_id is None:
                print(f"GA Line {inspect.currentframe().f_lineno} Races is empty, and self.latest_session_id None: {self.latest_session_id} DB is empty, set session_id to 1")                
                self.session_id = 1
            else:
                print(f"GA Line {inspect.currentframe().f_lineno} Races is empty, but self.latest_session_id is not None: {self.latest_session_id} No Session Changes: {self.session_id}")   
            self.session_id_updated.emit(self.session_id)
    ...
    def format_score_view(self, total_scores, race_count, last_positions, medal_counts, races):
        ...
        print(f"GA Line {inspect.currentframe().f_lineno} Latest session ID: {self.latest_session_id}")
        heading = (
            f"<span style='color:{self.color_labels}'>Session:</span>"
            f"<span style='color:{self.color_values}'> "
    ...
    def handle_latest_session_id(self, session_id, dato):
        logging.info(f"GA Line {inspect.currentframe().f_lineno} Def handle_latest_session_id entered")
        if session_id is not None:
            self.latest_session_id = session_id
        else:
            self.latest_session_id = 1
        print(f"GA Line {inspect.currentframe().f_lineno} Latest Session ID: {self.latest_session_id}")
        logging.info(f"GA Line {inspect.currentframe().f_lineno} RA Latest Session ID: {self.latest_session_id}")

    def start_new_session(self):
        logging.info(f"GA Line {inspect.currentframe().f_lineno} RA Start New Session entered")
        # self.display_score()
        ...
        print(f"GA Line {inspect.currentframe().f_lineno} Get latest session ID from DB. Line 3074 {inspect.currentframe().f_lineno}")
        ...
        if self.session_id - self.latest_session_id > 1:
            ...
            print(f"GA Line {inspect.currentframe().f_lineno} Max session is {self.session_id} rest of def will not be executed")  # Rest will not be executed.
        else:
            print(f"GA Line {inspect.currentframe().f_lineno} Sending session: {self.session_id} to load_score_data")
            logging.info(f"GA Line {inspect.currentframe().f_lineno} Sending session: {self.session_id} to load_score_data")
        ...
        print(f"GA Line {inspect.currentframe().f_lineno} Start New session.  Emitting self.session ID {self.session_id}")
        self.session_id_updated.emit(self.session_id)
        self.live_heading.setText(f"Waiting for Race Start. Current Session: {self.session_id}")  # Update the status label 
        
    def previous_session(self):
        logging.info(f"GA Line {inspect.currentframe().f_lineno} Def go back session entered")
        print(f"GA Line {inspect.currentframe().f_lineno} Self selected index: {self.session_id}")
        print(f"GA Line {inspect.currentframe().f_lineno} Session_id_dropdown: {self.session_id_dropdown}")
        self.session_id -= 1
        ...
        print(f"GA Line {inspect.currentframe().f_lineno} previous_session. Emitting Self.session ID: {self.session_id}")
        self.session_id_updated.emit(self.session_id)
