...
class DatabaseThread(QThread):
    ...
    def insert_lap_data(self, race_id, name, current_lap, lap_time, track, car):
        ...
        try:
            ...
            if name.endswith("(AI)"):
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Lap not written (AI)")
                print(f"DB Line {inspect.currentframe().f_lineno} Lap not written (AI)")
                return
            self.cursor.execute('''
...
class MonitorThread(QThread):
    ...
    async def get_api_data(self):
        def _handle_restore():
            if not self.connecton_restored_message_shown:
                logging.info("MT: Connection Restored")
                print("MT: Connection Restored")
        ...
        async def _handle_error(msg, sleep=5, exception=None):
            if not self.connecton_restored_message_shown:
                print(f"MT: {msg}")
                logging.error(f"MT: {msg}" + (f" - {exception}" if exception else ""))
        ...
        def _update_state(label, current):
            ...
            if current != prev:
                print(f"MT: {label} state changed to {current}")
                logging.info(f"MT: {label} state changed to {current}")
        ...
        try:
            ...
            logging.info("MT: Operation cancelled")
    ...
    async def connection_monitor(self):
        ...
        available_ips = [ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '').strip() for i in range(1, 21) if ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '')]
        
        def scan_active_computers():
            active = []
            for i, ip in enumerate(available_ips):
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(0.05)
                try:
                    if sock.connect_ex((ip, 8180)) == 0:
                        #print(f"MT Line {inspect.currentframe().f_lineno} Active computer found: {ip}")
                        active.append(ip)
                finally:
                    sock.close()
            return active
        ...
        while self.running:
            try:
                ...
                if time.monotonic() - last_scan_time >= 360000: #Scan every 100 hours #Disabled for now, since its run on race start.
                    print(f"MT Line {inspect.currentframe().f_lineno} Scanning for active computers...")
                    last_scan_time = time.monotonic()
                    active_computers = await asyncio.to_thread(scan_active_computers)
                    self.radio_button_update.emit(active_computers) #Disabled for now, since its run on race start.
                    #print(f"MT Line {inspect.currentframe().f_lineno} Active computers: {active_computers}")
                    ...
                    if participant['mCurrentLap'] <= data['eventInformation']['mLapsInEvent']: # Driver has fewer laps less than he should, either False start Or DNF
                        if driver_flags is not None:
                            ...
                            if driver_flags.get(participant_name) != 'Falsestart': # Check if the participant has a 'Falsestart' flag
                                ...
                                await asyncio.sleep(0.5)
                if not self.time_trial:                                
                    self.task_queue.put(('finalize_race', (data, lap_times_dict,driver_flags, self.pit_stops_dict))) # Write final data to DB
...
class RaceMonitorApp(QMainWindow): #Soon to be rename GuiMainApp
    ...
    #update_track_dbrecord = pyqtSignal(object, object)  # Signal to update the track database record
    ...
    def __init__(self):
        ...
        self.race_app = RaceApp(self.monitor_thread, self.task_queue, self.config_manager, self.db_thread, None, self, self.tab_widget, self)
        #newly added
        #self.race_app = RaceApp(None, self.task_queue, self.config_manager, self.db_thread, None, self, self.tab_widget)
        #self.race_app = RaceApp(self.monitor_thread, self.task_queue, self.config_manager, self.db_thread, None, self, self.tab_widget, self)  # Pass the tab_widget to RaceApp
        ...
        #newly added
        #self.race_app.gui_live_update.connect(self.handle_gui_live_updates)
        #self.race_app.gui_driver_update.connect(self.handle_gui_driver_updates)
    ...
    def initUI(self):
        ...
        print(f"RMA Line {inspect.currentframe().f_lineno} InitGUI button ID:", id(self.new_session_button))
        ...
        #self.previous_session_button.show() 
        ...
        # Add the Find Computers button
        self.find_computers_button = QPushButton("Find Computers", self)
        self.labels['find_computers_button'] = self.find_computers_button
        self.find_computers_button.setStyleSheet("""
            QPushButton {                                      
            font-size: 14px;
            background-color: #a32d2d;
            color: white;
        ...
        """)        
        self.find_computers_button.setFixedSize(150, 30)
        self.find_computers_button.clicked.connect(self.scan_active_computers)
        self.layout.addWidget(self.find_computers_button)

        # Enable/Disable CP writing of score data
        self.enable_disable_cp_button = QPushButton("Disable CP", self)
        self.labels['Enable_CP'] = self.enable_disable_cp_button
        self.enable_disable_cp_button.setStyleSheet("""
            QPushButton {                                      
            font-size: 14px;
            background-color: #a32d2d;
            color: white;
        ...
        """)        
        self.enable_disable_cp_button.setFixedSize(150, 30)
        self.enable_disable_cp_button.clicked.connect(self.enable_disable_cp)
        self.layout.addWidget(self.enable_disable_cp_button)   
        ...
        self.tab_widget_mapping = {  # Widget visibility mapping for each tab
            ...
            5: ['find_computers_button', 'Enable_CP']  # Widgets for Driver Info
        }
    
    def needle_rotation(self, data):
    ...
    '''
    def handle_gui_live_updates(self, gui_data):
        if 'heading' in gui_data:
                self.live_heading = gui_data.get(gui_data['heading'], None)

            # Add more cases as needed
        """Handle GUI updates from the RaceApp."""
    ...
    def enable_disable_cp(self):
        ...
        if self.config_manager.cp_enabled("check"):
            ...
            self.enable_disable_cp_button.setText("Enable CP")
            print(f"RMA Line {inspect.currentframe().f_lineno} Control Panel writing disabled.")
        else:
            self.config_manager.cp_enabled (True)
            self.enable_disable_cp_button.setText("Disable CP")
            print(f"RMA Line {inspect.currentframe().f_lineno} Control Panel writing enabled.")
    ...
    def scan_active_computers(self):
        ...
        for i, ip in enumerate(self.available_ips):
            ...
            try:
                ...
                if result == 0:
                    self.active_computers.append(ip)
    ...
    def base64_to_font(self, base64_str):
        ...
        if not buffer.open(QBuffer.ReadOnly):
            print(f"RMA Line {inspect.currentframe().f_lineno} Failed to open buffer for font data.")
            return None

        font_id = QFontDatabase.addApplicationFontFromData(buffer.data())

        if font_id == -1:
            print(f"RMA Line {inspect.currentframe().f_lineno} Failed to load font from memory!")
            logging.error("Failed to load font from base64 memory.")
            return None

        font_family = QFontDatabase.applicationFontFamilies(font_id)[0]
        print(f"RMA Line {inspect.currentframe().f_lineno} Font loaded successfully from memory: {font_family}")
        return font_family
    ...
    def on_race_loaded(self, race, participants, laps):
        ...
        if race_id:
            ...
            if not participants:
                logging.info(f"RMA Line {inspect.currentframe().f_lineno} No participants found for the selected race.")
                print(f"RMA Line {inspect.currentframe().f_lineno} No participants found for the selected race.")
    ...
    def on_highscore_loaded(self, high_scores):
        ...
        if self.tab_widget.currentWidget() != self.highscore_view_widget:
            if high_scores != None:
                ...
                print(f"RMA Line {inspect.currentframe().f_lineno} Track Record: {self.trackrecorddb} by {self.recorddriver}")
            else:
                print(f"RMA Line {inspect.currentframe().f_lineno} No High Scores found in DB")
                self.trackrecorddb = 0
                self.recorddriver = ""
            #self.live_heading.setText(f" {self.live_heading.text()} - Local Record:{self.format_lap_time(self.trackrecorddb) if self.trackrecorddb != 0 else 'N/A'}")
            #self.update_heading("skip", self.trackworldrecord, self.trackrecorddb)
            #self.update_track_dbrecord.emit(self.trackrecorddb, self.recorddriver)  # Emit the signal with the track record and driver name
    ...
    def update_api_status(self, status: bool, message: str = ""):
        if status:  # Error occurred
            logging.info(f"RMA Line {inspect.currentframe().f_lineno} Error message triggered")
            print(f"RMA Line {inspect.currentframe().f_lineno} Error message triggered")
            ...
            logging.info(f"RMA Line {inspect.currentframe().f_lineno} Error cleared")
            print(f"RMA Line {inspect.currentframe().f_lineno} Error cleared")
            if hasattr(self, 'error_label'):
                self.error_label.deleteLater()
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        #print(f"RMA Line {inspect.currentframe().f_lineno} Available IPs: {self.available_ips}")
        #self.driver_info_tab.setLayout(QVBoxLayout())  # Ensure tab has a layout
        self.sim_radio_group = QButtonGroup(self.driver_info_tab)
        for i, ip in enumerate(self.available_ips):
            btn = QRadioButton(str(i + 1))
            btn.setToolTip(ip)  # Show IP on hover
            btn.setFixedSize(40, 20)
            btn.setParent(self.driver_info_tab)
            btn.move(925 + (i % 5) * 45, 800 + (i // 5) * 25)  # Manual grid (5x4)
            self.sim_radio_group.addButton(btn, i)  # Add button to the group
            btn.setFixedSize(40, 20)
            #self.driver_info_tab.addWidget(btn)
            #self.driver_view_layout.addWidget(btn, row, col)  # Use grid layout with row, col
        self.sim_radio_group.buttonClicked.connect(lambda button: self.set_monitored_ip(self.sim_radio_group.id(button)))
        ...
        #self.sim_radio_group.button(0).setChecked(True)
    

    ''' 
    def update_driver_view(self, data):
        if self.tab_widget.currentWidget() != self.driver_info_tab or not data or 'participants' not in data or 'mParticipantInfo' not in data['participants'] or hasattr(self, 'driver_view_first_update') == False:
            return  # Early exit for inactive tab or invalid data or if driver_view_first_update is not set

        if self.driver_view_first_update:
            self.track_txt.setText(self.selected_track)
            ...
            self.local_record_updated = False
            if not self.waiting_for_highscores:
                #self.liverecorddb = self.trackrecorddb #self track record is fetched from DB and  initialized in live view first run
                print(f"RMA Line {inspect.currentframe().f_lineno} track record db: {self.trackrecorddb}, car: {carname}, driver: {drivername}")
                self.drivertrackrecorddb[carname] = { 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
                self.drivertrackrecord[carname] = {'worldrecord': self.trackworldrecord}
                self.driver_view_first_update = False
            self.previous_carname = data['vehicleInformation']['mCarName']
            #self.worldrecord = data['timings']['mWorldFastestLapTime']
            print(f"RMA Line {inspect.currentframe().f_lineno} Driver view first update, track: {self.selected_track}, car: {carname}, driver: {drivername} worldrecord: {self.trackworldrecord}")
        ...
        if self.classrace:
            if carname != self.previous_carname:  # Check if the car has changed
                if carname not in self.drivertrackrecord:
                    self.drivertrackrecord[carname] = {'worldrecord': self.trackworldrecord}
                self.trackworldrecord = data['timings']['mWorldFastestLapTime']
                print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, new car detected: {carname} - Old Car: {self.previous_carname}")
                if carname not in self.drivertrackrecorddb and not self.waiting_for_highscores:
                    self.task_queue.put(('load_highscores', (carname, self.selected_track)))
                    self.waiting_for_highscores = True
                    self.local_record_updated = False
                else: 
                    self.worldrecorddb = self.drivertrackrecorddb[carname]['worldrecord']
                    self.recorddriver = self.drivertrackrecorddb[carname]['recorddriver']
                    self.update_heading(carname, self.trackworldrecord, self.worldrecorddb)
                    print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, worldrecorddb for driver: {drivername}, car: {carname} already exists: {self.worldrecorddb}")

            if not self.local_record_updated and not self.waiting_for_highscores:
                    print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, worldrecddb update for driver: {drivername}, car: {carname}")
                    self.worldrecorddb = self.trackrecorddb
                    self.drivertrackrecorddb[carname] = {'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
                    self.update_heading(carname, self.trackworldrecord, self.worldrecorddb) 
                    self.local_record_updated = True
        else:
            if drivername not in self.drivertrackrecord:
                self.drivertrackrecord[carname] = {'worldrecord': self.trackworldrecord}

            if not self.waiting_for_highscores:
                self.worldrecorddb = self.trackrecorddb
                self.local_record_updated= True
        if self.local_record_updated:
            self.deltalocalrecord = bestlap - self.worldrecorddb if bestlap != -1 and self.worldrecorddb != 0 else 0
            if self.deltalocalrecord < 0: 
                print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, deltaworldrecord: {self.deltaworldrecord}, deltalocalrecord: {self.deltalocalrecord}")
                self.drivertrackrecorddb[carname] = {'worldrecord': bestlap, 'recorddriver': drivername}  
        ...
        # Batch UI updates
        if  not self.waiting_for_highscores:
            #print (f"RMA Line {inspect.currentframe().f_lineno} self.local_record_updated: {self.local_record_updated}, self.waiting_for_highscoresr: {self.waiting_for_highscores}, worldrecord: {self.worldrecord}, worldrecorddb: {self.worldrecorddb}, deltaworldrecord: {deltaworldrecord}, deltalocalrecord: {deltalocalrecord}")
            updates = [
                (self.driver_txt.setText, (drivername,)),
                (self.worldrecord_txt.setText, (self.format_lap_time(self.trackworldrecord) if self.trackworldrecord != -1 else "Mod Car/Track",)),
                (self.localrecord_txt.setText, (f"{self.format_lap_time(self.worldrecorddb)} - {self.recorddriver}" if self.worldrecorddb != 0 else "No Record",)),
                (self.deltaworldrecord_text.setText, (self.format_lap_time(self.deltaworldrecord),)),
                (self.deltalocalrecord_text.setText, (self.format_lap_time(self.deltalocalrecord),)),
    ...
    def set_monitored_ip(self, ip_index):
        if 0 <= ip_index < len(self.available_ips):
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} set_monitored_ip: {self.current_ip}")
    ...
    def on_first_live_view_run(self, data):
        ...
        self.classrace = not self.identical_cars(data)
        print (f"{'RMA Line '} {inspect.currentframe().f_lineno} {'classrace starting' if self.classrace else 'Single Race Starting'}")
        ...
        self.driver_sims = {}
        self.drivertrackrecorddb = {}  # Reset track record tracking for each update
        self.drivertrackrecord = {}  # Reset driver track record tracking for each update
        self.trackworldrecord = data['timings']['mWorldFastestLapTime']  # Get the world record time for the track
        if self.trackworldrecord == -1:  # If world record is not available, set it to 0
            trackworldrecord = 0
        else:
            trackworldrecord = self.format_lap_time(self.trackworldrecord)
        self.selected_track = f"{self.event_info['mTranslatedTrackVariation']} - {self.event_info['mTranslatedTrackLocation']}"
        selected_car = data['vehicleInformation']['mCarName'] # Get the viewed participant's car name
        ...
        self.world_record_label.setText(f"")  # Update the world record label
        ...
        self.session_id_updated.emit(self.session_id)
        self.task_queue.put(('load_highscores', (selected_car, self.selected_track)))
        ...
        if not self.identical_cars(data):
            car = "class or multi class race"
        else:
            car = participants[0]['mCarNames'] # Get the car name of the first participant
        heading_text = (
        f"Session Type: {self.session_type} - Track: {self.event_info['mTranslatedTrackLocation']} - {self.event_info['mTranslatedTrackVariation']} "
        #f"({'Qualify' if sessionstate == 3 else 'Practice' if sessionstate == 1 else 'Laps',  self.event_info['mLapsInEvent']}) <br>"
        f"({('Qualify' if sessionstate == 3 else 'Practice' if sessionstate == 1 else str(self.event_info["mLapsInEvent"]) + ' Laps')})<br>"
        f"Car: {car} - {len(participants)} Drivers<br>"
        f"Session ID: {self.session_id} - World record: {trackworldrecord}"
        )
        self.live_heading.setText(heading_text)
        self.scan_active_computers()
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            if participant['mLastLapTimes'] != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if self.classrace:
                        self.world_record_label.setText(f"Records may not be displayed here in class races!")
                        carname = participant['mCarNames']
                        if not self.waiting_for_highscores:
                            if carname in self.drivertrackrecorddb:
                                if self.lap_time[name] < self.drivertrackrecorddb[carname]['worldrecord']:
                                    self.world_record_label.setText(f"New Local Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                                    print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , LocalTrack Record: {self.trackworldrecord}")
                                    self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                            
                        if carname in self.drivertrackrecord:
                            if self.lap_time[name] < self.drivertrackrecord[carname, 'worldrecord']:
                                self.world_record_label.setText(f"New World Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                                print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , World Track Record: {self.drivertrackrecord[carname, 'worldrecord']}")
                                self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                    #self.world_record_label.setText(f"Unavailable in Class Race!")
                    else:
                        if self.lap_time[name] < self.trackworldrecord :
                            self.world_record_label.setText(f"New World Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                            print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , World Track Record: {self.trackworldrecord}")
                            self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                        #else:
                        #    if self.best_lap_time < self.trackworldrecord: self.world_record_label.setText(f"New World Record: {self.format_lap_time(self.best_lap_time)}")
                        if not self.waiting_for_highscores:
                            if  self.lap_time[name] < self.trackrecorddb:
                                print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , Local Record: {self.trackrecorddb}")
                                self.world_record_label.setText(f"New Local Record: {name} - {self.format_lap_time(self.lap_time[name])}")
                                self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                            #else:
                            #    if self.best_lap_time < getattr(self, 'liverecorddb', self.trackrecorddb): self.world_record_label.setText(f"New local Record: {self.format_lap_time(self.best_lap_time)}")
    ...
    def identical_cars(self, data):
        ...
        if not participants:
            return False
        first_car = participants[0]['mCarNames']
        return all(p['mCarNames'] == first_car for p in participants)

    def update_heading(self, new_car, worldrecord, localrecord):
        print(f"RMA Line {inspect.currentframe().f_lineno} update_heading called with new_car: {new_car}, worldrecord: {worldrecord}, localrecord: {localrecord}")
        current_text = self.live_heading.text()
        if "Local Record: " in current_text:
            local_record_start = current_text.index("Local Record: ") + 14
            if localrecord == 0:
                localrecord = "First Race"
            else:
                localrecord = self.format_lap_time(localrecord)
            self.live_heading.setText(current_text[:local_record_start] + localrecord)
            current_text = self.live_heading.text()
        else:
            self.live_heading.setText(f"{current_text} - Local Record: {self.format_lap_time(localrecord) if localrecord != 0 else ' First Race'}")
            current_text = self.live_heading.text()
        if new_car != "skip":
            if "Car: " in current_text:
                car_start = current_text.index("Car: ") + 5
                dash_index = current_text.index(" -", car_start)
                self.live_heading.setText(current_text[:car_start] + new_car + current_text[dash_index:])
                current_text = self.live_heading.text()
            if "World record: " in current_text:
                record_start = current_text.index("World record: ") + 14
                if worldrecord == 0:
                    worldrecord = "Mod Car/Track"
                else:
                    worldrecord = self.format_lap_time(worldrecord)
                dash_index = current_text.index(" -", record_start)
                self.live_heading.setText(current_text[:record_start] + worldrecord + current_text[dash_index:])
    ...
    def update_best_lap_and_speed(self, participant, sessionstate):
        #if self.driver_finished.get(participant['mName'], False):
        #    return
        name = participant['mName']
        updated = False
        ...
        if updated:
            ...
            self.live_best_label.setText(
                ...
                + (f"Best Lap: {self.best_lap_driver} {self.format_lap_time(self.best_lap_time)}" if self.best_lap_time != 999 else "Best Lap: ")
            )

            #self.live_best_label.setText(New_txt)
            #print(f"RMA Line {inspect.currentframe().f_lineno} Best Lap: {New_txt}")
    ...
    def calculate_score(self, races, participants, laps):
        ...
        if races: # Iterate over each race
            ...
            if self.latest_session_id is None:
                print(f"RMA Line {inspect.currentframe().f_lineno} Races is empty, and self.latest_session_id None: {self.latest_session_id} DB is empty, set session_id to 1")                
                self.session_id = 1
            else:
                print(f"RMA Line {inspect.currentframe().f_lineno} Races is empty, but self.latest_session_id is not None: {self.latest_session_id} No Session Changes: {self.session_id}")   
            self.session_id_updated.emit(self.session_id)
            ...
            f"{(self.latest_session_id if self.session_id - 1 == self.latest_session_id else self.session_id)}</span> "
    ...
    def start_new_session(self):
        ...
        self.task_queue.put(('get_latest_session_id', (handle_latest_session_id_wrapper,)))
        ...
        if self.session_id - self.latest_session_id > 1:
            ...
            self.task_queue.put(('load_score_data', (self.session_id if self.latest_session_id == self.session_id else self.session_id - 1,)))  # Get the score data for the current session ID
    ...
    def previous_session(self):
        ...
        self.task_queue.put(('load_score_data',(self.session_id,))) # Get the score data for the current session ID
        self.live_heading.setText(f"Waiting for Race Start. Current Session: {self.session_id}")  # Update the status label  
...
class RaceApp(QThread):
    #gui_live_update = pyqtSignal(object)  # Signal to update the GUI with
    #gui_driver_update = pyqtSignal(object)  # Signal to update the GUI with
    ...
    def __init__(self, monitor_thread, task_queue, config_manager, db_thread, control_panel, parent=None, tab_widget=None, race_monitor_app=None):
        ...
        self.task_queue = task_queue
        self.running = True
        #self.race_monitor_app = racemonitor_app
        self.race_monitor_app = race_monitor_app
        ...
        logging.info("RaceApp __init__ called")
        print("RaceApp __init__ called")
        self.monitor_thread.data_updated.connect(self.update_live_view)  # Connect the data updated signal
        self.monitor_thread.data_updated_newclass.connect(self.update_driver_view)
        #self.race_monitor_app.update_track_dbrecord.connect(self.update_track_dbrecord)
        self.db_thread.highscore_data_loaded_signal.connect(self.on_highscore_loaded)  # queue.put already directs this to the correct function
        self.db_thread.load_sessionid_on_start_signal.connect(self.handle_sessionid_data) # Get data to populate session ID dropdown
    ...
    def handle_sessionid_data(self, sessionids):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} RaceApp Def handle sessionid data entered")
        self.sessionids = sessionids
    ...
    '''
    def update_track_dbrecord(self, track_dbrecord, record_driver):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def update_track_dbrecord entered")
        print(f"RMA Line {inspect.currentframe().f_lineno} update_track_dbrecord entered")
        self.trackrecorddb = track_dbrecord
        self.recorddriver = record_driver  # Store the record driver
        self.waiting_for_highscores = False  # Reset the flag after receiving the track record data
    '''
    def on_highscore_loaded(self, high_scores):
        logging.info(f"RA Line {inspect.currentframe().f_lineno} Def RaceApp on_highscore_loaded entered")
        print(f"RA Line {inspect.currentframe().f_lineno} Def RaceAPP on_highscore_loaded entered")
    ...
    def handle_latest_session_id(self, session_id):
    ...
    def on_first_live_view_run(self, data):
        ...
        heading_text = (
            ...
            f"Track: {self.event_info['mTranslatedTrackLocation']} - "
            f"{self.event_info['mTranslatedTrackVariation']} "
            f"({session_label})<br>"
            f"Car: {car} - {len(participants)} Drivers<br>"
            f"Session ID: {self.session_id} - World record: {trackworldrecord}"
        )
    ...
    def update_live_view(self, data):
        ...
        if not participants:
            return
        if self.migration == 0:  # Disable for now, until migration to new class system is done
            self.live_first_time_run = False
            return # For testing purposes, return early to avoid processing live data
        sorted_participants = sorted(
            participants[:data['participants']['mNumParticipants']],
            key=lambda p: (float('inf') if self.driver_flags.get(p['mName']) == 'Falsestart' else p['mRacePosition'])
        )
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            if participant['mLastLapTimes'] != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if self.classrace:
                        self.gui_update.emit([(self.gui.world_record_label.setText,("Records may not be displayed here in class races!",))])
                        carname = participant['mCarNames']
                        if not self.waiting_for_highscores:
                            if carname in self.drivertrackrecorddb:
                                if self.lap_time[name] < self.drivertrackrecorddb[carname]['worldrecord']:
                                    self.gui_update.emit([(self.gui.world_record_label.setText,(f"New Local Record: {name} - {self.format_lap_time(self.lap_time[name])}",))])
                                    print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , LocalTrack Record: {self.trackworldrecord}")
                                    self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                            
                        if carname in self.drivertrackrecord:
                            if self.lap_time[name] < self.drivertrackrecord[carname, 'worldrecord']:
                                self.gui_update.emit([(self.gui.world_record_label.setText,(f"New World Record: {name} - {self.format_lap_time(self.lap_time[name])}",))])
                                print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , World Track Record: {self.drivertrackrecord[carname, 'worldrecord']}")
                                self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                    #self.world_record_label.setText(f"Unavailable in Class Race!")
                    else:
                        if self.lap_time[name] < self.trackworldrecord :
                            self.gui_update([(self.gui.world_record_label.setText,(f"New World Record: {name} - {self.format_lap_time(self.lap_time[name])}",))])
                            print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , World Track Record: {self.trackworldrecord}")
                            self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                        #else:
                        #    if self.best_lap_time < self.trackworldrecord: self.world_record_label.setText(f"New World Record: {self.format_lap_time(self.best_lap_time)}")
        ...
        # --------------- hopp over hvis Live-fanen er skjult ----------
        if self.active_tab != 0 and not self.live_first_time_run:
            self.gui_update.emit(updates)          # send ev. bakgrunns­endringer
            return                                       # ingen further GUI-data i denne runden
    ...
    def update_heading(self, new_car: str,
        ...
        # --- hent eksisterende stam-tekst før vi legger til feltene våre -----------
        base_text = self.gui.live_heading.text().split(" - Local Record:", 1)[0].strip()

        # --- formater tallverdier ---------------------------------------------------
        local_str  = "First Race"        if localrecord  == 0 else self.format_lap_time(localrecord)
        world_str  = "Mod Car/Track"     if worldrecord == 0 else self.format_lap_time(worldrecord)

        # hent eksisterende bil hvis vi skal «skippe» oppdatering av bilnavn
        if new_car == "skip":
            try:
                new_car = base_text.split("Car:")[1].split(" -")[0].strip()
            except (IndexError, ValueError):
                new_car = "Unknown"

        # --- bygg ny heading --------------------------------------------------------
        heading = (
            f"{base_text} - Local Record: {local_str} "
            f"- Car: {new_car} - World record: {world_str}"
        )

        # --- send til GUI i ett, trådsikkert kall -----------------------------------
        self.gui_update.emit([
            (self.gui.live_heading.setText, (heading,))
        ])

    def update_heading_old(self, new_car, worldrecord, localrecord):
        print(f"RA Line {inspect.currentframe().f_lineno} update_heading called with new_car: {new_car}, worldrecord: {worldrecord}, localrecord: {localrecord}")
        current_text = self.live_heading.text()
        if "Local Record: " in current_text:
            local_record_start = current_text.index("Local Record: ") + 14
            if localrecord == 0:
    ...
    def update_best_lap_and_speed_old(self, participant, sessionstate):
        #if self.driver_finished.get(participant['mName'], False):
        #    return
        name = participant['mName']
        updated = False
    ...
    def on_first_live_view_run_old(self, data):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def on_first_live_view_run entered")
        self.event_info = data['eventInformation'] # Extract event information and participant details from the data
        participants = data['participants']['mParticipantInfo']
        sessionstate = data['gameStates']['mSessionState']
        ...
        self.trackworldrecord = data['timings']['mWorldFastestLapTime']  # Get the world record time for the track
        self.driver_view_first_update = True
        self.live_first_time_run = False  # Reset the flag after the first run fjernes senere
        self.waiting_for_highscores = True  # Set the flag to indicate we are waiting for high scores
        self.driver_view_first_update = True
        self.current_lap = {}
        self.previous_current_lap = {}
        ...
        self.race_started = False
        self.waiting_for_highscores = True  # Set the flag to indicate we are waiting for high scores
        #for i in range(20): self.participant_labels[i].hide()  
        # Hide labels initially  # Assuming 20 participants max
        # Find out which sim the particpant is using and write the it after the name
        # Check participants in file drivers.txt, and match participant['mName'] with the name in the file.
        # This will help to identify which sim the participant is using.
        # Sim is the names position in the file.
    ...
    def update_live_view_old(self, data):
        if self.live_first_time_run:
            self.on_first_live_view_run(data)
    ...
    def update_driver_view(self, data):
        # Optimize needle rotations with precomputed painter
        #logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def update_driver_view entered")
        #print(f"RMA Line {inspect.currentframe().f_lineno} Def update_driver_view entered")
        if self.active_tab != 5 or not data or 'participants' not in data or 'mParticipantInfo' not in data['participants'] or hasattr(self, 'driver_view_first_update') == False:
            #print(f"RA Line {inspect.currentframe().f_lineno} active_tab: {self.active_tab}, driver_view_first_update: {hasattr(self, 'driver_view_first_update')}")
            return  # Early exit for inactive tab or invalid data or if driver_view_first_update is not set
            

        if self.driver_view_first_update:
            # Will be moved to live view first run later
              # Reset the flag after receiving the track record data
        ...
        if self.classrace:
            if carname != self.previous_carname:  # Check if the car has changed
                if carname not in self.drivertrackrecord:
                    self.drivertrackrecord[carname] = {'worldrecord': self.trackworldrecord}
                self.trackworldrecord = data['timings']['mWorldFastestLapTime']
                print(f"RA Line {inspect.currentframe().f_lineno} Classrace, new car detected: {carname} - Old Car: {self.previous_carname}")
                if carname not in self.drivertrackrecorddb and not self.waiting_for_highscores:
                    self.task_queue.put(('load_highscores', (carname, self.selected_track)))
                    self.waiting_for_highscores = True
                    self.local_record_updated = False
                    ...
                    self.recorddriver = self.drivertrackrecorddb[carname]['recorddriver']
                    self.update_heading(carname, self.trackworldrecord, self.worldrecorddb)
                    print(f"RA Line {inspect.currentframe().f_lineno} Classrace, worldrecorddb for driver: {drivername}, car: {carname} already exists: {self.worldrecorddb}")

            if not self.local_record_updated and not self.waiting_for_highscores:
                    print(f"RA Line {inspect.currentframe().f_lineno} Classrace, worldrecddb update for driver: {drivername}, car: {carname}")
                    self.worldrecorddb = self.trackrecorddb
                    self.drivertrackrecorddb[carname] = {'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
                    self.update_heading(carname, self.trackworldrecord, self.worldrecorddb) 
                    self.local_record_updated = True
        ...
        formatted_worldrecorddb = f"{self.format_lap_time(self.worldrecorddb)} - {self.recorddriver}" if self.worldrecorddb != 0 else "No Record"
        ...
        if  not self.waiting_for_highscores:
            ...
            updates = [
                ...
                (self.gui.collison_txt.setText, (str(crashindex),)),
                ...
                (self.gui.throttle_bar.setValue, (throttle,)),
                (self.gui.brake_bar.setValue, (brake,)),
                (self.gui.clutch_bar.setValue, (clutch,)),
                (self.gui.throttle_raw_bar.setValue, (throttle_raw,)),
                (self.gui.brake_raw_bar.setValue, (brake_raw,)),
                (self.gui.clutch_raw_bar.setValue, (clutch_raw,)),
            ...
            #    setter(*args)
    '''
    def rotate_needle(self, value, max_value, needle_pixmap, start_angle=-135, end_angle=135):
        value = max(0, min(value, max_value))
        sweep = end_angle - start_angle
        angle = (value / max_value) * sweep + start_angle
        size = needle_pixmap.size()
    ...
    def rotate_steering_wheel(self, value):
        angle = value * 360  # Assuming value in range [-1, 1]
        pixmap = self.wheel_image
        rotated_pixmap = QPixmap(pixmap.size())
        rotated_pixmap.fill(Qt.transparent)
        painter = QPainter(rotated_pixmap)
