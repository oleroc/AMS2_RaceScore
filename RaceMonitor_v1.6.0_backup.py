...
class DatabaseThread(QThread):
    ...
    highscore_data_loaded_signal = pyqtSignal(object, object)
    ...
    def load_highscores(self, car, track):
        ...
        try:
            ...
            print(f"DB Line {inspect.currentframe().f_lineno} Best laps calculated: {best_laps}")
            self.cursor.execute('''
                SELECT Name, lapnumber, BestLap, mTrackvariation, MCarName
                FROM HighScore
            ...
            if high_scores:
                ...
                self.highscore_data_loaded_signal.emit(high_scores, best_laps)
            else:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} No High Scores found, emitting None")
                self.highscore_data_loaded_signal.emit(None, None)  # Emit None if no high scores found
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} operation failed: {e}")
            self.highscore_data_loaded_signal.emit(None, None)  # Emit None if an error occurs
    ...
    async def race_running(self, data):
        ...
        if not driver_total_times:
            ...
            try:                
                for participant in participants:
                    participant_name = participant['mName']
                    print (f"MT Line {inspect.currentframe().f_lineno} Participant: {participant}")
...
class GuiMainApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.button_column_layout.addWidget(self.track_dropdown, alignment=Qt.AlignTop)
        ...
        self.button_column_layout.addWidget(self.car_dropdown, alignment=Qt.AlignTop)
        ...
        self.button_column_layout.addWidget(self.driver_dropdown, alignment=Qt.AlignTop)
        ...
        self.button_column_layout.addWidget(self.dropdown_races, alignment=Qt.AlignTop)
        ...
        self.button_column_layout.addWidget(self.dropdown_sessionid, alignment=Qt.AlignTop)
    ...
    def check_enable_disable_cp_button(self):
        ...
        if self.config_manager.cp_enabled("check"):
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} Control Panel writing is enabled.")
        else:
            self.Enable_CP.setText("Enable CP")
            print(f"RMA Line {inspect.currentframe().f_lineno} Control Panel writing is disabled.")
    ...
    def on_highscore_loaded(self, high_scores, notinuse):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def on highscore loaded entered")
        #print(f"RMA Line {inspect.currentframe().f_lineno} Def on highscore loaded entered", {'high_scores': high_scores})
...
class RaceApp(QThread):
    ...
    def handle_multi_data(self, data):
        ...
        if not self.driverworldrecord:
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} Def handle_multi_data: Driver World Records updated: {self.driverworldrecord}")
    ...
    def update_driver_names(self, ip_name_flag):
        ...
        print(f"RMA Line {inspect.currentframe().f_lineno} Def update_driver_names entered with ip_name_flag: {ip_name_flag}")
        """
    ...
    def on_highscore_loaded(self, high_scores, best_laps):
        ...
        print(f"RMA Line {inspect.currentframe().f_lineno} Best Scores: {best_laps}")
        self.localrecordlaps = best_laps  # Store the best laps for each driver
        if  self.localrecordlaps != None:
            #self.recordcar = min(high_scores, key=lambda x: x[2])[2]
            #self.recorddriver, _, self.recordcar, _, _ = min(best_laps, key=lambda x: x[2])
            if self.classrace:
                self.recordcar = 0
                ...
                self.recordcar =  self.localrecordlaps[self.selected_car]['worldrecord']
            
            
            print(f"RMA Line {inspect.currentframe().f_lineno} Track Record: {self.recordcar} by {self.recorddriver}")
        else:
            print(f"RMA Line {inspect.currentframe().f_lineno} No High Scores found in DB")            
            self.recordcar = 0
            self.recorddriver = ""

        self.update_heading("skip", self.trackworldrecord, self.recordcar)
    ...
    def on_first_live_view_run(self, data):
        ...
        self.session_id_updated.emit(self.session_id)
        self.task_queue.put(('load_highscores', (self.selected_car, self.selected_track)))
    ...
    def update_live_view(self, data):
        if self.live_first_time_run and hasattr(self, 'live_first_time_run_done') == False:
             self.on_first_live_view_run(data)
        if self.waiting_for_highscores:
            print(f"RMA Line {inspect.currentframe().f_lineno} Waiting for high scores to be loaded, skipping live view update.")
            return
        #self.waiting = False
            ...
            if participant['mLastLapTimes'] != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if carname in self.localrecordlaps:
                        #print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record: {self.localrecordlaps}")
                        print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , WorldTrack Record: {self.trackworldrecord}, Local Record: {self.localrecordlaps[carname]['worldrecord']}")
                        ...
                        print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record not found for car: {carname}, First entry!")
                        #self.gui_update.emit([(self.gui.world_record_label.setText,(f"Brand New Local Record: {name} - {self.format_lap_time(self.lap_time[name])}"))])
                        self.localrecordlaps[carname] = {'worldrecord': self.lap_time[name], 'recorddriver': name}
                        self.update_heading(carname, self.trackworldrecord, self.lap_time[name])
                        self.gui_update.emit([(self.gui.flash_timer.start, (500,))])
                    if carname in self.driverworldrecord:
                        print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record: {self.driverworldrecord}")
                        if self.lap_time[name] < self.driverworldrecord[carname]['worldrecord']:
                            #self.gui_update.emit([(self.gui.world_record_label.setText,(f"Laptime: {self.lap_time[name]} New World Record: {name} - {self.format_lap_time(self.lap_time[name])}"))])
                            print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , World Track Record: {self.driverworldrecord[carname]['worldrecord']}")
    ...
    def update_driver_view(self, data):
        ...
        if self.driver_view_first_update:
            ...
            #self.driverworldrecord[carname] = {'worldrecord': self.trackworldrecord}
            self.previous_carname = data['vehicleInformation']['mCarName']

            if not self.waiting_for_highscores:
                #print(f"RMA Line {inspect.currentframe().f_lineno} Driver view first update, track record db: {self.recordcar}, track: {self.selected_track}, car: {carname}, driver: {drivername} worldrecord: {self.trackworldrecord},drivertrackrecorddb {self.localrecordlaps}")                
                #if carname not in self.localrecordlaps:
                #    self.localrecordlaps[carname] = { 'worldrecord': self.recordcar, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
                #    print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname} not found, creating new entry.")
                #self.localrecord = self.localrecordlaps[carname]['worldrecord']
                #self.recorddriver = self.localrecordlaps[carname]['recorddriver']
                #self.update_heading(carname, self.trackworldrecord, self.localrecord)
                self.driver_view_first_update = False
                return
            else:
                #print(f"RMA Line {inspect.currentframe().f_lineno} Waiting for highscores, not updating driver view yet.") 
        ...
        if carname != self.previous_carname:  # Must be a classrace.
            self.previous_carname = carname
            self.trackworldrecord = data['timings']['mWorldFastestLapTime']
            #if carname not in self.driverworldrecord:
            #    self.driverworldrecord[carname] = {'worldrecord': self.trackworldrecord}
            #    print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname} not found, creating new entry.")
            #print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, new car detected: {carname} - Old Car: {self.previous_carname}")
            if carname not in self.localrecordlaps:
                self.localrecordlaps[carname] = {'worldrecord': self.recordcar, 'recorddriver':self.recorddriver}
                print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname} not found, creating new entry.")
            self.localrecord = self.localrecordlaps[carname]['worldrecord']
            print(f"RMA Line {inspect.currentframe().f_lineno} Self.localrecord: {self.localrecord}, car: {carname} ")
            self.recorddriver = self.localrecordlaps[carname]['recorddriver']
            print(f"RMA Line {inspect.currentframe().f_lineno} Driver track record db for car: {carname}, recorddriver: {self.recorddriver}")
            self.update_heading(carname, self.trackworldrecord, self.localrecord)
            print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, worldrecorddb for driver: {drivername}, car: {carname} already exists: {self.localrecord}")
            
                    
        # Only one car all variables are set in the first run
        self.deltalocalrecord = bestlap - self.localrecord if bestlap != -1 and self.localrecord != 0 else 0
        #print (f"RMA Line {inspect.currentframe().f_lineno} Deltalocalrecord: {self.deltalocalrecord} bestlap: {bestlap}, localrecord: {self.localrecord} ")
        #if self.deltalocalrecord < 0: 
        #    print(f"RMA Line {inspect.currentframe().f_lineno} New Record: Deltalocalrecord: {self.deltalocalrecord}, worldrecorddb: {self.localrecord}, trackworldrecord: {self.trackworldrecord}")
        ...
        formatted_worldrecorddb = f"{self.format_lap_time(self.localrecord)} - {self.recorddriver}" if self.localrecord != 0 else "No Record"
