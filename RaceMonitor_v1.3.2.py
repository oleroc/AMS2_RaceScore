...
class DatabaseThread(QThread):
    ...
    def write_race(self, data, session_id):
        ...
        try:
            ...
            sys.exit(1)  # Exit the program cleanly with an error status
    ...
    def finalize_race(self, data, lap_times_dict, flags_dict):
        try:
            ...
            for driver, lap_times in lap_times_dict.items():
                driver_total_times[driver] = sum(lap_times)
            ranked_drivers = self.calculate_score(participants, lap_times_dict, flags_dict, driver_total_times)
            for participant in participants:
                ...
                ''', (
                    ...
                    participant['mLapsCompleted'],
                    flags_data,
                    driver_total_times.get(participant_name, 0),
    ...
    def calculate_score(self, participants, laps, flags, driver_total_times):
            ...
            print(f"Ranked drivers: {ranked_drivers}")
        ...
        while self.running:
            try:
                ...
                if current_game_state != self.previous_game_state:
                    ...
                    #logging.info(f"MT RMA Race state changed to {current_game_state}")
                ...
                if participants:
                   ...
                   # time.sleep (4) # Wait for 3 seconds before fetching the next data
                    ...
                    while self.running: #loop while race is running
                        try:
                            ...
                            for participant in participants:
                                # print(f"Name: {participant['mName']} Speed: {participant.get('mSpeeds',0)} RaceState:{data['gameStates']['mRaceState']}")
                            ...
                            if current_game_state != self.previous_game_state:
                                print(f"MT Game state changed to {current_game_state}")  # Ensure console output remains
                                #logging.info(f"MT Race state changed to {current_game_state}")
                                self.previous_game_state = current_game_state                            
                            #time.sleep(2)
                            current_game_state = data['gameStates']['mGameState']
                            ...
                            time.sleep(2)                            
                    logging.info(f"MT Race Race_{self.race_id} has ended. Current_game_State: {current_game_state} self.running: {self.running}")
                    self.race_loop_first_time = True
                    print(f"MT Race Ended Finalizing Race for Race: Race_{self.race_id}, Current_game_State: {current_game_state} self.running: {self.running}")
                    logging.info(f"MT Race Ended Finalizing Race for Race: Race_{self.race_id}, Current_game_State: {current_game_state} self.running: {self.running}")
                    participants = data.get('participants', {}).get('mParticipantInfo', [])
                    ...
                    if not best_participant:
                        print(f"MT No reason to write race, no one finished. no best_participant.")
                        logging.info(f"MT No reason to write race, no one finished, no best_participant.")
                        self.first_time_run = True
                        self.race_finished.emit()
                        break   # no reason to write the race, no one finished
                    ...
                    for participant in participants:
                        ...
                        #print(f"MT Participant: {participant['mName']}, Race Position: {participant.get('mRacePosition', 0)}, Total Laps: {total_laps} Total Time: {total_time_participant[participant['mName']]}")
                        ...
                        if participant['mName'] == winner and current_lap < total_laps:
                            ...
                            logging.info(f"MT Driver is {participant_name} same as {winner} and did not finish race. That means False start!")
...
class RaceMonitorApp(QMainWindow):
    ...
    def on_first_live_view_run(self, data):
        ...
        event_info = data['eventInformation'] # Extract event information and participant details from the data
        participants = data['participants']['mParticipantInfo']
        ...
        self.driver_flags = {}
        heading_text = f"{event_info['mTranslatedTrackLocation']} - {event_info['mTranslatedTrackVariation']} ({event_info['mLapsInEvent']}) - {participants[0]['mCarNames']} -  {len(participants)} Drivers - Session ID: {self.session_id}  " # Update the heading with track variation and car names
    ...
    def update_live_view(self, data):
        ...
        #print(f" Flags: {self.driver_flags}")
    ...
    def display_score(self):
        ...
        print(f"Accumulated Score for Session {self.session_id_dropdown}")
    ...
    def calculate_score(self, races, participants, laps):
        ...
        print(f"Races: {races} ")
        # print(f"Ranked drivers: {self.ranked_drivers}")
    ...
    def stop(self):
        # Log the stopping sequence initiation
        logging.info("Stopping RaceMonitorApp...")

        # Stop the database thread if it exists and is running
        if hasattr(self, 'databaseThread') and self.databaseThread.isRunning():
            logging.info("Stopping DatabaseThread...")
            self.databaseThread.stop()
            self.databaseThread.wait()  # Wait for the thread to finish

        # Stop the monitor thread if it exists and is running
        if hasattr(self, 'monitorThread') and self.monitorThread.isRunning():
            logging.info("Stopping MonitorThread...")
            self.monitorThread.stop()
            self.monitorThread.wait()  # Wait for the thread to finish

        # Additional cleanup could go here if there are more threads or resources to manage
        # For instance, if there are any open files, network connections, or other resources:
        # self.someConnection.close()
        # self.someFile.close()

        # If using a GUI and it needs to be closed explicitly
        self.close()  # Assuming this is a QMainWindow or similar

        # Log completion of stopping sequence
        logging.info("RaceMonitorApp stopped successfully.")
