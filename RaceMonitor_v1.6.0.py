    ...
    def load_selected_race(self, race_id, callback = None):
        try:
            ...
            if race:
                ...
                self.cursor.execute('''
                    SELECT mName, mRacePosition, CalculatedPosition, mFastestLapTimes, mLastLapTimes, mCarNames, flags, PitStops
                    FROM Participants
                    WHERE RaceID = ?
...
class GuiMainApp(QMainWindow):
    ...
    def on_race_loaded(self, race, participants, laps):
        ...
        if race_id:
            ...
            car_name = participants[0][5] if participants and participants[0][5] else "Unknown Car"
            ...
            best_lap_participant = min(participants, key=lambda p: p[3] if p[3] not in (None, 0) else float('inf'))
            print(f"GA Line {inspect.currentframe().f_lineno} Best Lap Participant: {best_lap_participant[0]} with time: {best_lap_participant[3]}")

            place = 1
            for participant in participants:
                name, race_position, calculated_position, best_lap, last_lap, car, flags, pits = participant
                ...
                #spaces = (f"<br> {'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' if int(calculated_position) > 9 else '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'}")
                flags = flags or ''
                pits = '' if pits == 0 else pits
                loaded_places += (f"<span style='color:{self.color_labels};'>{calculated_position}</span><br>")
