...
class MonitorThread(QThread):
    ...
    async def get_participants_ips(self, participants):
        ...
        if testmode_enabled:
            ...
            # Original live mode logic (unchanged)
            ...
            for sim_index, (ip, data) in enumerate(results):
                ...
                if data is not None:
                    ...
                    if 0 <= viewed_index < len(infos):
                        ...
                        if fetched_name:
                            ...
                            if fetched_name in participant_names:
                                in_race = 1
                                print(f"MT Line {inspect.currentframe().f_lineno} Live mode: {ip} -> {driver_name} (in race)")
                            else:
                    ...
                    if hasattr(self, 'active_races') and ip in self.active_races:
                        race_info = self.active_races[ip]
                        race_group_ips = race_info.get('race_group_ips', [ip])
                        try:
                            ip_index = race_group_ips.index(ip)
                            race_group_letter = chr(65 + (ip_index // 4))
