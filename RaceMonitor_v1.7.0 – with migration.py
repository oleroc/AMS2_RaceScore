...
class DatabaseThread(QThread):
    ...
    def create_database(self):
        # Check current database version and perform migrations
    ...
    def check_and_migrate_database(self):
        ...
        try:
            ...
            if not metadata_exists:
                # This is likely a pre-1.7.0 database or new database
                current_version = self.detect_legacy_database_version()
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Legacy database detected, version: {current_version}")
            else:
            ...
            if self.version_compare(current_version, '1.7.0') < 0:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Upgrading database from {current_version} to 1.7.0")
                self.migrate_to_v170(current_version)
                
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Database migration check failed: {e}")
            print(f"DB Line {inspect.currentframe().f_lineno} Database migration check failed: {e}")
    ...
    def migrate_to_v170(self, from_version):
        """Migrate database to version 1.7.0 for multi-race monitoring"""
        try:
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Starting migration to v1.7.0 from {from_version}")
            
            # Skip confirmation dialog for now - GUI dialogs don't work well from worker threads
            # In a production environment, you'd want to signal the main thread for confirmation
            print(f"\n🔄 Database Migration Required")
            print(f"   Upgrading from v{from_version} to v1.7.0")
            print(f"   This will add multi-race monitoring capabilities")
            print(f"   All existing data will be preserved")
            print(f"   Proceeding with automatic migration...")
            ...
            # Create DatabaseMetadata table if it doesn't exist
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS DatabaseMetadata (
                    Key TEXT PRIMARY KEY,
                    Value TEXT,
            ...
            # Check if we need to recreate tables due to foreign key changes
            needs_table_recreation = self.check_foreign_key_compatibility()
            
            if needs_table_recreation:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Foreign key changes detected, recreating tables")
                self.recreate_tables_with_new_schema()
            else:
                # Simple migration - just add columns and new tables
                ...
                # Create SessionParticipants table for 50% matching
                self.cursor.execute('''
                    CREATE TABLE IF NOT EXISTS SessionParticipants (
                        SessionID INTEGER,
                        ParticipantName TEXT,
                ...
                # Add foreign key to Races table if missing
            ...
            # Post-migration verification and fixes
            print("\n🔧 Post-Migration Verification & Repairs")
            print("=" * 50)
            self.verify_and_fix_database_schema()
            
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Successfully migrated database to v1.7.0")
            print(f"\n✅ Database successfully upgraded to v1.7.0 for multi-race monitoring")
            return True
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Migration to v1.7.0 failed: {e}")
    ...
    def add_column_if_not_exists(self, table_name, column_name, column_definition):
        ...
        try:
            ...
            if column_name not in columns:
                ...
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Added column {column_name} to table {table_name}")
                
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to add column {column_name} to {table_name}: {e}")
    ...
    def migrate_existing_sessions(self):
        ...
        try:
            ...
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Migrated {len(existing_sessions)} existing sessions")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to migrate existing sessions: {e}")
    ...
    def analyze_database_schema(self):
        ...
        try:
            ...
            # Check for v1.7.0 specific tables
            ...
            if missing_tables:
                print(f"   ⚠️  Missing v1.7.0 tables: {', '.join(missing_tables)}")
            else:
                print("   ✅ All v1.7.0 tables present")
            ...
            if 'Races' in tables:
                ...
                if missing_columns:
                    print(f"   ⚠️  Missing Races columns: {', '.join(missing_columns)}")
                else:
                    print("   ✅ Races table has v1.7.0 columns")
    ...
    def verify_and_fix_database_schema(self):
        """Verify database schema and fix any issues"""
        try:
            print("🔧 Verifying and fixing database schema...")
            
            # Check and fix foreign keys
            missing_fks = self.check_missing_foreign_keys()
            if missing_fks:
                print(f"   ⚠️  Found {len(missing_fks)} missing foreign key constraints")
                for table, constraint_info in missing_fks.items():
                    print(f"      {table}: {constraint_info}")
                
                # Attempt to fix foreign keys
                self.fix_missing_foreign_keys(missing_fks)
            else:
                print("   ✅ All foreign key constraints verified")
            
            # Verify indexes
            self.verify_performance_indexes()
            ...
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Schema verification failed: {e}")

    def check_missing_foreign_keys(self):
        """Check for missing foreign key constraints"""
        missing_fks = {}
        
        try:
            # Define expected foreign keys for v1.7.0
            expected_fks = {
                'Races': [
                    ('SessionID', 'Sessions', 'SessionID', 'ON DELETE SET NULL')
                ],
                'SessionParticipants': [
                    ('SessionID', 'Sessions', 'SessionID', 'ON DELETE CASCADE')
                ],
                'Participants': [
                    ('RaceID', 'Races', 'RaceID', 'ON DELETE CASCADE')
                ],
                'Laps': [
                    ('RaceID', 'Races', 'RaceID', 'ON DELETE CASCADE'),
                    ('RaceID', 'Participants', 'RaceID', 'ON DELETE CASCADE'),
                    ('mName', 'Participants', 'mName', 'ON DELETE CASCADE')
                ],
                'HighScore': [
                    ('phone', 'Drivers', 'Phone', 'ON DELETE CASCADE'),
                    ('Name', 'Drivers', 'Name', 'ON DELETE NO ACTION')
                ]
            }
            
            for table, expected_constraints in expected_fks.items():
                ...
                current_fks = self.cursor.fetchall()
                
                # Convert to comparable format
                current_fk_set = set()
                for fk in current_fks:
                    current_fk_set.add((fk[3], fk[2], fk[4]))  # (from_column, to_table, to_column)
                
                # Check for missing constraints
                for constraint in expected_constraints:
                    from_col, to_table, to_col, action = constraint
                    if (from_col, to_table, to_col) not in current_fk_set:
                        if table not in missing_fks:
                            missing_fks[table] = []
                        missing_fks[table].append(constraint)
            
            return missing_fks
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to check foreign keys: {e}")
            return {}

    def fix_missing_foreign_keys(self, missing_fks):
        """Attempt to fix missing foreign key constraints"""
        try:
            print("   🔧 Attempting to fix missing foreign key constraints...")
            
            for table, constraints in missing_fks.items():
                print(f"      Fixing {table}...")
                
                # This is complex in SQLite - we need to recreate the table
                # For now, we'll log the issue and suggest manual intervention
                for constraint in constraints:
                    from_col, to_table, to_col, action = constraint
                    constraint_desc = f"{from_col} → {to_table}({to_col}) {action}"
                    print(f"        Missing: {constraint_desc}")
                
                # For critical foreign keys, we can attempt recreation
                if table == 'Races' and any(c[1] == 'Sessions' for c in constraints):
                    self.fix_races_session_foreign_key()
            
        except Exception as e:
            print(f"   ❌ Foreign key fix failed: {e}")
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Foreign key fix failed: {e}")

    def fix_races_session_foreign_key(self):
        """Fix the critical Races → Sessions foreign key constraint"""
        try:
            print("      🔧 Fixing Races → Sessions foreign key...")
            
            # Check if Sessions table exists
            self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='Sessions'")
            if not self.cursor.fetchone():
                print("         Sessions table not found, skipping foreign key fix")
                return
            
            # Check current foreign keys on Races table
            self.cursor.execute("PRAGMA foreign_key_list(Races)")
            current_fks = self.cursor.fetchall()
            
            # Check if Races → Sessions FK already exists
            sessions_fk_exists = any(fk[2] == 'Sessions' and fk[3] == 'SessionID' for fk in current_fks)
            
            if sessions_fk_exists:
                print("         Races → Sessions foreign key already exists")
                return
            
            # Disable foreign keys temporarily
            self.cursor.execute("PRAGMA foreign_keys = OFF")
            
            # Get current Races table structure
            self.cursor.execute("PRAGMA table_info(Races)")
            ...
            self.cursor.execute("DROP TABLE Races")
            self.cursor.execute("ALTER TABLE Races_new RENAME TO Races")
            
            # Re-enable foreign keys
            self.cursor.execute("PRAGMA foreign_keys = ON")
            
            # Recreate indexes
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_races_session ON Races(SessionID)")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_races_date ON Races(RaceDate)")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_races_completed ON Races(IsCompleted)")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_races_ip ON Races(IPAddress)")
            
            self.conn.commit()
            print("         ✅ Races → Sessions foreign key fixed successfully")
            
        except Exception as e:
            print(f"         ❌ Failed to fix Races → Sessions foreign key: {e}")
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to fix Races → Sessions FK: {e}")
    ...
    def verify_performance_indexes(self):
        """Verify that all performance indexes are created"""
        try:
            print("   📈 Verifying performance indexes...")
            
            # Get current indexes
            self.cursor.execute("SELECT name FROM sqlite_master WHERE type='index' AND name LIKE 'idx_%'")
            current_indexes = [row[0] for row in self.cursor.fetchall()]
            
            expected_indexes = [
                'idx_races_session', 'idx_races_date', 'idx_races_completed', 'idx_races_ip',
                'idx_sessions_completed', 'idx_sessions_created', 'idx_session_participants',
                'idx_participants_race', 'idx_laps_race', 'idx_laps_name'
            ]
            
            missing_indexes = [idx for idx in expected_indexes if idx not in current_indexes]
            ...
            if missing_indexes:
                ...
                print(f"      ✅ All {len(expected_indexes)} performance indexes verified")
                
        except Exception as e:
            print(f"   ❌ Index verification failed: {e}")
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Index verification failed: {e}")
    ...
    def show_database_upgrade_confirmation(self, from_version, to_version):
        ...
        try:
            ...
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to show upgrade confirmation: {e}")
            # Fallback - assume user wants to upgrade
            print(f"⚠️  Could not show upgrade dialog, proceeding with upgrade from v{from_version} to v{to_version}")
            return True
    ...
    def check_foreign_key_compatibility(self):
        """Check if existing tables have compatible foreign key constraints"""
        try:
            # Check if Races table has proper foreign key structure
            self.cursor.execute("PRAGMA table_info(Races)")
            races_columns = {column[1]: column for column in self.cursor.fetchall()}
            
            # Check if Sessions table exists (new in v1.7.0)
            self.cursor.execute('''
                SELECT name FROM sqlite_master 
            ...
            # If Sessions table doesn't exist, we need recreation to add proper FK references
            if not sessions_exists:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Sessions table doesn't exist, table recreation needed")
                return True
                
            # Check foreign key constraints on existing tables
            self.cursor.execute("PRAGMA foreign_key_list(Races)")
            races_fks = self.cursor.fetchall()
            
            # Check if Races table has the new foreign key to Sessions
            has_sessions_fk = any(fk[2] == 'Sessions' for fk in races_fks)
            
            if not has_sessions_fk and 'SessionID' in races_columns:
                logging.info(f"DB Line {inspect.currentframe().f_lineno} Missing Sessions foreign key, table recreation needed")
                return True
                
            return False
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Error checking foreign key compatibility: {e}")
            return False

    def recreate_tables_with_new_schema(self):
        """Recreate tables with updated foreign key constraints"""
        try:
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Starting table recreation with new schema")
            
            # Step 1: Create backup tables with existing data
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS Races_backup AS 
                SELECT * FROM Races
            ''')
            ...
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS Laps_backup AS 
                SELECT * FROM Laps
            ''')
            
            # Step 2: Drop existing tables
            self.cursor.execute('DROP TABLE IF EXISTS Laps')
            self.cursor.execute('DROP TABLE IF EXISTS Participants')
            self.cursor.execute('DROP TABLE IF EXISTS Races')
            
            # Step 3: Create Sessions table first (for foreign key references)
            self.cursor.execute('''
            ...
            # Step 4: Create Races table with new schema and foreign keys
            self.cursor.execute('''
                CREATE TABLE Races (
                    RaceID INTEGER PRIMARY KEY,
                    mTranslatedTrackVariation TEXT,
            ...
            # Step 5: Create Participants table with updated foreign keys
            self.cursor.execute('''
            ...
            # Step 6: Create Laps table with updated foreign keys
            self.cursor.execute('''
                CREATE TABLE Laps (
                    LapID INTEGER PRIMARY KEY AUTOINCREMENT,
                    RaceID INTEGER,
            ...
            # Step 7: Create SessionParticipants table
            self.cursor.execute('''
            ...
            # Step 8: Migrate data back from backup tables
            self.migrate_data_from_backup()
            
            # Step 9: Drop backup tables
            self.cursor.execute('DROP TABLE IF EXISTS Races_backup')
            self.cursor.execute('DROP TABLE IF EXISTS Participants_backup')
            self.cursor.execute('DROP TABLE IF EXISTS Laps_backup')
            
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Table recreation completed successfully")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Table recreation failed: {e}")
            # Attempt to restore from backup
            self.restore_from_backup()
            raise

    def migrate_data_from_backup(self):
        """Migrate data from backup tables to new schema"""
        try:
            # First, create sessions for existing data
            ...
            # Migrate Races data with new columns
            self.cursor.execute('''
                INSERT INTO Races (
            ...
            self.cursor.execute('''
                ...
                SELECT * FROM Participants_backup
            ''')
            
            # Migrate Laps data
            self.cursor.execute('''
                INSERT INTO Laps 
                SELECT * FROM Laps_backup
            ''')
            
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Data migration from backup completed")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Data migration from backup failed: {e}")
            raise

    def restore_from_backup(self):
        """Restore original tables from backup in case of migration failure"""
        try:
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Attempting to restore from backup")
            ...
            self.cursor.execute('DROP TABLE IF EXISTS SessionParticipants')
            
            # Restore original structure
            self.cursor.execute('ALTER TABLE Races_backup RENAME TO Races')
            self.cursor.execute('ALTER TABLE Participants_backup RENAME TO Participants')
            self.cursor.execute('ALTER TABLE Laps_backup RENAME TO Laps')
            
            logging.info(f"DB Line {inspect.currentframe().f_lineno} Database restored from backup")
            
        except Exception as e:
            logging.error(f"DB Line {inspect.currentframe().f_lineno} Failed to restore from backup: {e}")
