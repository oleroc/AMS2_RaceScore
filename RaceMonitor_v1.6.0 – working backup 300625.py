    ...
    async def periodic_fetch_enriched_data(self):
        ...
        while self.race_in_session: 
            try:
                ...
                print(f"Error in periodic_fetch_enriched_data: {e}")
            await asyncio.sleep(2)          

    async def fetch_race_data_all_ips_old(self, ip_name_flag):
        #print(f"MT Line {inspect.currentframe().f_lineno} Fetching data all race IPs")
        """
        ...
        async def fetch(ip):
            ...
            try:
                ...
                print(f"Error fetching detailed race data from {ip}: {e}")
            return ip, None
    ...
    async def fetch_race_data_all_ips(self, ip_name_flag):
        ...
        async def fetch(ip):
            ...
            if failed_attempts[ip] >= 3:
                print(f"Marking {ip} as not in race after 3 failed attempts")
                    ...
                    if participant['mCurrentLap'] <= data['eventInformation']['mLapsInEvent']: # Driver has fewer laps less than he should, either False start Or DNF
                        if self.driver_flags is not None:
                            ...
                            if self.driver_flags.get(participant_name) != 'Falsestart': # Check if the participant has a 'Falsestart' flag
                                ...
                                if self.driver_flags is not None: self.flags_updated.emit(self.driver_flags)
                                await asyncio.sleep(0.5)
                    print(f"MT Line {inspect.currentframe().f_lineno} Participant: {participant_name}, Flags: {self.driver_flags[participant_name]}")
...
class GuiMainApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.main_layout.addWidget(self.track_dropdown, alignment=Qt.AlignTop)
        ...
        self.main_layout.addWidget(self.car_dropdown, alignment=Qt.AlignTop)
        ...
        self.main_layout.addWidget(self.driver_dropdown, alignment=Qt.AlignTop)
        ...
        self.main_layout.addWidget(self.dropdown_races, alignment=Qt.AlignTop)
        ...
        self.calendar_widget.setStyleSheet("""
            ...
            padding: 2px 5px;
        """)
        self.calendar_widget.setGeometry(700, 975, 300, 150)
        ...
        self.main_layout.addWidget(self.dropdown_sessionid, alignment=Qt.AlignTop)
        ...
        def add_control_button(label, callback, key, layout):
        ...
        add_control_button("Find Computers", self.scan_active_computers, 'find_computers_button', self.button_column_layout)
        add_control_button("Filter Drivers", lambda: self.handle_radio_button_update(None), 'filter_drivers_button', self.button_column_layout)
        add_control_button("Disable CP", self.enable_disable_cp, 'Enable_CP', self.button_column_layout)
        add_control_button("Remove Flag", self.remove_falseflag, 'Remove_FF', self.button_column_layout)

        # Buttons in main_layout
        add_control_button("Load High Scores", self.load_selected_highscore, 'load_high_scores_button', self.main_layout)
        add_control_button("Delete Driver", self.delete_selected_driver, 'delete_driver_button', self.main_layout)
        add_control_button("Delete Selected Race", self.delete_selected_race, 'delete_button', self.main_layout)
        add_control_button("Delete All Races", self.delete_all_races, 'delete_all_races_button', self.main_layout)
        add_control_button("Next Session", self.start_new_session, 'new_session_button', self.main_layout)
        add_control_button("Previous Session", self.previous_session, 'previous_session_button', self.main_layout)
    ...
    def remove_falseflag(self):
        ...
        print(f"RMA Line {inspect.currentframe().f_lineno} Current driver text: {current_value}")
        if current_value:
            driver_name = current_value.strip()
            print(f"RMA Line {inspect.currentframe().f_lineno} Removing false flag for driver: {driver_name}")
    ...
    def handle_radio_button_update(self, data):
        ...
        """
        if data is None:
            print(f"RMA Line {inspect.currentframe().f_lineno} No data provided, using stored value.")
            data = getattr(self, 'active_computers', None)
            if data is None:
                print(f"RMA Line {inspect.currentframe().f_lineno} Still no data, skipping.")
                return
        else:
        ...
        for i, ip in enumerate(self.available_ips):
            ...
            if isinstance(data, dict):  # New style: {ip: (name, in_race)}
                ...
                print(f"RMA Line {inspect.currentframe().f_lineno} Button {i+1}: {name}, {'ENABLED' if in_race else 'DISABLED'}")
            elif isinstance(data, list):  # Old style: list of IPs
                button.setEnabled(ip in data)
                print(f"RMA Line {inspect.currentframe().f_lineno} Computer {i+1} is {'active' if ip in data else 'inactive'}: {ip}")
    ...
    def setup_highscore_view_setup(self):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def setup_highscore_view entered")
        labels = [
    ...
    def setup_driver_statistics_view_setup(self):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def setup_driver_statistics_view entered")
        labels = [
            ("driver_statistics_track",   "Track",   45, 165, 1000, 900, 14, False, False),
            ("driver_statistics_car",     "Car",     520,165,1000, 900, 14, False, False),
            ("driver_statistics_laptime", "Laptime", 900,165,150,  900, 14, False, False),
            ("driver_statistics_lap",     "Lap",     1015,165,150, 900, 14, False, False),
        ]
        self.setup_view(
            widget=self.driver_statistics_widget,
            layout=self.driver_statistics_layout,
            background_image=self.driver_statistics_background_image,
            heading_text="Select a driver to view statistics",
            heading_attr_name="driver_statistics_heading",
            heading_size=22,
            labels_info=labels
        )
    ...
    def setup_live_view(self):
        ...
        self.live_heading.setMinimumSize(1000, 120)
        self.live_view_layout.addWidget(self.live_heading, alignment=Qt.AlignTop)

        # Status/Top Speed/World Record
        self.live_status_label = create_label("Race Messages...", x=10, y=-5, w=325)
        self.live_best_label = create_label("Top Speed and Time:", x=350, y=-5, w=600)
        self.world_record_label = create_label("Records", x=650, y=-20, w=450, h=50, bold=True)
        ...
        for i in range(20):
            label = QLabel("", self.live_view_widget)
            label.hide()
            self.live_view_layout.addWidget(label)
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        driver_info_fields = [
            ("Car:", "car_heading_text", "car_txt", 53, 300),
            ("Driver:", "driver_heading_text", "driver_txt", 82, 300),
            ("Crash:", "collison_heading_text", "collison_txt", 82, 300),
            ("World Record:", "worldrecord_heading_text", "worldrecord_txt", 180, 200),
            ("Local Record:", "localrecord_heading_text", "localrecord_txt", 180, 200),
        ...
        lap_x = pedal_x + self.pedals_bg.width() + 30
        lap_y = 25
        ...
        driver_info_bgs = [
            ...
            ("bestlaptime", self.bestlaptime_image, "0:00.00", font_size, Qt.AlignCenter, lap_x, lap_y + self.lap_image.height() + self.laptime_image.height() + 2 * bg_spacing),
            ("deltaworldrecord", self.delta_world_record_image, "0:00.00", font_size, Qt.AlignCenter, lap_x, lap_y + self.lap_image.height() + self.laptime_image.height() + self.bestlaptime_image.height() + 3 * bg_spacing),
            ("deltalocalrecord", self.delta_local_record_image, "0:00.00", font_size, Qt.AlignCenter, lap_x, lap_y + self.lap_image.height() + self.laptime_image.height() + self.bestlaptime_image.height() + self.delta_world_record_image.height() + 4 * bg_spacing),
        ]
        ...
        for name, pixmap, text, fsize, align, pos_x, pos_y in driver_info_bgs:
            ...
            txt_label.setGeometry(25 if align == Qt.AlignLeft else 0, -5 if name != "lap" else -15, pixmap.width(), pixmap.height())
            txt_label.raise_()
...
class RaceApp(QThread):
    ...
    def handle_multi_data(self, data):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def handle_multi_data entered")
        #print(f"RMA Line {inspect.currentframe().f_lineno} Def handle_multi_data entered")
        self.multi_data = data

    def session_finished(self):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def session_finished entered")
        print(f"RMA Line {inspect.currentframe().f_lineno} Def session_finished entered")
        self.live_first_time_run = True  # Reset the flag to ensure the live view is updated with the new session data

    def update_ip_address(self, ip_address):
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} Def update_ip_address entered with IP: {ip_address}")
        print(f"RMA Line {inspect.currentframe().f_lineno} Def update_ip_address entered with IP: {ip_address}")
    ...
    def update_driver_names_old(self, driver_sims):
    ...
    def on_first_live_view_run(self, data):
        ...
        self.heading_wrecord = (
            f"World record: {trackworldrecord}")
    ...
    async def get_participants_ips_old(self):
        """
        Poll all sim IPs from config and check if they're active in a race.
        Returns a dict: ip_name_flag[ip] = (name, in_race_flag)
        - name: the name of the viewed participant (can be 'S-1', 'Bollocks', etc.)
        - in_race_flag: 1 if participants were returned from the API, else 0
        """
        ip_name_flag = {}
        sim_config = [
            ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '').strip()
            for i in range(1, 21)
            if ConfigManager.get_config_value(f'sim-{i}', 'simsettings', '').strip()
        ]

        async with aiohttp.ClientSession() as session:
            for i, ip in enumerate(sim_config, start=1):
                fallback_name = f"S-{i}"
                name = fallback_name
                in_race = 0

                try:
                    async with session.get(f"http://{ip}:8180/crest2/v1/api", timeout=3) as response:
                        if response.status != 200:
                ...
                index = data.get("viewedParticipantIndex", -1)

                if 0 <= index < len(participants):
                    fetched_name = participants[index].get("mName", "").strip()
                    if fetched_name:
                        name = fetched_name
                    in_race = 1
        ...
        self.radio_button_update.emit(ip_name_flag)

    def get_pariticipants_ips_old(self, participants):
        logging.info(f"RA Line {inspect.currentframe().f_lineno} Def get_participants_ips entered")
        #print(f"RA Line {inspect.currentframe().f_lineno} Participants: {participants}")
        ...
        for participant in participants:
            ...
            if name in self.driver_sims:
                try:
                    ...
                    print(f"RA Line {inspect.currentframe().f_lineno} Matched {name} (sim {sim_pos}) -> {ip}")
                except (ValueError, IndexError):
                    print(f"RA Line {inspect.currentframe().f_lineno} Sim-position {sim_pos} for {name} not in config.")
            else:
                print(f"RA Line {inspect.currentframe().f_lineno} {name} not found in self.driver_sims")

        print(f"RA Line {inspect.currentframe().f_lineno} Active drivers IPs: {self.active_drivers}")
    ...
    def update_live_view(self, data):
        ...
        if not getattr(self, "live_first_time_run_done", False):
            return
        if not getattr(self, 'multi_data', None):
            print(f"RMA Line {inspect.currentframe().f_lineno} No multi_data available, skipping live view update.")
            return
    ...
    def get_full_api_data_by_driver_name(self, name):
        ...
        """
        #print(f"RMA Line {inspect.currentframe().f_lineno} Def get_full_api_data_by_driver_name entered with name: {name}")
        #print (f"RMA Line {inspect.currentframe().f_lineno} Self.ip_name_flag: {self.ip_name_flag}")
        #print (f"RMA Line {inspect.currentframe().f_lineno} Self.multi_data: {self.multi_data}")
    ...
    def update_driver_view(self, data):
        ...
        if self.driver_view_first_update: # Or if IP changes
            self.deltarecordupdated = False
            ...
            # If driver changend, update these values 
            self.index = min(participants.get('mViewedParticipantIndex', 0), participants.get('mNumParticipants', 0) - 1)

            drvnode = participants['mParticipantInfo'][self.index]
            drivername = drvnode['mName']
            self.carname = drvnode['mCarNames']
            self.drivers = {
                index: participant['mName']
                for index, participant in enumerate(participants['mParticipantInfo'])
            }
            ...
            if not self.waiting_for_highscores:
                ...
                print(f"RMA Line {inspect.currentframe().f_lineno} track record db: {self.trackrecorddb}, car: {self.carname}, driver: {self.recorddriver}")
                print(f"RMA Line {inspect.currentframe().f_lineno} world record: {self.drivertrackrecord}, car: {self.carname}, driver: {self.trackworldrecord}")
                self.drivertrackrecorddb[self.carname] = { 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
                self.drivertrackrecord[self.carname] = {'worldrecord': self.trackworldrecord}
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} Driver view first update, track: {self.selected_track}, car: {self.carname}, driver: {drivername} worldrecord: {self.trackworldrecord}")
        ...
        drvnode = data['participants']['mParticipantInfo'][self.index]
        inputnode = data['unfilteredInput']
        speed = int(carnode['mSpeed'] * 3.6)
        crashindex = carnode['mLastOpponentCollisionIndex']
        crashname = self.drivers.get(crashindex, "N/A") if crashindex != -1 else "N/A"
        ...
        if self.classrace:
            if self.carname != self.previous_carname:  # Check if the car has changed
                if self.carname not in self.drivertrackrecord:
                    self.drivertrackrecord[self.carname] = {'worldrecord': self.trackworldrecord}
                self.trackworldrecord = data['timings']['mWorldFastestLapTime']
                print(f"RA Line {inspect.currentframe().f_lineno} Classrace, new car detected: {self.carname} - Old Car: {self.previous_carname}")
                if self.carname not in self.drivertrackrecorddb and not self.waiting_for_highscores:
                    self.task_queue.put(('load_highscores', (self.carname, self.selected_track)))
                    self.waiting_for_highscores = True
                    self.local_record_updated = False
                else: 
                    self.worldrecorddb = self.drivertrackrecorddb[self.carname]['worldrecord']
                    self.recorddriver = self.drivertrackrecorddb[self.carname]['recorddriver']
                    self.update_heading(self.carname, self.trackworldrecord, self.worldrecorddb)
                    print(f"RA Line {inspect.currentframe().f_lineno} Classrace, worldrecorddb for driver: {drivername}, car: {self.carname} already exists: {self.worldrecorddb}")

            if not self.local_record_updated and not self.waiting_for_highscores:
                    print(f"RA Line {inspect.currentframe().f_lineno} Classrace, worldrecddb update for driver: {drivername}, car: {self.carname}")
                    self.worldrecorddb = self.trackrecorddb
                    self.drivertrackrecorddb[self.carname] = {'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver}
                    self.update_heading(self.carname, self.trackworldrecord, self.worldrecorddb) 
            ...
            if not self.waiting_for_highscores and self.local_record_updated == False:
                print(f"RMA Line {inspect.currentframe().f_lineno} Variables: {self.waiting_for_highscores}, car: {self.local_record_updated}")
                self.worldrecorddb = self.trackrecorddb
                self.local_record_updated= True
        if self.local_record_updated:
            ...
            if self.deltalocalrecord < 0 and not self.deltarecordupdated: 
                ...
                self.drivertrackrecorddb[self.carname] = {'worldrecord': bestlap, 'recorddriver': drivername}
                self.deltarecordupdated = True  
        self.deltaworldrecord = bestlap - self.trackworldrecord if bestlap != -1 and self.trackworldrecord != -1 else 0

                
        self.previous_carname = self.carname
        ...
        if self.local_record_updated:
            ...
            formatted_trackworldrecord = "waiting for world record..."
            formatted_worldrecorddb = "Waiting for Highscores..."
            formatted_deltaworldrecord = "0"
            formatted_deltalocalrecord = "0"
        flag_value = self.driver_flags.get(drivername, None)
        #print(f"RMA Line {inspect.currentframe().f_lineno} flag: {flag_value}")
        ...
        if  not self.waiting_for_highscores:
            ...
            updates = [
                ...
                (self.gui.car_txt.setText, (self.carname,)),
    ...
    def create_participant_text(self, participant, flag, last_lap_str, lap_times_str, sessionstate,
        ...
        #print(f"RMA Line {inspect.currentframe().f_lineno} Def create_participant_text entered with name: {name}, sim: {sim}")
