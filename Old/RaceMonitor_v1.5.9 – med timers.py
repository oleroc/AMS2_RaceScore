                    ...
                    if response.status == 200: #Ensure the response content is valid JSON
                        try:
                            if data is None:
                                ...
                                if not self.running or self.session is None: return
                                end_time = time.time()
                                logging.info(f"API request took {end_time - start_time:.3f} seconds")
...
class RaceMonitorApp(QMainWindow):
    ...
    def on_highscore_loaded(self, high_scores):
        ...
        self.highscores = high_scores  # Store the high scores in a class variable
        print(f"RMA Line {inspect.currentframe().f_lineno} High Scores: {self.highscores}")
        if high_scores:
            loaded_names = ''
            loaded_lap = ''
            loaded_place = ''
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        self.car_txt = QLabel("Unknown Car", self.driver_info_tab)
        self.car_txt.setStyleSheet(f"font-family:{font_name};font-size: 22px; color: red;")
        self.car_txt.setMinimumSize(300, 50)
        self.car_txt.move(90, 300)
        self.track_txt = QLabel("Unknown Track", self.driver_info_tab)
        self.track_txt.setStyleSheet(f"font-family:{font_name};font-size: 22px; color: red;")
        self.track_txt.setMinimumSize(300, 50)
        self.track_txt.move(110, 350)
        self.driver_txt = QLabel("Unknown Driver", self.driver_info_tab)
        ...
        for bar, color, offset in zip([self.clutch_bar, self.brake_bar, self.throttle_bar], ['blue', 'red', 'green'], [0, 30, 60]):
            ...
            bar.setRange(0, 100)
            bar.setGeometry(offset + 10, 32, 20, 75)  # Leave 10px padding at top
            bar.setStyleSheet(f"""
                QProgressBar {{
                    border: none;
        ...
        self.laptime_text.setAlignment(Qt.AlignCenter)
        self.laptime_text.setGeometry(0, -5, self.laptime_label.width(), self.laptime_label.height())
        ...
        self.pos_label.move(self.laptime_label.x() + 50, self.laptime_label.y() + self.laptime_label.height() + 10)
    ...
    def update_driver_view(self, data):
        ...
        if not data or 'participants' not in data or 'mParticipantInfo' not in data['participants']:
            return
        speed= int(data['carState']['mSpeed'] * 3.6)  # Convert speed from m/s to km/h
        #speed_kph = int(data['carState']['mSpeed'] * 3.6)
        driverindex = data['participants']['mViewedParticipantIndex']
        current_lap = data['participants']['mParticipantInfo'][driverindex]['mCurrentLap']
        total_laps = data['eventInformation']['mLapsInEvent']
        Race_positing = data['participants']['mParticipantInfo'][driverindex]['mRacePosition']
        drivername = data['participants']['mParticipantInfo'][driverindex]['mName']
        carname = data['participants']['mParticipantInfo'][driverindex]['mCarNames']
        trackname = data['eventInformation']['mTranslatedTrackVariation']
        ...
        self.lap_text.setText(f"{current_lap}/{total_laps}")
        ...
        def update_driver_current_lap_time(data):
            driver = data['participants']['mParticipantInfo'][driverindex]
            name = driver['mName']
            lap = driver['mCurrentLap']
            sector_index = driver['mCurrentSector']
            s1 = driver['mCurrentSector1Times']
            s2 = driver['mCurrentSector2Times']
            s3 = driver['mCurrentSector3Times']

            if not hasattr(self, 'driver_sector_times'):
                driver_sector_times = {}
            if not hasattr(self, 'driver_last_sector'):
                driver_last_sector = {}

            # Store live sector time
            driver_sector_times[(lap, sector_index)] = s1 if sector_index == 0 else s2 if sector_index == 1 else s3

            # Finalize sector switch
            last_sector = driver_last_sector.get(name)
            if sector_index != last_sector:
                if last_sector == 0:
                    driver_sector_times[(lap, 0)] = s1
                elif last_sector == 1:
                    driver_sector_times[(lap, 1)] = s2
                elif last_sector == 2:
                    driver_sector_times[(lap - 1, 2)] = s3
            driver_last_sector[name] = sector_index

            # Aggregate total time this lap
            current_lap_time = sum(driver_sector_times.get((lap, s), 0.0) for s in range(3))
            self.laptime_text.setText(self.format_lap_time(current_lap_time))
        ...
        # Rotate the steering wheel needle based on mUnfilteredSteering input
        update_driver_current_lap_time(data)
        ...
        end_time = time.time()
        logging.info(f"update_driver_view took {end_time - start_time:.3f} seconds")
        # Force a repaint and log the time
        self.speed_display.repaint()  # Force immediate repaint
        repaint_time = time.time()
        logging.info(f"Repaint triggered at {repaint_time:.3f}")
    ...
    def update_live_view(self, data):
        ...
        if self.tab_widget.currentWidget() != self.live_view_widget or self.gui_first_time_run == None:
    ...
    def update_live_view_logic(self, data):
       ...
       # if race_state == 3 or race_state == 6:
       #     pass  # Race ending, logic only
        ...
        self.gui_first_time_run = True

    def update_live_view_gui(self, data):
        participants = self.sorted_participants
        sessionstate = data['gameStates']['mSessionState']

        if self.best_lap_time < self.previous_best_lap_time or self.gui_first_time_run:
