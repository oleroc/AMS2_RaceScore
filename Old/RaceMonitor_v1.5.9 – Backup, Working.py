    ...
    def load_selected_race(self, race_id, callback = None):
        try:
            ...
            if race:
                ...
                self.cursor.execute('''
                    SELECT mName, CalculatedPosition, mFastestLapTimes, mLastLapTimes, mCarNames, flags, PitStops
                    FROM Participants
                    WHERE RaceID = ?
                    ORDER BY CalculatedPosition ASC
                ''', (race[0],))
                participants = self.cursor.fetchall()
    ...
    async def get_api_data(self):
        ...
        while self.no_server_connection:
            ...
            if self.previous_ipaddress != self.ip_address:
                ...
                print(f"MT Line {inspect.currentframe().f_lineno} Ipadress changed, new ipadress {self.ip_address}")
                logging.info(f"MT Line {inspect.currentframe().f_lineno} Ipadress changed, new ipadress {self.ip_address}")
                error_message_shown = False  # Reset error message shown flag when IP address changes
            try:
...
class RaceMonitorApp(QMainWindow):
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        font_name = self.font_family_digits
        # Scale both dials to 250x250
        ...
        self.speed_display = QLabel(self.driver_info_tab)
        self.speed_display.setFont(QFont(self.font_family_digits, 32))
        self.speed_display.setStyleSheet("color: red; background-color: black;")  # only non-font styles here
        self.speed_display.setAttribute(Qt.WA_TranslucentBackground)
        #self.speed_display.setStyleSheet("background: transparent;")
        self.speed_display.setAlignment(Qt.AlignCenter)
        self.speed_display.setFixedSize(50, 30)
        self.speed_display.raise_()

        speedo_pos = self.speedometer_label.pos()
        speedo_size = self.speedometer_label.size()
        speed_display_x = speedo_pos.x() + int(speedo_size.width() * 0.55)
        speed_display_y = speedo_pos.y() + int(speedo_size.height() * 0.67)
        self.speed_display.move(speed_display_x, speed_display_y)

        self.gear_display = QLabel(self.driver_info_tab)
        self.gear_display.setFont(QFont(self.font_family_digits, 32))
        self.gear_display.setStyleSheet("color: red; background-color: black;")  # only non-font styles here
        self.gear_display.setAttribute(Qt.WA_TranslucentBackground)
        #self.gear_display.setStyleSheet("background: transparent;")
        self.gear_display.setAlignment(Qt.AlignCenter)
        self.gear_display.setFixedSize(50, 30)
        #self.gear_display.move(tacho_pos_x + offset_x, tacho_pos_y + offset_y)
        self.gear_display.raise_()
        tacho_pos = self.tachometer_label.pos()
        tacho_size = self.tachometer_label.size()
        gear_display_x = tacho_pos.x() + int(tacho_size.width() * 0.55)
        gear_display_y = tacho_pos.y() + int(tacho_size.height() * 0.67)
        self.gear_display.move(gear_display_x, gear_display_y)
        # Dynamically place speed and gear displays
        self.speed_display.move(speed_display_x, speed_display_y)
        self.gear_display.move(gear_display_x, gear_display_y)

        # Needle overlay labels for speedometer and tachometer
        self.scaled_needle_image = self.needle_image.scaled(134, 134, Qt.KeepAspectRatio, Qt.SmoothTransformation)

        self.speed_needle_label = QLabel(self.driver_info_tab)
        ...
        self.speed_needle_label.setFixedSize(self.needle_image.size())
        #self.speed_needle_label.move(112, 163)  # replace with pixel coordinates

        self.tacho_needle_label = QLabel(self.driver_info_tab)
        ...
        self.tacho_needle_label.setFixedSize(self.needle_image.size())
        #self.tacho_needle_label.move(482, 163)  # replace with pixel coordinates


        # Labels with images
        self.lap_label = QLabel(self.driver_info_tab)
        self.lap_label.setPixmap(self.lap_image)
        self.lap_text = QLabel("0/0")
        self.lap_text.setStyleSheet("color: red; font-size: 22px; font-family: 'Digital';")

        self.laptime_label = QLabel(self.driver_info_tab)
        self.laptime_label.setPixmap(self.laptime_image)
        self.laptime_text = QLabel("0:00.00")
        self.laptime_text.setStyleSheet("color: red; font-size: 22px; font-family: 'Digital';")

        self.pos_label = QLabel(self.driver_info_tab)
        self.pos_label.setPixmap(self.pos_image)
        self.pos_text = QLabel("0")
        self.pos_text.setStyleSheet("color: red; font-size: 22px; font-family: 'Digital';")

        pedal_x = self.tachometer_label.x() + self.tachometer_label.width() + 30
        base_y = self.tachometer_label.y()

        # Car & Track
        self.car_text = QLabel("Car: -", self.driver_info_tab)
        self.track_text = QLabel("Track: -", self.driver_info_tab)
        self.pedals_background = QLabel(self.driver_info_tab)
        self.pedals_background.setPixmap(self.pedals_image)
        self.pedals_background.setScaledContents(True)  # optional, if you want auto-scaling
        self.pedals_background.setFixedSize(100, 110)   # adjust based on your pedal bar area
        self.pedals_background.move(pedal_x-10, base_y)    # same x, y as the top-left of your first pedal bar
        self.pedals_background.lower()  # Send it behind everything else

        # Progress bars
        self.throttle_bar = QProgressBar(self.driver_info_tab)
        ...
        self.brake_bar = QProgressBar(self.driver_info_tab)
        ...
        self.clutch_bar = QProgressBar(self.driver_info_tab)
        self.clutch_bar.setOrientation(Qt.Vertical)
        self.clutch_bar.setRange(0, 100)
        self.clutch_bar.setStyleSheet("QProgressBar::chunk { background-color: blue; }")
        ...
        self.clutch_bar.setGeometry(pedal_x, base_y, 20, 100)
        self.brake_bar.setGeometry(pedal_x + 30, base_y, 20, 100)
        self.throttle_bar.setGeometry(pedal_x + 60, base_y, 20, 100)
        
        

        self.throttle_bar.raise_()
        self.brake_bar.raise_()
        self.clutch_bar.raise_()

        # Read tachometer dial geometry after setting its size and position
        tacho_pos = self.tachometer_label.pos()
        tacho_size = self.tachometer_label.size()
        tacho_center = QPoint(tacho_pos.x() + tacho_size.width() // 2,
                            tacho_pos.y() + tacho_size.height() // 2)

        needle_center = self.scaled_needle_image.rect().center()

        self.tacho_needle_label.setPixmap(self.scaled_needle_image)
        self.tacho_needle_label.setFixedSize(self.scaled_needle_image.size())
        self.tacho_needle_label.move(tacho_center.x() - needle_center.x(),
                                    tacho_center.y() - needle_center.y())
        self.tacho_needle_label.raise_()

                
        # Read speedometer dial geometry (after setting size and position)
        speedo_pos = self.speedometer_label.pos()
        speedo_size = self.speedometer_label.size()
        speedo_center = QPoint(speedo_pos.x() + speedo_size.width() // 2,
                            speedo_pos.y() + speedo_size.height() // 2)

        needle_center = self.scaled_needle_image.rect().center()

        self.speed_needle_label.setPixmap(self.scaled_needle_image)
        self.speed_needle_label.setFixedSize(self.scaled_needle_image.size())
        self.speed_needle_label.move(speedo_center.x() - needle_center.x(),
                                    speedo_center.y() - needle_center.y())
    ...
    def update_driver_view(self, data):
        ...
        if not data or 'participants' not in data or 'mParticipantInfo' not in data['participants']:
            return
        speed= data['carState']['mSpeed'] * 3.6
        speed_kph = int(data['carState']['mSpeed'] * 3.6)
        gear = data['carState']['mGear']  # 0 = neutral

        self.speed_display.setText(str(speed_kph))
        self.gear_display.setText('N' if gear == 0 else str(gear))

        inputs = data.get('unfilteredInput', {})

        throttle = int(inputs.get('mUnfilteredThrottle', 0) * 100)
        brake    = int(inputs.get('mUnfilteredBrake', 0) * 100)
        clutch   = int(inputs.get('mUnfilteredClutch', 0) * 100)
        ...
        rotate_needle(data['carState']['mRpm'], self.tacho_max, self.tacho_needle_label,
    ...
    def on_first_live_view_run(self, data):
        ...
        heading_text = f"{self.event_info['mTranslatedTrackLocation']} - {self.event_info['mTranslatedTrackVariation']} ({'Qualify' if sessionstate == 3 else 'Practice' if sessionstate == 1 else self.event_info['mLapsInEvent']}) - {participants[0]['mCarNames']} - {len(participants)} Drivers - Session ID: {self.session_id}" # Create the heading text
    ...
    def update_live_view(self, data):
        self.update_live_view_logic(data)

        if self.tab_widget.currentWidget() != self.live_view_widget:
            #print(f"RMA Line {inspect.currentframe().f_lineno} Live View tab not active, skipping GUI updates")
            return  # Skip GUI updates if Live View tab is not active

        self.update_live_view_gui(data)

    def update_live_view_logic(self, data):
        ...
        self.sorted_participants = sorted_participants
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            if (self.current_lap[name] - 1 == self.event_info['mLapsInEvent'] and sessionstate not in (1, 3)):
                self.driver_finished[name] = True

            if not self.driver_finished.get(name, False):
                last_lap = self.previous_current_lap.get(name)
                last_sector = self.last_sector.get(name)
                sector_index = participant['mCurrentSector']

                self.sector_times[(name, self.current_lap[name], sector_index)] = (
                    s1 if sector_index == 0 else s2 if sector_index == 1 else s3
                )

                if sector_index != last_sector:
                    if last_sector == 0:
                        self.sector_times[(name, self.current_lap[name], 0)] = s1
                    elif last_sector == 1:
                        self.sector_times[(name, self.current_lap[name], 1)] = s2
                    elif last_sector == 2:
                        self.sector_times[(name, self.current_lap[name] - 1, 2)] = s3

                if self.current_lap[name] != last_lap and s3 != -1:
                    self.sector_times[(name, last_lap, 2)] = s3

                self.last_sector[name] = sector_index

                total_time_driver = sum(
                    self.sector_times.get((name, lap, sector), 0.0)
                    for lap in range(1, self.current_lap[name] + 1)
    ...
    def update_live_view_gui(self, data):
        participants = self.sorted_participants
        sessionstate = data['gameStates']['mSessionState']

        if self.best_lap_time < self.previous_best_lap_time or self.live_first_time_run:
            for j in range(min(20, len(participants))):
        ...
        for i, participant in enumerate(participants):
            ...
            if label:
                ...
                gap_to_ahead_str, gap_to_ahead_distance_str = self.get_gap_to_ahead(participant, participants)
        ...
        if race_state == 2 and not self.race_started:
            ...
            self.live_status_label.setText(f"{leader['mName']} is leading lap {leader['mCurrentLap']}")
        self.live_first_time_run = False
    
    def get_gap_to_leader(self, participant):
        if participant['mName'] == self.leader_name:
            return "Leader", "0.0m"
    ...
    def get_race_status(self, participant, sessionstate):
        if participant['mCurrentLap'] - 1 == self.event_info['mLapsInEvent'] and sessionstate not in (1, 3):
