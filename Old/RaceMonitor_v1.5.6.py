...
class RaceMonitorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        print(f"RMA Line {inspect.currentframe().f_lineno} Font file will be written to: {self.font_filename}")
    ...
    def update_live_view(self, data):
        ...
        for i, participant in enumerate(sorted_participants): # Loop through each participant to update their corresponding label
            ...
            #current_sector = participant['mCurrentSector']
            s1 = participant['mCurrentSector1Times']
            s2 = participant['mCurrentSector2Times']
            s3 = participant['mCurrentSector3Times']

            if (self.current_lap[participant['mName']] - 1 == self.event_info['mLapsInEvent'] and sessionstate != 3 and sessionstate != 1):
                self.driver_finished[participant['mName']] = True
                

            if not self.driver_finished.get(participant['mName'], False) and s1 != -1:
                
                last_lap = self.previous_current_lap.get(participant['mName'])
                last_sector = self.last_sector.get(participant['mName'])
                self.sector_times[(participant['mName'], self.current_lap[participant['mName']], participant['mCurrentSector'])] = s1 if participant['mCurrentSector'] == 0 else s2 if participant['mCurrentSector'] == 1 else s3

                if participant['mCurrentSector'] != last_sector:
                    if last_sector == 0:
                        self.sector_times[(participant['mName'], self.current_lap[participant['mName']], 0)] = s1
                    elif last_sector == 1 :
                        self.sector_times[(participant['mName'], self.current_lap[participant['mName']], 1)] = s2
                    elif last_sector == 2 :
                        self.sector_times[(participant['mName'], self.current_lap[participant['mName']] - 1, 2)] = s3

                if self.current_lap[participant['mName']] != last_lap:
                    if s3 != -1:
                        self.sector_times[(participant['mName'], last_lap, 2)] = s3

                self.last_sector[participant['mName']] = participant['mCurrentSector']

                total_time_driver = sum(
                    self.sector_times.get((participant['mName'], lap, sector), 0.0)
                    for lap in range(1, self.current_lap[participant['mName']] + 1)
                    for sector in range(3)
                )

                if participant['mRacePosition'] == 1:
                    self.leader_name = participant['mName']
                    self.leader_current_lap = self.current_lap[participant['mName']]
            ...
            if s1 != -1:        
                self.total_time_seconds[participant['mName']] = total_time_driver
            else: total_time_driver = 0.0
            ...
            if participant['mLastLapTimes'] != -123 and participant['mName'] in self.previous_current_lap:
                if self.current_lap[participant['mName']] != self.previous_current_lap.get(participant['mName'], None):
                    self.live_status_label.setText(f"{participant['mName']} is now on lap {participant['mCurrentLap']}{' (Qualify)' if sessionstate == 3 else ' (Practice)' if sessionstate == 1 else ''}")  # Update the status label
            ...
            participant_text = self.create_participant_text(participant,flag, last_lap_str, lap_times_str, sessionstate, total_time_str, gap_str, gap_to_ahead_str, gap_to_ahead_distance_str, gap_distance_str) # Create the participant text using the utility function
    ...
    def get_gap_to_leader(self, participant):
        ...
        if not self.lap_list.get(self.leader_name, []):
            return "N/A", "0.0 m"

        driver_lap = participant['mCurrentLap']
        leader_lap = self.leader_current_lap
        track_length = self.event_info.get('mTrackLength', 0)

        driver_progress = participant.get('mCurrentLapDistance', 0.0)
        leader_progress = self.leader_progress

        driver_abs_progress = driver_lap * track_length + driver_progress
        leader_abs_progress = leader_lap * track_length + leader_progress

        total_gap_distance = leader_abs_progress - driver_abs_progress
        leader_speed_mps = self.get_average_speed(self.leader_name)

        if leader_speed_mps <= 1e-3:
            gap_distance_str = f"{total_gap_distance/1000:.1f} km" if total_gap_distance >= 1000 else f"{total_gap_distance:.0f} m"
            return "0.0s", gap_distance_str

        gap_time = total_gap_distance / leader_speed_mps
        gap_str = f"+{gap_time:.0f}"
        gap_distance_str = f"{total_gap_distance/1000:.1f} km" if total_gap_distance >= 1000 else f"{total_gap_distance:.0f} m"
    ...
    def _gap_details(self, participant):
        ...
        speed = self.get_average_speed(self.leader_name)
        gap_dist = (self.leader_current_lap * track_length + self.leader_progress) - abs_progress
        gap_time = gap_dist / speed if speed > 1e-3 else 0.0
    ...
    def create_participant_text(self, participant,flag, last_lap_str, lap_times_str, sessionstate, total_time_str="", gap_str="", gap_to_ahead_str="", gap_to_ahead_distance_str="", gap_distance_str="" ): # Helper function to create participant text with custom styling.self.initialize_dropdown
        ...
        sim = self.driver_sims.get(participant['mName'], '')  # Get the sim for the participant, default to empty string if not found
        # print(f"RMA Line {inspect.currentframe().f_lineno} Name: {participant['mName']} sim: {sim}")
        ...
        if participant['mFastestLapTimes'] < self.best_lap_time and participant['mCurrentLap'] > 1 and participant['mFastestLapTimes'] != -123:
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} New best Lap Time: {self.format_lap_time(self.best_lap_time)}")
            self.live_status_label.setText(f"{participant['mName']} just got a new best lap time{' (Qualify)' if sessionstate == 3 else ' (Practice)' if sessionstate == 1 else ''}: {self.format_lap_time(self.best_lap_time)}")
        ...
        if participant['mCurrentLap'] -1 == self.event_info['mLapsInEvent'] and sessionstate != 3 and sessionstate !=1: race_status = "<span style='color:#FF0000;font-weight:bold'>(Finished!)</span>"
        else: race_status = ""
        ...
        if participant['mPitModes'] == 2 and  participant['mCurrentLap'] not in self.pit_stop[participant['mName']]:
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} Pit Stop added for {participant['mName']} number of pitstops:{sum(self.pit_stop[participant['mName']].values())} all pit pit stop: {self.pit_stop}")
            print (f"RMA Line {inspect.currentframe().f_lineno} Number of pitstops for participant {participant['mName']}: {sum(self.pit_stop[participant['mName']].values())}")
            logging.info(f"RMA Line {inspect.currentframe().f_lineno} Pit Stop added for {participant['mName']} number of pitstops:{sum(self.pit_stop[participant['mName']].values())} all pit pit stop: {self.pit_stop}")
        ...
        # Only add Sim line if sim is non-empty
        if sim:
            output += f"<span style='color:#00fff0;'>Sim: <span style='color:#FFD700;'>{sim}</span> - "

        # Rest of the inline info
        output += (
            f"<span style='color:#00fff0;'>Speed: <span style='color:#FFD700;'>{participant['mSpeeds'] * 3.6:.0f} Km/t</span> "
            #f"Car: <span style='color:#FFD700;'>{participant['mCarNames']}</span> - "
            f"Top Speed: <span style='color:#FFD700;'>{self.top_speed[participant['mName']] * 3.6:.0f}</span> "
            ...
            #f"Behind: <span style='color:#FFD700;'>gap_str:</span>"
        if participant['mName'] != self.leader_name:
            output += (

            f" Dist: <span style='color:#FFD700;'>{gap_distance_str}</span> "
            f"Gap Ahead: <span style='color:#FFD700;'>{gap_to_ahead_str}</span> "
            f"Dist: <span style='color:#FFD700;'>{gap_to_ahead_distance_str}</span>"
        )

        # Separate Laptimes line
        output += (
