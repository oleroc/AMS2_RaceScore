    ...
    def handle_driver_statistics_on_start(self, driverdata):
        ...
        if driverdata:
            for driver in driverdata:
                ...
                self.driver_dropdown.setCurrentIndex(self.driver_dropdown.count() - 1)
            if driverdata:
                print(f"Number of Highscores loaded: {len(driverdata)}")
            if not len(driverdata) >1: self.load_selected_driver()
            else: self.driver_statistics_heading.setText("Select a Driver to View Driver Statistics")
        self.driver_dropdown.blockSignals(False)

    def load_selected_driver(self):
        logging.info("RMA load_selected_driver def entered")
        print("RMA load_selected_driver def entered")
        selected_driver = self.driver_dropdown.currentText()
        if self.driver_dropdown.currentIndex() >= 0:
            print(f"Load driver statistics for {selected_driver}")
            logging.info(f"RMA Load driver statistics for {selected_driver}")
            self.on_driver_statistics_loaded(selected_driver)
        else:
            print(f"Driver not selected:{selected_driver}")
            logging.info(f"Driver not selected:{selected_driver}")

    def on_driver_statistics_loaded(self, driver):
