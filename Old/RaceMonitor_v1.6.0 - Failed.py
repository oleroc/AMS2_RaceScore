...
import socket, atexit, sys, re, inspect, requests, sqlite3, queue, json, time, traceback, logging, os, asyncio, aiohttp, math, base64, configparser, tempfile
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QSizePolicy, QVBoxLayout, QWidget, QPushButton, QComboBox, QHBoxLayout, QTabWidget, QSpacerItem, QMessageBox, QLineEdit, QCalendarWidget, QProgressBar, QButtonGroup, QRadioButton, QGridLayout
...
class DatabaseThread(QThread):
    ...
    def __init__(self, task_queue, config_manager):
        ...
        self.conn = None #connection logic is not expected to occur in __init__.
...
class ControlPanel(QObject):
    ...
    def __init__(self, db_thread, config_manager, racelogicapp):
        super().__init__()
        self.running = True
        self.config_manager= config_manager
        self.config = configparser.ConfigParser()
        ...
        self.racelogicapp = racelogicapp
        
        
        #signalling
        self.db_thread.write_cp_signal.connect(self.process_cp_write_signal) # Connect the signal to a slot that updates Session ID
        ...
        self.racelogicapp.get_request_for_sim_signal.connect(self.process_sim_signal) # Connect the signal to a slot that processes the sim data
    ...
    def process_cp_write_signal(self, ranked_drivers, lap_times_dict):
...
class MonitorThread(QThread):
    ...
    def __init__(self, gui_main_app,race_logic_app, tab_widget ,task_queue, config_manager, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.config_manager = config_manager
        self.ip_address = self.config_manager.read_ip_address()   #threading.Thread.__init__(self)   
        self.gui_main_app = gui_main_app
        self.race_logic_app = race_logic_app
        self.task_queue = task_queue
        ...
        #Signalling
        self.gui_main_app.session_id_updated.connect(self.set_session_id) # Connect the signal to a slot that updates Session ID
        self.gui_main_app.pit_stops_updated.connect(self.update_pit_stops) # Connect the signal to a slot that updates pit stops
    ...
    async def connection_monitor(self):
        last_scan_time = time.monotonic() - 300  # Initialize last scan time to 5 minutes ago
        print(f"MT Line {inspect.currentframe().f_lineno} Connection Monitor started with IP: {last_scan_time}")
        ...
        while self.running:
            try:
                ...
                if time.monotonic() - last_scan_time >= 300:
                    ...
                    self.radio_button_update.emit(active_computers)
    ...
    async def race_running(self, data):
        ...
        while self.session_id == 0: # Just to be sure the session ID is not 0, wait for it to be updated from GuiMainApp Thread via signalling.
...
class GuiMainApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.control_panel = ControlPanel(self.db_thread, self.config_manager, self.race_logic_app)  # Pass the db_thread and config_manager to ControlPanel
        self.race_logic_app = RaceLogicApp(monitor_thread=None, task_queue=self.task_queue, config_manager=self.config_manager, db_thread=self.db_thread, control_panel=self.control_panel)   # Pass the task queue and config manager to RaceLogicApp and add the place holder for circular reference.
        self.config_manager.update_config_file(self)
        ...
        self.temp_dir = "tmp/"
        self.initUI() # Running the DEF Initializing the GUI
        self.monitor_thread = MonitorThread(self, self.race_logic_app, self.tab_widget, self.task_queue, self.config_manager) #Self.tab_widget is defind in initUI()
        # Fix the missing reference inside race_logic_app
        self.race_logic_app.set_monitor_thread(self.monitor_thread)
        self.monitor_thread.start()
        self.db_thread.start()
        self.race_logic_app.start()
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        '''
        self.driver_view_layout.setAlignment(Qt.AlignBottom | Qt.AlignRight)
        self.driver_view_layout.setSpacing(0)  # Reduce spacing between items to 2 pixels
        self.sim_radio_group = QButtonGroup(self.driver_info_tab)
        for i in range(20):
            btn = QRadioButton(str(i + 1))
            btn.setToolTip(self.available_ips[i])
            self.sim_radio_group.addButton(btn, i)
            row = i // 5  # 0 to 3 for 4 rows (5 buttons per row)
            col = i % 5   # 0 to 4 for 5 columns
            btn.setFixedSize(40, 20)
            self.driver_view_layout.addWidget(btn, row, col)  # Use grid layout with row, col
        self.sim_radio_group.buttonClicked.connect(lambda button: self.set_monitored_ip(self.sim_radio_group.id(button)))
        self.sim_radio_group.button(0).setChecked(True)
            
        #self.driver_view_layout.setColumnStretch(col, 1)
        #self.driver_view_layout.setRowStretch(row, 1)
        #self.driver_view_layout.setContentsMargins(0, 0, 0, 0)  # Remove margins

         # Working version with buttons in the bottom widget
        self.driver_view_layout.setAlignment(Qt.AlignBottom)        
        self.sim_radio_group = QButtonGroup(self.driver_info_tab)
        for i in range(20):
            btn = QRadioButton(str(i + 1))
            btn.setToolTip(self.available_ips[i])  # Show IP on hover
            self.sim_radio_group.addButton(btn, i)
            self.driver_view_layout.addWidget(btn)
        self.sim_radio_group.buttonClicked.connect(lambda button: self.set_monitored_ip(self.sim_radio_group.id(button)))
        self.sim_radio_group.button(0).setChecked(True)  # Default to first sim
        # End of working version with buttons in the bottom widget

        #self.bottom_widget.show()  # Show when tab is active       
        #self.bottom_widget.setLayout(self.driver_view_layout)
        
        
        self.bottom_widget = QWidget()
        self.bottom_widget.setLayout(QVBoxLayout())  # Ensure bottom_widget has its own layout
        for i in range(20):
            btn = QRadioButton(str(i + 1))
            btn.setToolTip(self.available_ips[i])
            self.bottom_widget.layout().addWidget(btn)  # Add buttons to bottom_widget's layout
        
        self.driver_info_tab.layout().addWidget(self.bottom_widget)  # Add bottom_widget to tab layout
        self.bottom_widget.setParent(self.driver_info_tab)  # Explicitly set parent
        self.bottom_widget.hide()  # Hide until tab is active
        '''
        #print(f"Tab layout: {self.driver_info_tab.layout()}, Bottom widget: {self.bottom_widget.layout()}")
        
        #self.bottom_widget.show()  # Show when tab is active
        #print(f"Bottom widget visible: {self.bottom_widget.isVisible()}")
    ...
    def stop(self):
        ...
        if hasattr(self, 'monitor_thread') and self.monitor_thread.isRunning(): # Stop the monitor thread if it exists and is running
                ...
                pass  # Signals might already be disconnected or not connected
        logging.info(f"RMA Line {inspect.currentframe().f_lineno} GuiMainApp stopped successfully.") # Log completion of stopping sequence

class RaceLogicApp(QThread):
    finished = pyqtSignal()
    ...
    def __init__(self, task_queue, config_manager, db_thread=None, monitor_thread=None, control_panel=None, parent=None):
    ...
    async def main(self):
        try:
            while self.running:
                await self.task()
                await asyncio.sleep(1)  # Your polling interval
        except asyncio.CancelledError:
            pass

    async def task(self):
        # Put your async logic here
