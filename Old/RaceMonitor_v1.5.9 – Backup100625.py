...
class RaceMonitorApp(QMainWindow):
    ...
    def setup_driver_info_view(self): # Setup your driver info view widgets here
        ...
        self.pos_bg.move(self.pedals_bg.x() - int((self.pos_bg.width() - self.pedals_bg.width())/2), self.pedals_bg.y() + self.pedals_bg.height() + 10)
    ...
    def update_driver_view(self, data):
        ...
        if self.driver_view_first_update:
            ...
            if not self.waiting_for_highscores:
                ...
                self.drivertrackrecorddb[drivername] = {'car': carname, 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} #dictionary  initialized in live view first first run
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} Driver view first update, track: {self.selected_track}, car: {carname}, driver: {drivername} worldrecord: {self.worldrecord}")
        ...
        if self.classrace:
            if carname != self.previous_carname:  # Check if the car has changed
                self.worldrecord = data['timings']['mWorldFastestLapTime']
                print(f"RMA Line {inspect.currentframe().f_lineno} Classrace, new car detected: {carname} - {self.previous_carname}")
                if drivername not in self.drivertrackrecorddb and not self.waiting_for_highscores:
                    self.db_queue.put(('load_highscores', (carname, self.selected_track)))
                    self.waiting_for_highscores = True
                    self.local_record_updated = False
                    self.break_loop = False
                else: 
                    self.worldrecorddb = self.drivertrackrecorddb[drivername]['worldrecord']
                    self.recorddriver = self.drivertrackrecorddb[drivername]['recorddriver']
            ...
            if not self.local_record_updated and not self.waiting_for_highscores:
                    ...
                    self.drivertrackrecorddb[drivername] = {'car': carname, 'worldrecord': self.trackrecorddb, 'recorddriver':self.recorddriver} 
        ...
        self.deltaworldrecord = bestlap - self.worldrecord if bestlap != -1 and self.worldrecord != -1 else 0
        if self.deltalocalrecord < 0: 
            ...
            self.drivertrackrecorddb[drivername] = {'car': carname, 'worldrecord': bestlap, 'recorddriver': drivername}
        ...
        if  not self.waiting_for_highscores:
            ...
            updates = [
                (self.driver_txt.setText, (drivername,)),
                (self.worldrecord_txt.setText, (self.format_lap_time(self.worldrecord) if self.worldrecord != -1 else "Mod Car/Track",)),
    ...
    def on_first_live_view_run(self, data):
        ...
        self.session_id_updated.emit(self.session_id)
        self.db_queue.put(('load_highscores', (selected_car, self.selected_track)))
        ...
        f"({('Qualify' if sessionstate == 3 else 'Practice' if sessionstate == 1 else f'Laps {self.event_info["mLapsInEvent"]}')})<br>"
        f"Car: {participants[0]['mCarNames']} - {len(participants)} Drivers<br>"
            ...
            if participant['mLastLapTimes'] != -123 and name in self.previous_current_lap and not self.driver_finished.get(name, False):
                if self.current_lap[name] != self.previous_current_lap.get(name):
                    ...
                    if self.classrace: 
                        ...
                        if not self.waiting_for_highscores:
                            if self.lap_time[name] < self.trackworldrecord and self.lap_time[name]:
                            ...
                            if self.lap_time[name] < getattr(self, 'liverecorddb', self.trackrecorddb) and self.lap_time[name]:
                                print(f"RMA Line {inspect.currentframe().f_lineno} Driver laptime: {self.lap_time[name]} , Local Record: {getattr(self, 'liverecorddb', self.trackrecorddb)}")
