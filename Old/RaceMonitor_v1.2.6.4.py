...
class RaceMonitorApp(QMainWindow):
    ...
    def update_live_view(self, data):
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            if current_lap < 3:
                ...
                # Replace direct database access with fetch_recorded_laps call
                self.db_thread.fetch_recorded_laps(
                    participant_name,
                    self.current_race_id,
                    current_lap,
                    callback=lambda p_name, laps, lap_num: self.process_fetched_laps(i, p_name, laps, lap_num, best_lap_time, top_background_color, background_color)
                )
            ...
            if valid_lap_times:
                ...
                self.live_status_label.setText("Race Started!")
    ...
    def process_fetched_laps(self, index, participant_name, laps, current_lap, best_lap_time, top_background_color, background_color):
        def on_laps_fetched(participant_name, recorded_laps, current_lap):
            ...
            return lap_times_str, total_time_str

        lap_times_str, total_time_str = on_laps_fetched(participant_name, laps, current_lap)

        label = self.participant_labels[index]
        last_lap_str = self.format_lap_time(label['mLastLapTimes'])
        fastest_lap_str = self.format_lap_time(label['mFastestLapTimes'])

        participant_text = self.create_participant_text(label, last_lap_str, fastest_lap_str, lap_times_str, total_time_str)
        label.setText(participant_text)

        if label['mFastestLapTimes'] == best_lap_time:
            label.setStyleSheet(f"font-size: 14px; background-color: {top_background_color}; padding: 5px; margin-bottom: 2px; border-radius: 5px;")
        else:
            label.setStyleSheet(f"font-size: 14px; background-color: {background_color}; padding: 5px; margin-bottom: 2px; border-radius: 5px;")
        label.show()
    '''    
    def update_live_view(self, data):
        # Removed the cursor assignment as instructed
        ...
        # Callback function to handle the fetched laps
        def handle_fetched_laps(participant_name, recorded_laps, current_lap):
            ...
            # Find the participant in the sorted list to update their label
            for i, participant in enumerate(sorted_participants):
                if participant['mName'] == participant_name:
                    last_lap_str = self.format_lap_time(participant['mLastLapTimes'])
                    fastest_lap_str = self.format_lap_time(participant['mFastestLapTimes'])
        ...
        for i, participant in enumerate(sorted_participants):
            ...
            if current_lap < 3:
                ...
                # Use the DatabaseThread to fetch recorded laps
                self.db_thread.fetch_recorded_laps(participant_name, self.current_race_id, current_lap, handle_fetched_laps)

                # Commented out old SQL query code
                # cursor.execute('''
    ...
    def load_selected_race(self):
        ...
        if selected_index >= 0:
            ...
            # New Code
            def on_race_data_loaded(race_id, track_variation, laps_in_event, participants):
                ...
                for participant in participants:
                    ...
                    # Fetch lap times from DatabaseThread
                    def on_lap_times_loaded(participant_name, lap_times):
                    ...
                    print(f"before fetch lap times n Thread ID in operation: {threading.get_ident()}")
                    self.db_queue.put(('fetch_lap_times', (name, race_id, on_lap_times_loaded)))
            print(f"before load selected race Thread ID in operation: {threading.get_ident()}")
            self.db_queue.put(('load_selected_race', (race_index, on_race_data_loaded)))
