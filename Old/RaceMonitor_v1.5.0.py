...
from file_base64_strings import (High_scores_Background_img_base64, Race_results_Background_img_base64, Race_scores_Background_img_base64, liverace_liveview_img_base64, liverace_status_img_base64, icon_base64, sui_generis_rg_font_base64)
...
class DatabaseThread(QThread):
    ...
    def process_queue(self, operation, *args):
        try:
            ...
            elif operation == 'get_highscore_data': self.get_highscore_data()
...
class RaceMonitorApp(QMainWindow):
    ...
    def initialize_dropdown(self):
        ...
        self.db_queue.put(('get_highscore_data',())) #Send the request to the DatabaseThread to load highscore data on start
    ...
    def set_session_id(self, new_session_id, dato):
        ...
        if new_session_id is None:
            ...
            if self.session_id > 1:
                self.db_queue.put(('get_score_data',(self.session_id-1,))) # Get the score data for the current session ID
            else:
                self.db_queue.put(('get_score_data',(self.session_id,))) # Get the score data for the current session ID
    ...
    def handle_race_data_on_start(self, races, participants):
        ...
        if races:
            ...
            else: self.results_heading.setText("Select a Race to View previous races") 
        else:  
    ...
    def setup_result_view(self): # Content label for displaying loaded results
        ...
        self.results_heading = QLabel("Select a race to view previous races", self.results_view_widget)
    ...
    def setup_highscore_view(self): # Content label for displaying Final results
        ...
        # Add the content label for displaying the Header Data
        self.highscore_heading = QLabel("Select a track and car to view high Scores", self.highscore_view_widget)
        ...
        for i, participant in enumerate(sorted_participants): # Loop through each participant to update their corresponding label
            ...
            if participant['mLastLapTimes'] != -123 and participant['mName'] in self.previous_current_lap:
                if self.current_lap[participant['mName']] != self.previous_current_lap.get(participant['mName'], None):
                    self.live_status_label.setText(f"{participant['mName']} is now on lap {participant ['mCurrentLap']}")  # Update the status label
    ...
    def create_participant_text(self, participant,flag, last_lap_str, lap_times_str, total_time_str=""): # Helper function to create participant text with custom styling.self.initialize_dropdown
        ...
        if participant['mFastestLapTimes'] < self.best_lap_time and participant['mCurrentLap'] > 1 and participant['mFastestLapTimes'] != -123:
            ...
            self.live_status_label.setText(f"{participant['mName']} just got a new best lap time: {self.format_lap_time(self.best_lap_time)}")
    ...
    def previous_session(self):
        ...
        self.db_queue.put(('get_score_data',(self.session_id_dropdown,))) # Get the score data for the current session ID
        #self.final_heading.setText(f"Accumulated Score for Session {self.session_id_dropdown} Next Race will be session: {self.session_id}")
