...
class DatabaseThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                if operation == 'write_race':
                ...
                elif operation == 'get_latest_session_id':
                    print ("DB Get Latest Session_id")
                    logging.info("DB Get Latest Session_id")
                    self.get_latest_session_id(*args)
                    print ("Get Latest Session_id succeeded")
                    logging.info("Get Session_id succeeded")
                    ...
                    print ("DB Load Session IDs on start")
                    logging.info("DB Load Session IDs on start")
                ...
                elif operation == 'delete_db':
                    print("DB Delete DB")
                    logging.info("DB Delete DB")
                    self.delete_db()
    ...
    def delete_db(self):
        logging.info("DB Def delete db entered")
    ...
    def delete_race(self, race_id, callback):
        logging.info(f"DB Def delete:race def entered with race_id{race_id}")
        try:
        ...
        try:
            ...
            if races:
                for race in races:
                    ...
                    logging.info(f"DB Check if any Laps in race, Rows in laps: {rows_in_race}, Race ID: {race_id}")
                
                    # check if there are any participants in the race
                    self.cursor.execute('SELECT COUNT(*) FROM Participants WHERE RaceID = ?', (race_id,))
                    rows_in_participants = self.cursor.fetchone()[0]  # This will give the count of participants
                    logging.info(f"DB Check if any participants in race, Rows in participants: {rows_in_participants}, Race ID: {race_id}")
                ...
                if participants:
                    ...
                    print(f"DB Load race on start signal emitted")
    ...
    def load_sessionid_on_start(self):
        logging.info("DB Def load session ids on start def entered")
        try:
            self.cursor.execute('''
    ...
    def load_selected_race(self, race_id, callback = None):
        logging.info(f"DB Load selected race Def entered with race_id: {race_id}") 
        print(f"DB Load selected race Def entered with race_id: {race_id}") 
    ...
    def get_latest_session_id(self, callback=None):
        print("DB get get_latest_session_id entered")
        logging.info("DB Def get latest session ID entered")
        try:
    ...
    def get_latest_race_id(self, callback =None):
        logging.info("DB Def get latest race ID entered")
        try:
            ...
            if result:
                ...
                if callback and callable(callback): callback(last_race_id)
                    ...
                    print(f"No Callback, returning Rac_id: {last_race_id}")
    ...
    def write_race(self, data, session_id):
        logging.info(f"DB Def write entered with session_id: {session_id}")
        try:
    ...
    def finalize_race(self, data, lap_times_dict, flags_dict):
        ...
        print(f"Finalizing DB With participants:{participants} Lap times dict:{lap_times_dict} flags: {flags_dict}")
        logging.info(f"Finalizing DB, Lap times dict:{lap_times_dict} flags: {flags_dict}")    
        # Get the RaceID for the last
        race_id = self.get_latest_race_id()
        print(f"Writing Race with Race ID: {race_id}")
        # Check if there are any laps recorded for the last RaceID
        self.cursor.execute('SELECT COUNT(*) FROM Laps WHERE RaceID = ?', (race_id,))
        rows_in_race = self.cursor.fetchone()[0]  # This will give the count of laps
        ...
        for participant in participants:
            ...
            self.cursor.execute('''
                INSERT OR REPLACE INTO Participants (
                    RaceID, mName, mCarNames, mRacePosition, mFastestLapTimes, mLastLapTimes, lapscompleted, flags
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                race_id,
                ...
                len(lap_times_dict.get(participant_name, [])),
                flags_data
            ))
...
class MonitorThread(QThread):
    ...
    def set_session_id(self, new_session_id):
        ...
        if new_session_id is None:
            print(f"MT Session ID is None, setting to 1")
            self.session_id = 1
        else:            
    ...
    def run(self):
        while self.running:
            try:
                ...
                logging.info(f"MT Attempting to get data from the API.")
                ...
                #logging.info(f"MT Full API response: {json.dumps(data, indent=2)}")
                ...
                if participants:
                    ...
                    time.sleep(3) # Wait for 3 seconds before fetching the next data
                    self.db_queue.put(('get_latest_race_id', (self.set_race_id,)))  # Only pass the necessary data, not the function
                   #print(f"MT Race written to database.")


                    while self.running: #loop while race is running
                        try:
                            #self.race_id_updated.emit(self.race_id)  # Emit signal with the updated RaceID   
                            previous_ipaddress=self.ip_address
                            if self.race_loop_first_time:
                                #logging.info(f"MT RaceID is: {self.race_id}.")
                                #print(f"MT RaceID is: {self.race_id}.")
                                total_laps = data['eventInformation']['mLapsInEvent']
                            #logging.info(f"MT Attempting to get data from the API.")
                            #print(f"MT Attempting to get data from the API.")
                            self.ip_address = self.config_manager.read_ip_address()  # Read IP address before each API call
                            previous_data = data
                            with self.session.get(f'http://{self.ip_address}:8180/crest2/v1/api') as response:
                                data = response.json()
                                ...
                                if response.status_code == 200:
                                    ...
                                    logging.error(f"MT Unexpected status code {response.status_code} received from the API.")
                                    print(f"MT Unexpected status code {response.status_code} received from the API.")
                            ...
                            # Proceed with processing the valid data                             
                            participants = data.get('participants', {}).get('mParticipantInfo', [])    
                            if not participants or all(participant.get('mCurrentLap', 0) > total_laps for participant in participants):  # Check if all participants have completed every lap or if there is data.
                                if not participants:                                                       
                                    data = previous_data  # Revert to the previous data if no participants are found
                                    logging.info(f"MT No participants found , Race is over, breaking the loop")
                                    print(f"MT No participants found, Race is over.")
                                else:                              
                                    logging.info(f"MT All participants finished, Race is over, breaking the loop")
                                    print(f"MT All participants finished race, Race is over.")
                                self.first_time_run = True
                                break # Race is over, break the loop
                            
                            logging.info(f"MT Processing data for {len(participants)} participants.")
                            #print(f"MT Processing data for {len(participants)} participants. Current Race_ID {self.race_id}")
                            ...
                            for participant in participants:
                                ...
                                print(f"Name:{participant['mName']} Speed: {(participant.get('mSpeeds')} ")
                                if (participant.get('mCurrentLap', 0) == 2 and participant.get('mLastLapTimes') == -123) or (participant.get('mSpeeds') >5 and data['gameStates']['mGameState'] == 1):
                                ...
                                #print(f"MT Participant {participant['mName']} is on lap {participant.get('mCurrentLap', 0)}, number of laps: {lap_number} Current lap:{previous_lap} Latest Lap Time: {latest_lap_time}")
                                #logging.info(f"MT Participant {participant['mName']} is on lap {participant.get('mCurrentLap', 0)}, number of laps: {lap_number} Current lap:{previous_lap} Latest Lap Time: {latest_lap_time}")
                                #print(f"MT Should I store Lap, current lap: {participant.get('mCurrentLap', 0)} Last lap: {last_lap_counts.get(participant['mName'], 1)}")
                                if participant.get('mCurrentLap', 0) > last_lap_counts.get(participant['mName'], 1):
                                    print(f"MT Updated lap_times for {participant['mName']} {self.lap_times_dict[participant['mName']]}   at lap {participant.get('mCurrentLap', 0)}: {self.lap_times_dict[participant['mName']]}")
                                    logging.info(f"MT Updated lap_times for {participant['mName']} {self.lap_times_dict[participant['mName']]}   at lap {participant.get('mCurrentLap', 0)}: {self.lap_times_dict[participant['mName']]}")

                                    if participant.get('mLastLapTimes') != -123: 
                                        ...
                                        self.lap_times_dict[participant['mName']].append(participant.get('mLastLapTimes', None))
                                        ...
                                        print(f"MT Storing lap time for {participant['mName']}: Lap {participant.get('mCurrentLap', 0)}, Time {participant.get('mLastLapTimes', 0)},Last Lap:{participant.get('mLastLapTimes', None)} Driver Total Time: {driver_total_times[participant['mName']]} ")
                                last_lap_counts[participant['mName']] = participant.get('mCurrentLap', 0)
                                  
                            self.data_updated.emit(data) #Send the data to Live view 
                            #print(f"MT Data Sent to live view Race: Race_{race_id}.")
                            current_race_state = data['gameStates']['mGameState'] 
                            if current_race_state != self.previous_race_state:
                                print(f"MT Game state changed to {current_race_state}")  # Ensure console output remains
                                logging.info(f"MT Race state changed to {current_race_state}")
                                self.previous_race_state = current_race_state                            
                            #time.sleep(2)
                            current_race_state = data['gameStates']['mGameState']
                            ...
                            self.error_occurred.emit(f'MT Request Error: {str(e)}')
                            time.sleep(2)                            
                    logging.info(f"MT Race Race_{self.race_id} has ended. Current_Race_State: {current_race_state} self.running: {self.running}")
                    self.race_loop_first_time = True
                    print(f"MT Race Ended Finalizing Race for Race: Race_{self.race_id}, Current_Race_State: {current_race_state} self.running: {self.running}")
                    logging.info(f"MT Race Ended Finalizing Race for Race: Race_{self.race_id}, Current_Race_State: {current_race_state} self.running: {self.running}")
                    participants = data.get('participants', {}).get('mParticipantInfo', [])
                    
                    total_time_participant = {}
                    print(f"MT Driver Total times: {driver_total_times}")
                    logging.info(f"MT Driver Total times: {driver_total_times}")
                    if not driver_total_times:
                        print(f"MT No reason to write race, no one finished.")
                        logging.info(f"MT No reason to write race, no one finished.")
                    ...
                    print(f"MT Best Participant: {best_participant}")
                    logging.info(f"MT Best Participant: {best_participant}")
                    winner_data = next((p for p in participants if p['mRacePosition'] == 1), None)
                    winner = winner_data.get('mName', 'Unknown')
                    #print(f"MT Winner: {winner}")
                    #logging.info(f"MT Winner: {winner}")
                    for participant in participants:
                        ...
                        print(f"MT Participant: {participant['mName']}, Race Position: {participant.get('mRacePosition', 0)}, Total Laps: {total_laps} Tolal Time: {total_time_participant[participant['mName']]}")
                        logging.info(f"MT Participant: {participant['mName']}, Race Position: {participant.get('mRacePosition', 0)}, Total Laps: {total_laps} Tolal Time: {total_time_participant[participant['mName']]}")
                        participant_name = participant['mName']
                        current_lap = participant.get('mCurrentLap', 0)
                        print(f"MT Participant: {participant_name}, Current Lap: {current_lap}, Total Laps: {total_laps}")
                        print(f"MT Best Participant: {best_participant}, Winner: {winner}")
                        if participant['mName'] == winner and current_lap < total_laps:
                            print(f"MT Driver is {participant_name} same as {winner} and did not finish. That means False start!")
                            logging.info(f"MT Driver is {participant_name} same as {winner} and did not finish. That means False start!")
                            print(f"MT Adding Falsestart to Flags for {participant_name}")
                            logging.info(f"MT Adding Falsestart to Flags for {participant_name}")
                        ...
                        if current_lap <= total_laps: # Driver has fewer laps less than he should, either False start Or DNF
                            ...
                            print(f"MT Participant: {participant_name}, Flags: {self.driver_flags}")
                            logging.info(f"MT Participant: {participant_name}, Flags: {self.driver_flags}")
                    ...
                    self.flags_updated.emit(self.driver_flags)
                    print(f"MT Sending Data for the last time")
                    logging.info(f"MT Sending Data for the last time")
                    self.data_updated.emit(data) #Send the data to Live view to update the flags
                    self.db_queue.put(('finalize_race', (data, self.lap_times_dict,self.driver_flags)))
                    time.sleep (3)
...
class RaceMonitorApp(QMainWindow):
    ...
    def load_selected_race(self): # Function to be called when the data is loaded
        logging.info("RMA Load Selected selected race")
    ...
    def on_tab_changed(self, index):
        ...
        for widget_name in self.tab_widget_mapping.get(index, []): # Show only the widgets associated with the active tab
            if widget_name in self.labels:
                ...
                print(f"racestarted: {self.racestarted}, widget {widget_name}")
                if widget_name == 'new_session_button' and self.racestarted != True:
                    self.labels[widget_name].show()
                else:
                    if widget_name != 'new_session_button':
                        self.labels[widget_name].show()
    ...
    #    logging.info("RMA set race id def entered")
    #    self.race_id = new_latest_id
    #    #print(f"Racemonitorapp Race ID set to {self.race_id}")
        
    def on_first_live_view_run(self,event_info, participants):
        ...
        self.driver_flags = {}
        # Set Heading text
        heading_text = f"{event_info['mTranslatedTrackVariation']} ({event_info['mLapsInEvent']}) - {participants[0]['mCarNames']} -  {len(participants)} Drivers - Session ID: {self.session_id}  " # Update the heading with track variation and car names
    ...
    def update_live_view(self, data):
        ...
        if self.live_first_time_run: 
            self.on_first_live_view_run(event_info, participants)
        
        if self.gamestate == 2 and data['gameStates']['mGameState'] == 4:
             self.live_status_label.setText("Race Ending....")  # Update the status label"self.live_status_label.setText
             print(f"RMA Race Ending Label Set,  GamesState: {data['gameStates']['mGameState']}")
        self.gamestate = data['gameStates']['mGameState']
        ...
        for i, participant in enumerate(sorted_participants): # Loop through each participant to update their corresponding label
            ...
            flag = self.driver_flags.get(participant['mName'], None)
