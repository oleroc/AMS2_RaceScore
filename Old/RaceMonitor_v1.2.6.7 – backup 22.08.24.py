...
class DatabaseThread(QThread):
            ...
            if result:
                ...
                if callback and callable(callback):
                    if rows_in_race > 0:
                        ...
                        callback(last_race_number - 1)  # Pass the previous number if no laps exist
            else:
    ...
    def get_race_index(self, callback=None):
        try:
            ...
            self.cursor.execute('SELECT COUNT(*) FROM Laps WHERE RaceIndex = ?', (last_race_index,))
            rows_in_race = self.cursor.fetchone()[0]  # This will return the count of laps for that race

            if callback and callable(callback): # Process the result and call the callback with the new race count
                if last_race_index:
                    ...
                    if rows_in_race: callback(last_race_number)
                    else: callback(last_race_number-1)
                else:
                    callback(None)
            else:
                print("Callback is not callable or is None")
            ...
            try:
                ...
                if participants:
                                ...
                                if current_lap > last_lap_counts.get(participant_name, 1):
                                    if current_lap - 1 <= len(lap_times):
                                        ...
                                        if current_lap == 2 and latest_lap_time == -123:
                                            ...
                                            if participant_name not in self.driver_flags:
                                                self.driver_flags[participant_name] = []  # Initialize an empty list for the driver's flags
                    ...
                    for participant in participants:
                        ...
                        total_time_participant[participant['mName']] = driver_total_times.get(participant['mName'], 0)
                        participant_name = participant['mName']
                        current_lap = participant.get('mCurrentLap', 0)
                        ...
                        if current_lap <= total_laps: # Driver has fewer laps less than he should, either False start Or DNF
                            ...
                            has_falsestart = "Falsestart" in self.driver_flags.get(participant_name, [])
                            if not has_falsestart:
                                if participant_name not in self.driver_flags:
                                    self.driver_flags[participant_name] = []  # Initialize an empty list for the driver's flags                               
                                print(f"Adding DNF to Flags for {participant_name}")
                                self.driver_flags[participant_name].append ('DNF')
                                #self.driver_flags.append([participant_name, 'Falsestart'])# Add 'DNF' to the Flags list                            
                                self.flags_updated.emit(self.driver_flags)
                        best_participant = min(total_time_participant, key=total_time_participant.get)
                        if best_participant != winner:
...
class RaceMonitorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        print(f"Self DB Empty set to True: {self.db_empty}")
        self.runs = 0
    ...
    def update_live_view(self, data):
        ...
        for i, participant in enumerate(sorted_participants): # Loop through each participant to update their corresponding label
            ...
            if current_lap < 3: # For the driver being processed.
                ...
                participant_text = self.create_participant_text(participant,flag, last_lap_str, fastest_lap_str, last_lap_str, last_lap_str)
    ...
    def display_score(self):
        print(f"Is db empty? Value: {self.db_empty}")
        print(f"Display Score Result: {self.session_id}")
        if self.db_empty:
             return
    ...
    def calculate_score(self, races, participants, laps):
        ...
        for race in races:
            ...
            for lap in laps:
                ...
                #best_lap = lap[4]  # Best lap time in the participants table
            ...
            ranked_drivers = sorted(driver_total_times.items(), key=lambda x: (flag_priority(driver_flags.get(x[0], "No Flag")),x[1]))  # Sort primarily by flag priority
                    #x[1])  # Sort by time for No Flag, inf for others
            ...
            for mName, _ in ranked_drivers:
                ...
                #final_score_str = '<br>'.join([f"{mName}: {score}" for mName, score in sorted(total_scores.items(), key=lambda x: total_scores.get(x[0], 0), reverse=True)])
            ...
            #else:
                #print("No drivers completed a lap.")
                # Handle the case where no laps were completed, for example:
                # Assign no best lap bonus, or handle ranking differently if needed.    
                    #best_lap_driver = min(best_lap_times.items(), key=lambda x: x[1])[0]
                    # Award the best lap bonus to that driver
