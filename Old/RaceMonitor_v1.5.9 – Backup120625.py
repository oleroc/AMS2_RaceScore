...
class MonitorThread(QThread):
    ...
    async def get_api_data(self):
        ...
        try:
            ...
            async with self.session.get(f'http://{self.ip_address}:8180/crest2/v1/api', timeout=3) as response:
                ...
                if response.status == 200: #Ensure the response content is valid JSON
                    try:
                        if data is None:
                            ...
                            if not self.connecton_restored_message_shown:
                                ...
                                self.api_signal_status_updated.emit(False, "") # Emit signal for successful connection restoration to clear the error message
                    ...
                    if not error_message_shown:
                        ...
                        self.api_signal_status_updated.emit(True,f'Game Not Started: Unexpected status code {response.status}')
                        error_message_shown = True
                        self.connecton_restored_message_shown = False
            ...
            self.api_signal_status_updated.emit(True,f'Connection Error: Check IP Address in Config.ini:')
            if not error_message_shown:
    ...
    async def auto_scan_active_computers(self):
        while self.running:
        #self.active_computers.clear() #skal gjøres i andre classe med on mottaksdef.
            for i, ip in enumerate(self.available_ips): #Må sendes fra annen class med emit.
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(0.05)  # 50ms timeout
                try:
                    result = sock.connect_ex((ip, 8180))
                    if result == 0:
                        self.active_computers.append(ip) #skal gjøres i andre classe med on mottaksdef.
                        print(f"RMA Line {inspect.currentframe().f_lineno} Computer {i+1} is active: {ip}")
                    else:
                        self.sim_radio_group.button(i).setEnabled(False) #skal gjøres i andre classe med on mottaksdef.
                finally:
                    sock.close()
                await asyncio.sleep(300)  # Small delay to avoid overwhelming the network         
...
class RaceMonitorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.enable_disable_cp_button = QPushButton("Enable CP", self)
        self.labels['find_computers_button'] = self.enable_disable_cp_button
        ...
        self.tab_widget_mapping = {  # Widget visibility mapping for each tab
            ...
            5: ['find_computers_button', 'Enable CP']  # Widgets for Driver Info
        }
