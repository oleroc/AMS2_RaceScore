...
class DatabaseThread(QThread):
    ...
    def finalize_race(self, data, race_index, lap_times_dict, flags):
        ...
        print(f"Finalizing DB DataRace index: {race_index} Lap times dict:{lap_times_dict} flags: {flags}")        
        self.cursor.execute('SELECT RaceID FROM Races WHERE RaceIndex = ?', (race_index,))
        race_id = self.cursor.fetchone()[0]
                ...
                if participants:
                    ...
                    for participant in participants:
                        ...
                        if current_lap <= total_laps: # Driver has fewer laps less than he should, either False start Or DNF
                            ...
                            has_falsestart = any(driver_flag[0] == participant_name and driver_flag[1] == "Falsestart" for driver_flag in self.driver_flags)
                            if not has_falsestart:
                                print(f"Adding DNF to Flags for {participant_name}")
                                self.driver_flags.append([participant_name, 'DNF'])  # Add 'DNF' to the Flags list                            
                                self.flags_updated.emit(self.driver_flags)
...
class RaceMonitorApp(QMainWindow):
    ...
    def update_live_view(self, data):
        ...
        for i, participant in enumerate(sorted_participants): # Loop through each participant to update their corresponding label
            ...
            for driver_flag in self.driver_flags:
                if driver_flag[0] == participant_name:  # Compare participant's name with the first element in each driver_flag list
                    flag = driver_flag[1]  # The flag is the second element in the driver_flag list
                    break            
            current_lap = participant.get('mCurrentLap', 0)
        ...
        for race in races:
            ...
            ranked_drivers = sorted(
                ...
                key=lambda x: (
                    ...
                    float('inf') if driver_flags.get(x[0]) != 'No Flag' else x[1]  # Push non-No Flag drivers down
                )
            )
