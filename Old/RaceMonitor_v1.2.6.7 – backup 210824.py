...
class DatabaseThread(QThread):
    ...
    laps_fetched_signal = pyqtSignal(str, list, int)
    load_race_on_start_signal = pyqtSignal(list)
       ...
       # except Exception as e:
        ...
        try:
           ...
           # if self.cursor is None:
            ...
            self.cursor.execute('SELECT LapNumber, LapTime FROM Laps WHERE RaceID = ? AND mName = ?',
    ...
    def load_race_data_on_start(self,callback=None):
        try:
            ...
            self.load_race_on_start_signal.emit(races)
    ...
    def get_race_index(self, callback=None):
        try:
            ...
            print(f"Race index fetched: {last_race_index}")
            # Process the result and call the callback with the new race count
            if callback and callable(callback):
                if last_race_index:
                    race_str = last_race_index[0]  # This should be something like 'Race_52'
                    print(f"Race string: {race_str}")
                    last_race_number = int(race_str.split('_')[1])
                    callback(last_race_number)
                else:
                    callback(0)
            else:
                print("Callback is not callable or is None")
    ...
    def get_session_id(self, callback=None):
        ...
        try:
            ...
            result = self.cursor.fetchone()
            session_id = result[0] if result and result[0] is not None else 1
    ...
    def finalize_race(self, data, race_index, lap_times_dict, flags):
        ...
        for participant in participants:
            ...
            # Find the flag for this participant
            flags_data = 'None'
            for flag in flags:
                if flag[0] == participant_name:
                    flags_data = flag[1]
                    break
            self.cursor.execute('''
            ...
            try:
                ...
                if participants:
                    ...
                    def handle_race_id (race_id):
                        ...
                        print(f"Race ID received from DB Race id: {self.current_race_id}")
                            ...
                            for participant in participants:
                                ...
                                if current_lap > last_lap_counts.get(participant_name, 1):
                                    if current_lap - 1 <= len(lap_times):
                                        ...
                                        def receive_number_of_rows(rows):
                                            self.number_of_rows = rows
                                        self.db_queue.put(('get_number_of_rows',self.current_race, self.receive_numbers_of_rows))
                                        if (current_lap - 1) == 2:
                                            if self.number_of_rows >= participant.get('mRacePosition', None):
                                                self.driver_flags.append([participant_name, 'Falsestart'])
...
class RaceMonitorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        #self.race_monitor_app = queue.Queue()
        #self.db_queue = db_queue
        ...
        # Widget visibility mapping for each tab
    ...
    def update_flags(self, flags):
        # Update the flags label with the message
    ...
    def handle_race_data(self, races):
        self.dropdown.clear()  # Add this line to clear the dropdown
        if races:
    ...
    def handle_sessionid_data(self, sessionids):
        ...
        if sessionids:
            ...
            # Load the latest race results by default
            self.dropdown_sessionid.setCurrentIndex(self.dropdown_sessionid.count() - 1)
        # Send the request to the DatabaseThread to load race data on start
        # self.db_queue.put(('load_race_data_on_start', self.handle_race_data,))
    #'''       
    ...
    def on_first_live_view_run(self):
        ...
        self.load_race_data_on_start()
        print(f"Knapp oppdatert med nytt race.Race_ID{self.current_race_id}")
        #self.load_race_data_on_start()
        self.final_content.setText("Score - Waiting for a new race to start...")
        #print("Knapp populert 2 ganger")
        
        
    def update_live_view(self, data):
        if self.first_time_run:
            self.on_first_live_view_run()
        self.runs += 1
        #self.data_save = data
        ...
        heading_text = f"{event_info['mTranslatedTrackVariation']} ({event_info['mLapsInEvent']}) - {participants[0]['mCarNames']}" # Update the heading with track variation and car names
    ...
    def display_score(self):
        ...
        # self.previous_best_lap_time = None
        # self.best_lap_time = None
        # Load any existing race data only applicable when there is only one race in the DB.
        # self.load_race_data_on_start()
    ...
    def calculate_score(self, races, participants, laps):
        ...
        for race in races:
            ...
            number_of_laps = race[3]  # mLapsInEvent is at index 3 in the races tuple
            ...
            for participant in participants:
                ...
                # Store the flag and best lap time for each driver
            ...
            ranked_drivers = sorted(
                ...
                key=lambda x: (
                    flag_priority(driver_flags.get(x[0], "No Flag")),  # Use custom flag priority
                    x[1]  # Then by total race time
                )
            )               

            # Assign scores for the current race
            place = 1
            for mName, total_time in ranked_drivers:
                race_scores[race_id][mName] = score_table.get(place, 0)
                
                last_positions[mName] = place # Track the last position for each driver
                # Track gold, silver, and bronze medals
                if place == 1:
                ...
                #if best_lap_times[mName] < float('inf'):  # If a valid best lap exists
                #    race_scores[race_id][mName] += best_lap_bonus
            ...
            race_scores[race_id][best_lap_driver] += best_lap_bonus# Add the best lap bonus to the driver with the best lap time
        ...
        # Create the final score string, listing drivers from highest to lowest total score
        #final_score_str = '<br>'.join([f"{mName}: {score}" for mName, score in sorted(total_scores.items(), key=lambda x: x[1], reverse=True)])

        # Update the UI with the calculated scores
        #self.final_content.setText(f"Score:<br>{final_score_str}")
        number_of_races = len(processed_race_ids)  # Determine the number of unique races after processing
    ...
    def format_score_view(self, total_scores, race_count, last_positions, medal_counts):
        ...
        #middle = '<table style="width: 100%; border-collapse: separate; border-spacing: 0 5px;>'
        rows = ''
        ...
        #num_races_str = f'<div style="text-align: center;">Number of Races {race_count}</div>'

        # Combine the number of races with the score table
        #final_score_str = num_races_str + final_score_str
    ...
    '''    
    def calculate_score(self, races, participants, laps):
        # Retrieve race details (race is a tuple from the load_selected_race method)
        #race = self.race_score  # This should be set when the signal is emitted
        #participants = self.participants_score  # This should also be set when the signal is emitted

        # Correctly retrieve the total number of laps from the race data
        number_of_laps = races[3]  # mLapsInEvent is at index 2 in the race tuple
        print(f"Number of laps: {number_of_laps}")
        session_group_nr = races[3]
        print(f"Session Group Number: {session_group_nr}")
        # Initialize driver flags from participants' flags
        self.driver_flags = []
        #Calculate the total time for each driver for each race
        <find total race time for each participant for each race from laps table>
        <find flag and best lap time for each participant for each race in participant table>
        ...
        <rank the drivers by flags 1: no flags, 2: DNF, 3: Falsestart>
        <use this rank so that first the drivers with no flags get score for the the first positions based on total_race_time, then DNF drivers, then Falsestart drivers>
        <assign scores to drivers based on flags, score_table and best_lap_bonus for each race>
        Like this:
        Race_1:
        1. Driver_1, no flag, best total time, 1_place (from config.ini)
        2. Driver_2, no flag, second best total time, 2_place
        3. Driver_3, DNF, best total time from all DNF 3_place
        4. Driver_4, Falsestart, best total time from all Falsestart 4_place
        Driver with best lap regardless of flag +best_lap points from config.ini.
        
        <add up scores for each race>
         # Create the final score string
        final_score_str = '<br>'.join(<all drivers with total score for all races listed from highest score and down.)
        ...
        for lap in laps:
            race_id = lap[1] #race_id is the second item in the lap tuple
            participant_name = laps[2] # mName is the third item in the lap tuple
            Lap_number = laps[3] # Lap number is the fourth item in the lap tuple
            lap_time = laps[4] # Lap time is the fifth item in the lap tuple
            # Make table of participants total lap time for each race

        for participant in participants:
            
            lap = participant[4] # mName is the fifthh item in the Laps
            flag = participant[6] # flags is the seventh item in the participant tuple
            Fastest_lap = participant[4] # Fastest lap is the Fifth item in the participant tuple
            Race_id = participant[0] # Race ID is the first item in the participant tuple
            completed_laps = sum(1 for lap if lap[2] == participant_name)  # mName is the third item in the lap tuple
            #completed_laps = sum(1 for lap in self.laps_score if lap[2] == participant_name and lap[1] == session_group_nr)  # mName is the third item in the lap tuple, and filtering by session_group_nr

        
            # Add the flag (if any) to the driver flags
            if flag:
                self.driver_flags.append([participant_name, flag])
        ...
        # Initialize dictionaries for lap sums and scores
        driver_total_times = {}
        driver_scores = {}

        # Calculate the sum of lap times for each driver
        for lap in laps:
            #print(f"Score record: {score}")
            #break  # Just to see one example
            #LapID, race_id, mName, lap_number, lap_time = laps
            mName = lap[2]
            #if race_id != race[0]:  # Correct comparison to ensure only laps from the correct race are considered
               # continue
            if mName not in driver_total_times:
                driver_total_times[mName] = 0
            driver_total_times[mName] += lap_time

        # Rank drivers by total lap time
        ranked_drivers = sorted(driver_total_times.items(), key=lambda x: x[1])

        # Adjust scores based on driver flags
        clean_drivers = [d for d in ranked_drivers if d[0] not in [flag[0] for flag in self.driver_flags]]
        dnf_drivers = [d for d in ranked_drivers if d[0] in [flag[0] for flag in self.driver_flags if flag[1] == 'DNF']]
        falsestart_drivers = [d for d in ranked_drivers if d[0] in [flag[0] for flag in self.driver_flags if flag[1] == 'Falsestart']]

        # Assign scores to clean drivers
        for i, (mName, _) in enumerate(clean_drivers):
            place = i + 1
            driver_scores[mName] = score_table.get(place, 0)

        # Assign scores to DNF drivers
        for i, (mName, _) in enumerate(dnf_drivers):
            place = len(clean_drivers) + i + 1
            driver_scores[mName] = score_table.get(place, 0)

        # Assign scores to falsestart drivers
        for i, (mName, _) in enumerate(falsestart_drivers):
            place = len(clean_drivers) + len(dnf_drivers) + i + 1
            driver_scores[mName] = score_table.get(place, 0)

        # Add best lap bonuses
        for record in participants:
            race_id, mName, car_name, Race_Position, fastest_lap, Last_lap, flag = record
           # if race_id != race[0]:  # Ensure it matches the correct race ID
               # continue

            if mName in driver_scores:
                driver_scores[mName] += best_lap_bonus

        # Create the final score string
        final_score_str = '<br>'.join([f"{mName}: {score}" for mName, score in sorted(driver_scores.items(), key=lambda x: x[1], reverse=True)])
    ...
    def calculate_score(self, scores, best_laps_list):
        # Retrieve participants and race details from the database
        participants = self.participants_score  # This is a list of dictionaries representing each participant
        race = self.race_score

        # Retrieve the total number of laps for the race
        number_of_laps = race[2]  # Assuming race[3] holds the total number of laps in the race
        ...
        for participant in participants:
            ...
            flag = participant[5]  # Assuming 'flags' contains the participant's flag status
            completed_laps = sum(1 for lap in self.laps_score if lap[2] == participant_name)

            # If the participant did not complete the required number of laps and has no flag, mark as DNF
            if completed_laps < number_of_laps and not flag:
                flag = 'DNF'
        ...
        # Calculate the sum of lap times for each driver
        for score in scores:
            race_id, mName, lap_number, lap_time = score
            if mName not in driver_total_times:
                driver_total_times[mName] = 0
            driver_total_times[mName] += lap_time
        ...
        # Add best lap bonuses
        for mName, lap_time in best_laps_list:
            if mName in driver_scores:
                driver_scores[mName] += best_lap_bonus
    ...
    def calculate_score(self, scores, best_laps_list):
        # Find DNF Drivers
        #<new data source>
        #participants = self.participants_score
        #laps = self.laps_score
        race = self.race_score
        #</new data source>
        
        #participants = self.participants_score['mParticipantInfo']
        print(f"self.race_score: {self.race_score[0]}")
        #number_of_laps = self.race_score[3]
        number_of_laps = 1 #midlertidig variabel

        for participant in self.participants_score:
            participant_name = participant['mName']
            completed_laps = participant.get('mCurrentLap', 0)

            # Check if the driver did not complete the required number of laps
            if completed_laps < number_of_laps:
                self.driver_flags.append([participant_name, 'DNF'])  # Add DNF to driver flags
        # Load the score table from the config file
        config = configparser.ConfigParser()
        config.read('config.ini')
    ...
    def calculate_score(self, scores, best_laps_list):
        ...
        self.driver_flags
        <driver flags variable>
        [Sven, Falsestart]
        [Thiago Izequiel  (AI), DNF]
        [Lee Chorley  (AI), DNF]
        </driver flags variable>
        self.score_table = <read from config file>
        <config.ini content>
                [Score Table]
        1_place = 25
        2_place = 18
        ...
        <find sum of laps for all mName in each race>
        score_name = <Calculate score for each race with the best total lap time and add best_lap points from best_laps_list for each race >
        <Drivers without flags will be give scores for the first places ranked by total time, then DNF will get next places by total, then falsestart last places by total time  >
        final_score_str = <calculate score here>
        calculated_core_str = final_score_str
        self.final_content.setText(f"Score:<br>{calculated_core_str}")
    '''       
    def load_selected_race(self):
        print("Load Selected race def entered")
                # Function to be called when the data is loaded
    ...
    def on_race_loaded(self, race, participants, laps):
        #self.participants_score = participants
        #self.laps_score = laps
        #self.race_score = race
        ...
        if race_id:
            ...
            for participant in participants:
                ...
                #lap_times_str = ", ".join(f"{int(lap[4] // 60)}:{lap[4] % 60:05.2f}" for lap in participant_laps)
                ...
                #f"Fastest Lap:<span style='font-weight:normal; color:#78281f;'>{fastest_lap_str}</span> - "
