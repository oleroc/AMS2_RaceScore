...
class DatabaseThread(QThread):
    ...
    def get_latest_session_id(self, callback=None):
        try:
            ...
            if callback:
                callback(None, None)
                print(f"DB Line {inspect.currentframe().f_lineno}  no session stored, sending default 1: {e}")
                logging.info(f"DB Line {inspect.currentframe().f_lineno} no session stored, sending default 1: {e}")
...
class MonitorThread(QThread):
    ...
    async def race_running(self, data):
        ...
        #print(f"MT Line {inspect.currentframe().f_lineno} Data to be written to database: race index: {race_index} Sesson id: {self.session_id}")
...
class RaceMonitorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setGeometry(50, 50, int(self.config['config']['width']), int(self.config['config']['height']))
        ...
        # self.layout.addWidget(self.calendar_widget, Qt.AlignTop)
    ...
    def navigate_left(self):
        ...
        self.calendar_widget.setSelectedDate(previous_month)
        self.update_month_label(previous_month)
    ...
    def navigate_right(self):
        ...
        self.update_month_label(next_month)

    def update_month_label(self, date):
        # Update the month and year label
        self.current_month_label.setText(date.toString("MMMM yyyy"))
    
    def on_date_selected(self, date):
        # Handle date selection from the calendar
        print(f"Selected date: {date.toString('yyyy-MM-dd')}")
        # Load the corresponding race for the selected date
        self.load_race_for_date(date)

    def load_race_for_date(self, date):
        # Example function to load a race for the selected date
        selected_date_str = date.toString("yyyy-MM-dd")
        print(f"Loading race for date: {selected_date_str}")
        # Filter races by the selected date and load the race
        for race in self.races:  # Assuming self.races contains all race data
            race_date = race[3]  # Assuming the date is the 4th element in the race tuple
            if race_date == selected_date_str:
                self.load_selected_race(race)
                break
    ...
    def set_session_id(self, new_session_id, dato):
        ...
        if new_session_id is None:
            ...
            self.session_id = 1
            print(f"RMA Line {inspect.currentframe().f_lineno} latest_session_id set to: {self.latest_session_id}")
            ...
            if dato != datetime.now().date():
                ...
                print(f"RMA Line {inspect.currentframe().f_lineno} Session ID is not for today, setting to +1: self.sesssion_id: {self.session_id} self.latest_session_id: {self.latest_session_id}")
                # self.db_queue.put(('load_score_data',(self.session_id-1,))) # Get the score data for the current session ID
            else:
                self.session_id = new_session_id
                print(f"RMA Line {inspect.currentframe().f_lineno} Session ID is for today, setting to {self.session_id}. Self.latest_session_id: {self.latest_session_id}")
            self.session_id_updated.emit(self.session_id)
            print(f"RMA Line {inspect.currentframe().f_lineno} set_session_id. Latest DB Session ID set to new_session_id: {new_session_id}")
    ...
    def on_race_loaded(self, race, participants, laps):
        ...
        self.dropdown_races.setCurrentIndex(0)  # Reset the dropdown to the default item
    ...
    def display_score(self):
        ...
        if 0 < self.session_id_dropdown <= self.latest_session_id: # Check if the session ID is valid and greater than the latest session ID
            ...
            self.error_label = QLabel("No valid session ID selected", self.final_view_widget)
            self.error_label.setStyleSheet("font-size: 14px;font-weight:bold; color: red;")
            ...
            print(f"RMA Line {inspect.currentframe().f_lineno} No valid session ID selected. Index removed")
