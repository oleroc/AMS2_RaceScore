...
from file_base64_strings import (lap_bg_img_base64, pos_bg_img_base64,laptime_bg_img_base64, speedometer_bg_img_base64,tachometer_bg_img_base64,needle_bg_img_base64, Driver_Statistics_Background_img_base64, High_scores_Background_img_base64, Race_results_Background_img_base64, Race_scores_Background_img_base64, liverace_liveview_img_base64, liverace_status_img_base64, icon_base64, sui_generis_rg_font_base64)
...
class RaceMonitorApp(QMainWindow):
    ...
    def update_live_view(self, data):
        ...
        self.sorted_participants = sorted_participants  # Store sorted participants for later use
...
# --- New Monitored Driver Tab Functionality (injected) ---
def update_monitored_driver_tab(self, data):
    monitored_driver = data.get('monitored_driver')
    if not monitored_driver:
        return
    speed = monitored_driver.get('mSpeeds', 0.0) * 3.6
    rpm = monitored_driver.get('mEngineRps', 0.0) * 60.0
    gear = monitored_driver.get('mGear', 0)
    self.speed_label.setText(f"{speed:.0f} km/h")
    self.rpm_label.setText(f"{rpm:.0f} RPM")
    self.gear_label.setText(str(gear))
    # Rotate needle pixmap logic goes here
