from re import L, T; from timeit import Timer; import sys, re, requests, sqlite3, queue, json, time, traceback, logging, os, asyncio, aiohttp, math, base64, configparser
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QSizePolicy, QVBoxLayout, QWidget, QPushButton, QComboBox, QHBoxLayout, QTabWidget, QSpacerItem, QMessageBox
from PyQt5.QtGui import QPixmap, QIcon
from PyQt5.QtCore import QThread, pyqtSignal, QMetaObject, Qt, Q_ARG, QTimer, QObject
from datetime import datetime
from image_base64_strings import (bronse_img_base64, gold_img_base64, lastplace_img_base64, name_img_base64, place_img_base64, points_img_base64, silver_img_base64, table_background_img_base64, liverace_liveview_img_base64, liverace_status_img_base64, icon_base64)
...
class DatabaseThread(QThread):
    ...
    def finalize_race(self, data, lap_times_dict, flags_dict, pit_stops_dict):
        try:
            ...
            if read_cp_status:
                logging.info("Emitted write_cp_signal")
...
class MonitorThread(QThread):
    ...
    async def race_running(self, data):
        ...
        print(f"Api Fetch time, average execution time: {average_time:.6f} seconds, maximum execution time: {max_time:.6f} seconds, minimum execution time: {min_time:.6f} seconds")
        if self.session is not None: await self.session.close()
        self.session = None
...
class RaceMonitorApp(QMainWindow):
    ...
    def handle_race_data_on_start(self, races, participants):
        ...
        if races:
            ...
            self.results_content.setText("No Races in DB")     
    ...
    def on_race_loaded(self, race, participants, laps):
        ...
        if race_id:
            ...
            for participant in participants:
                name, race_position, fastest_lap, last_lap, car, flags, pits = participant
                ...
                f"Pits: <span style='font-weight:normal; color:#78281f;'>{participant[6]}</span> - "
    ...
    def on_first_live_view_run(self, data):
        ...
        participants = data['participants']['mParticipantInfo']
        if len(self.sessionids) < 1: self.final_content.setText("Score - Waiting for a new race to finish...")
    ...
    def update_live_view(self, data):
        ...
        for i, participant in enumerate(sorted_participants): 
            ...
            participant_text = self.create_participant_text(participant,flag, last_lap_str, lap_times_str, total_time_str) # Create the participant text using the utility function
    ...
    def format_lap_time(self, lap_time): # Helper function to format lap time from seconds to 'MM:SS.ss' format.
        if lap_time is None or lap_time == -123.0: return "No Valid Lap!"
        minutes = int(lap_time // 60)
        seconds = lap_time % 60
        return f"{minutes}:{seconds:05.2f}"
    
    def create_participant_text(self, participant,flag, last_lap_str, lap_times_str, total_time_str=""): # Helper function to create participant text with custom styling.self.initialize_dropdown
        ...
        if lap_time_updated or speed_time_updated:
            ...
            f"Best Top Speed:  {self.top_speed_driver} Top Speed: {self.best_top_speed *3.6:.0f} Km/t "
            f" Best time: {self.best_lap_driver} Best Lap in race: {self.format_lap_time(self.best_lap_time) if self.best_lap_time != 999 else 'No Valid Lap!'}")
            speed_time_updated = False
            lap_time_updated = False
        ...
        if len(lap_times_newstr) > 17: lap_times_newstr = lap_times_newstr[len(lap_times_newstr) - 17:]
        lap_times_str = ", ".join(lap_times_newstr)

        if participant['mCurrentLap'] -1 == self.event_info['mLapsInEvent']: race_status = "<span style='color:#FF0000;font-weight:bold'>(Finished!)</span>"
        else: race_status = ""
        ...
        spaces = (f"<span style='color:{self.top_background_color if participant['mFastestLapTimes'] == self.best_lap_time else self.background_color};'><br>----{'-|' if participant['mRacePosition'] > 9 else ''}<span style='color:#00FF00;'>")
    ...
    def calculate_score(self, races, participants, laps):
        ...
        if races: # Iterate over each race
            ...
            self.final_content.setText("All races Deleted")
            self.session_id = 1
            logging.info(f"Self selected index: {self.selected_index}")
            print(f"Self selected index set to: {self.selected_index}")
            self.session_id_updated.emit(self.session_id)
            self.final_heading.setText(f"Next Race will be session: {self.session_id}")
    ...
    def format_score_view(self, total_scores, race_count, last_positions, medal_counts, races):
        ...
        '''
        table_header = f'''  
        <table style="text-align: left; background-image: url('data:image/png;base64,{table_background_img_base64}');width:100%; border-spacing: 0 5px;">
            <tr>
        ...
        for i, (mName, score) in enumerate(sorted_scores, start=1):
            ...
            rows += f''' 
                ...
                <td style="background-image: url('data:image/png;base64,{place_img_base64}'); text-align: center; vertical-align: middle;">{i}</td>
                <td style="background-image: url('data:image/png;base64,{name_img_base64}'); padding-left: 10px;">{mName}</td>
                <td style="background-image: url('data:image/png;base64,{lastplace_img_base64}'); text-align: center;">{last_pos}</td>
                <td style="background-image: url('data:image/png;base64,{points_img_base64}'); text-align: center;">{score}</td>
                <td style="background-image: url('data:image/png;base64,{gold_img_base64}'); text-align: center;">{gold}</td>
                <td style="background-image: url('data:image/png;base64,{silver_img_base64}'); text-align: center;">{silver}</td>
                <td style="background-image: url('data:image/png;base64,{bronse_img_base64}'); text-align: center;">{bronze}</td>
                </tr>
            '''
    ...
    def delete_selected_race(self):
        ...
        if selected_index >= 0:
            ...
            QTimer.singleShot(1000, self.after_timer)

    def after_timer(self):
        new_text= self.results_content.text() + "\n" "Race Deleted"
        self.results_content.setText(new_text)  
